...
export default function DemoPage() {
  ...
  return <div className="wave-night night-secondary night-demo"><WaveHeader current="planner" /><main className="demo-page" id="demo-main"><header><h1>[시연] 예시 여행으로 먼저 해보세요</h1><p>장소·편의시설·운영시간·이동시간은 가상의 예시이며, 이미지는 AI 생성 삽화입니다. 실제 시설의 이용 가능성을 뜻하지 않으며 실제 관광정보와 저장된 여행은 변경하지 않습니다.</p><a href="/planner">실제 여행 설계로 돌아가기 →</a></header>
    ...
    {pending && <div className="demo-proposal" role="region" aria-label="변경안"><h3>변경안</h3><p>{pending === 'rest' ? '첫 장소 뒤에 20분 휴식을 넣고 이후 시작 시각을 조정합니다.' : '첫 장소의 체류 시간을 90분으로 바꾸고 이후 시작 시각을 조정합니다.'} 장소 순서와 이동시간은 유지합니다.</p><button onClick={apply}>일정에 적용</button><button disabled={!hydrated} onClick={() => setPending(null)}>취소</button></div>}
    <p role="status">{notice}</p><ol className="demo-timeline">{day.entries.map(entry => <li key={entry.place.id}><strong>{entry.startsAtLabel}</strong><div><h3>{entry.place.name}</h3><p>체류 {entry.visitMinutes}분 · 이동 {entry.travelMinutes}분{entry.breakMinutes ? ` · 휴식 ${entry.breakMinutes}분` : ''}</p></div></li>)}</ol></section>
    <aside className="demo-proposal"><h2>나루와 실제 여행 만들기</h2><p>이 예시의 버튼은 기능 시연입니다. 실제 나루와 대화하려면 여행 설계에서 ‘나루와 계획하기’를 선택하세요.</p><a href="/planner">나루와 여행 설계하기 →</a></aside><DemoStories /></main><SiteFooter /></div>;
}
