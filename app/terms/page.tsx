...
export const metadata: Metadata = {
  title: "서비스 이용약관",
  description: "WAVE의 서비스 범위, 계정, 사용자 콘텐츠와 운영 기준을 안내합니다.",
...
export default function TermsPage() {
  return <main className="wave-night night-secondary policy-page" id="main">
    ...
    <header><p>TERMS OF SERVICE</p><h1>여행 결정에 필요한 근거와<br />확인할 한계를 함께 봅니다.</h1><span>약관 버전 1.1 · 시행일 2026년 9월 20일</span></header>
    ...
    <article className="policy-article policy-terms">
      <section id="terms-purpose"><p className="policy-section-kicker">제1조 · 목적과 서비스 범위</p><h2>여행 판단을 돕는 정보 서비스</h2><p>이 약관은 WAVE와 사용자의 서비스 이용 기준을 정합니다. WAVE는 경상남도 무장애 여행을 위해 공식 관광·편의·날씨·교통·지도 정보와 여행자 경험을 연결하고, 조건별 추천·일정·경로·공유 기능을 제공합니다.</p><p>WAVE는 한국관광공사, 경상남도, 지도·교통·숙박·시설 제공기관의 공식 운영 서비스가 아닙니다. 실제 예약, 운송, 시설 제공과 현장 안전에 관한 계약은 각 제공기관과 사용자 사이에 성립합니다.</p><p>출발 전 확인 목록은 사용자가 준비할 항목을 정리하는 선택 기능입니다. WAVE는 보험이나 금융 상품을 추천·비교·중개하지 않으며, 가입 또는 보장 여부를 판단하지 않습니다.</p><p>지역 상품 안내는 공식 외부 판매처 링크만 제공하며 W.A.V.E는 상품을 판매·중개하거나 결제·주문·배송·기부에 관여하지 않습니다.</p><p>WAVE는 동행자를 모집·연결·배정하지 않으며 동행 서비스의 자격, 안전 또는 이용 가능성을 보증하지 않습니다. 공공 제도의 신청과 이용은 각 주관 기관이 정합니다.</p></section>

      <section id="terms-account"><p className="policy-section-kicker">제2조 · 계정과 접근</p><h2>핵심 기능은 비회원, 참여 기능은 회원</h2><div className="policy-detail-grid"><section><h3>비회원 이용</h3><p>여행 설계, 지도, 일정·여행집, 공유 여행 열람과 공개 후기 읽기는 가입 없이 이용할 수 있습니다.</p></section><section><h3>회원 이용</h3><p>커뮤니티 글·댓글·좋아요·신고와 계정 관리는 로그인이 필요합니다. 사용자는 자신이 이용할 수 있는 이메일과 안전한 비밀번호를 사용하고 인증 링크와 세션을 타인에게 넘기지 않아야 합니다.</p></section><section><h3>계정 복구</h3><p>등록 이메일로 비밀번호를 재설정하고 로그인 뒤 비밀번호를 변경할 수 있습니다. 계정 공유나 타인 사칭으로 생긴 문제는 확인을 위해 이용이 일시 제한될 수 있습니다.</p></section><section><h3>탈퇴</h3><p>계정 관리에서 탈퇴를 요청할 수 있습니다. 자동 탈퇴가 인증 제공처 상태로 완료되지 않으면 운영 문의를 통해 계정과 연결 데이터의 수동 삭제를 요청할 수 있습니다.</p></section></div></section>

      <section id="terms-content"><p className="policy-section-kicker">제3조 · 사용자 콘텐츠</p><h2>권리는 작성자에게, 공개에 필요한 범위만 서비스에</h2><p>게시글·댓글·장소 제보의 권리는 작성자 또는 정당한 권리자에게 있습니다. 사용자는 자신이 작성할 권리가 있는 내용만 올려야 하며, 공개한 콘텐츠에 대해 WAVE가 서비스 안에서 저장·표시·검색·형식 조정하고 운영정책에 따라 검토할 수 있는 비독점적 권한을 부여합니다. 이 권한은 서비스 제공과 운영에 필요한 범위로 한정되며 콘텐츠 삭제 시 종료됩니다. 다만 보안 기록, 분쟁 처리 또는 법령상 보존이 필요한 부분은 예외가 될 수 있습니다.</p><p>여행 이야기와 질문 및 댓글은 공개 글이며 연락처나 개인정보를 적지 않아야 합니다. WAVE는 여행자가 작성한 내용의 정확성이나 안전성을 보증하지 않습니다. 공식 관광정보와 사용자 경험은 서로 다른 출처로 표시됩니다. 게시물이 공개되더라도 WAVE가 그 정확성이나 특정 이용자의 접근 가능성을 공식 확인했다는 뜻은 아닙니다.</p><p>현장에서 확인한 정보도 여행자가 남긴 경험이며 WAVE가 사실을 확인한 정보가 아닙니다. 공식 시설 상태나 편의근거 점수에 반영되지 않습니다.</p></section>
      ...
      <section id="terms-liability"><p className="policy-section-kicker">제6조 · 책임, 준거와 문의</p><h2>확인 가능한 범위에서 책임 있게 운영합니다.</h2><p>WAVE는 합리적인 보안·검증·복구 절차로 서비스를 운영하지만 천재지변, 통신망, 외부 제공처, 사용자의 기기·권한 설정 등 통제하기 어려운 사유로 중단되거나 정보가 달라질 수 있습니다. 법령상 배제할 수 없는 책임은 이 약관으로 제한하지 않습니다. 긴급한 구조·의료·교통 안전 판단에는 관계 기관과 현장 안내를 우선해야 합니다.</p><p>약관과 서비스 이용에 관한 문의는 <a href="https://github.com/jeongiryang/wave-barrier-free-gyeongnam/issues" target="_blank" rel="noreferrer">WAVE 운영 문의</a>로 접수합니다. 공개 문의에는 개인정보나 인증정보를 작성하지 마세요. 대한민국 법령을 기준으로 운영하며 분쟁은 당사자 사이의 협의를 우선합니다.</p><p className="policy-note">이 약관은 2026년 9월 20일부터 시행합니다. 중요한 변경은 버전과 시행일을 갱신해 공개합니다.</p></section>
    </article>
