...
export default function PlannerPage() {
  ...
  const [itineraryMapView, setItineraryMapView] = useState(true);
  const {
  ...
  async function searchPlaces(revealResults = true, requestedTheme = theme) {
    ...
    const effectiveTheme = requestedTheme || "nature,history,leisure,food";
    if (!region) planController.setRegion(requestedRegion);
    if (!requestedTheme) planController.setTheme(effectiveTheme);
  ...
  return (
    <main className="planner-page journey-editorial planner-reference" lang={locale}>
      ...
      <section className="planner-journey-workspace" id="planner" aria-label="여행 만들기">
        <div className="journey-control-layout" data-plan-stage={stageView.view === 'guided' ? journey.activeStepId : ''} data-compact-conditions={stageView.view === "guided" && journey.activeStepId === "conditions"}>
