...
type SharedTrip = {
  ...
  selections: { region?: string; theme?: string; profiles?: string[]; travelStart?: string; travelEnd?: string; scheduleAssignments?: Record<string, string> };
  origin?: { label?: string };
...
export default function SharedTripPage() {
  ...
  const scheduledDates = trip ? [...new Set(Object.values(trip.selections.scheduleAssignments || {}).filter(Boolean))].sort() : [];
  ...
  return (
    <main className="shared-trip-page">
      ...
      {trip && <>
        ...
        {trip.restoration?.missing ? <p className="shared-restoration-notice" role="status">저장한 장소 {trip.restoration.missing}곳은 공식 관광정보에서 현재 확인되지 않아 {trip.restoration.mode === "condition-fallback" ? "같은 여행 조건의 최신 후보를 대신 표시합니다." : "일정에서 제외했습니다."}</p> : null}
        <section className="shared-content"><div className="shared-places">{trip.plan.places.map((place, index) => <article key={place.id}><div style={place.image ? { backgroundImage: `linear-gradient(180deg, transparent, rgba(4,25,44,.7)), url("${place.image}")` } : undefined}><span>{String(index + 1).padStart(2, "0")}</span><b>{place.city}</b></div><section><h2>{place.name}</h2><p>{place.summary}</p><div>{place.features.slice(0, 3).map((feature) => <span key={feature}>✓ {feature}</span>)}</div><strong className={place.score === null ? "pending" : ""}>{place.score === null ? "판단 보류" : `${place.score}%`}<small>{place.score === null ? "공식 편의근거 부족" : "선택 편의조건 일치"}</small></strong></section></article>)}</div><aside><p className="section-kicker">ITINERARY</p><h2>{scheduledDates.length ? "날짜별 저장 일정" : "여행 순서"}</h2>{scheduledDates.length ? <div className="shared-schedule">{scheduledDates.map((date, dayIndex) => <section key={date}><header><small>DAY {dayIndex + 1}</small><strong>{new Intl.DateTimeFormat("ko-KR", { month: "long", day: "numeric", weekday: "short" }).format(new Date(`${date}T12:00:00`))}</strong></header><ol>{trip.plan.places.filter((place) => trip.selections.scheduleAssignments?.[place.id] === date).map((place, index) => <li key={place.id}><span>{index + 1}</span><div><small>저장한 여행지</small><h3>{place.name}</h3><p>{place.city}</p></div></li>)}</ol></section>)}</div> : <ol>{trip.plan.stops.map((stop, index) => <li key={`${stop.title}-${index}`}><span>{index + 1}</span><div><small>{stop.source}</small><h3>{stop.title}</h3><p>{stop.note}</p></div></li>)}</ol>}<a href={`/planner?region=${encodeURIComponent(trip.selections.region || "창원")}`}>이 조건으로 다시 설계하기 →</a></aside></section>
      </>}
