    ...
    <div className="travel-book-card-body">
      <header>
        ...
        <div className="travel-book-status" role="group" aria-label={`${book.title} 여행 상태`}>
          <button type="button" aria-pressed={book.status === "planned"} onClick={() => onUpdate(book.id, { status: "planned" })}>갈 여행</button>
          <button type="button" aria-pressed={book.status === "visited"} onClick={() => onUpdate(book.id, { status: "visited" })}>다녀온 여행</button>
        </div>
      </header>
      ...
      <label className="travel-book-note">
        ...
        <textarea maxLength={1200} defaultValue={book.note} placeholder={book.status === "visited" ? "현장에서 편했던 동선이나 다음 여행에 참고할 점을 남겨보세요." : "운영시간, 준비물처럼 다시 확인할 내용을 남겨보세요."} onBlur={(event) => onUpdate(book.id, { note: event.currentTarget.value })} />
        <small>메모도 이 기기에만 저장됩니다. 입력 후 다른 곳을 누르면 자동 저장돼요.</small>
      </label>
      ...
      <div className="travel-book-delete">
        {deleteReady ? <><span>이 기기에서 이 여행을 지울까요?</span><button type="button" onClick={() => onRemove(book.id)}>삭제 확인</button><button type="button" onClick={() => setDeleteReady(false)}>취소</button></> : <button type="button" onClick={() => setDeleteReady(true)}>여행집에서 삭제</button>}
      </div>
    </div>
  </article>;
}
...
export default function TravelBookPage() {
  ...
  return <main className="travel-book-page">
    <a className="skip-link" href="#travel-book-main">여행집 본문으로 바로가기</a>
    ...
    {!hydrated ? <section className="travel-book-empty" aria-live="polite"><p>여행집을 펼치는 중이에요.</p></section> : books.length ? <section className="travel-book-list" aria-label="보관한 여행">{books.map((book) => <TravelBookCard key={book.id} book={book} onUpdate={update} onRemove={remove} onRestore={restore} />)}</section> : <section className="travel-book-empty">
    ...
    <footer className="travel-book-footer"><Link href="/planner">새 여행 설계</Link><Link href="/community">여행자 후기 읽기</Link><GithubFooterLink /></footer>
  </main>;
}
