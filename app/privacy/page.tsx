    ...
    <article className="policy-article">
      ...
      <section id="privacy-retention"><p className="policy-section-kicker">02 · RETENTION</p><h2>보관 기간과 파기</h2>
        <div className="policy-detail-grid">
          ...
          <section><h3>나루의 대화</h3><p>따로 저장하지 않은 대화는 현재 페이지의 메모리에 최근 30개 메시지만 유지하며, 새로고침하거나 페이지를 떠나면 사라집니다. 요청 처리를 위해 웹 서버와 WAVE AI 서버를 거치지만 질문·답변·첨부 사진을 데이터베이스나 애플리케이션 로그에 기록하거나 모델 학습에 사용하지 않습니다. 사진은 브라우저 메모리에 보내기 전까지 유지하고 성공 후 제거합니다. 실패·중단 시 재전송을 위해 남기며 삭제할 수 있습니다. 사진 요청은 사진 1장과 해당 질문만 전달하고 이전 대화는 전달하지 않습니다. 대화로 변경한 일정은 기존 기기 저장·계정 저장 선택에 따릅니다. 나루에서 저장을 누르면 여행 이름과 최근 대화 최대 30개, 작성 중인 질문을 이 기기에 최대 10개 여행으로 보관합니다. 사진 원본과 실행 가능한 변경안은 저장하지 않으며, 저장한 내용의 기록 삭제 또는 브라우저 사이트 데이터 삭제로 지울 수 있습니다. 다른 기기나 계정으로 대화 기록을 자동 동기화하지 않습니다.</p></section>
