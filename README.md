...
- 현재 위치는 사용자가 버튼을 눌러 별도 안내를 확인한 때에만 브라우저 권한을
  요청합니다. 현재 위치 좌표는 브라우저 메모리의 지도 표시용으로만 사용하며 W.A.V.E
  API·Kakao·ODsay·데이터베이스·운영 로그·분석 도구로 전송하거나 저장하지 않습니다.
- 경로 비교에는 사용자가 직접 선택한 공개 출발지·관광지를 사용합니다. 주변 검색은
  현재 GPS와 분리된 지도 중심을 사용자가 별도로 검색할 때만 외부 제공자에 전달할 수
  있으며, 이 경계는 코드·네트워크 회귀·Preview에서 확인되기 전까지 Release Gate입니다.
- 위치정보지원센터의 2026-09-07 사전 상담 결과와 구현 완료 조건은
  [위치정보 처리 및 사전 상담 기록](docs/location-information-compliance.md)에 정리했습니다.
