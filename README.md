...
- 현재 위치는 사용자가 버튼을 누를 때 브라우저 권한을 요청하며 기기에서 별도로 표시합니다.
  GPS를 출발지로 선택한 동안 서버 경로 요청을 차단합니다. 공개 거점·관광지·사용자가 고른
  지도 지점의 경로는 W.A.V.E API를 거쳐 Kakao·ODsay에 요청합니다. 지도 제공처의 타일·SDK·
  주변 검색 처리가 있으므로 모든 위치 처리가 기기 안에서 끝난다고 설명하지 않습니다.
