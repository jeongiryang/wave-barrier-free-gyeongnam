...
- 현장 접근성 AI 스캐너는 사용자가 올린 사진만 분석하며, 결과를 공식 무장애 정보와 분리해 표시하고 공식 값을 덮어쓰지 않습니다. 사진은 브라우저에서 다시 그려 촬영 위치(EXIF)를 지운 뒤 전송하고 원본을 저장하지 않습니다.
...
| `GEMINI_API_KEY` | 선택 | 현장 접근성 AI 스캐너; 비우면 스캐너만 비활성화 |
| `GEMINI_MODEL` | 선택 | 스캐너 모델명; 비우면 `gemini-2.0-flash` |
