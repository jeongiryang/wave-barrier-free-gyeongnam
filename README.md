...
> 현재 화면 기준은 [Simple is Best 개편](docs/design/simple-wave-20260913.md)이며, 구현·로컬 검증과 실제 배포를 구분해 기록합니다. 최신 운영 배포는 [CD 실행 결과](https://github.com/jeongiryang/wave-barrier-free-gyeongnam/actions/workflows/cd.yml)에서 확인할 수 있습니다. 공모전 최종 제출 캡처와 사람 확인 항목은 [출시 준비 기록](docs/launch-readiness-status.md)을 따릅니다.
...
W.A.V.E는 확인된 편의시설과 추가 확인이 필요한 항목을 구분하고, 여행자가 날짜·순서·이동을
직접 정한 뒤 출발 전 정보를 다시 확인하도록 돕습니다. 기본 화면은 **여행지 찾기**와 **내 일정**입니다.
상세 정보·편의 조건·일정 수정·출발 정보는 필요할 때 열고, PC에서는 시간표와 지도를 나란히 봅니다.
관광 탐색, 일정 편집, 지도, 출발 전 확인, 이 기기 저장과 공유는 로그인 없이 이용할 수 있습니다.
공개 여행 후기도 로그인 없이 읽을 수 있으며 커뮤니티 글·댓글·좋아요·신고에는 계정이 필요합니다.
등록 이메일로 비밀번호를 복구하고
로그인 뒤 계정 관리에서 비밀번호를 변경할 수 있습니다. 자동 탈퇴는 Neon 제공처 설정 문제로 최종 검증이 필요합니다.

1. [여행지 찾기에서 지역을 고르고 원하는 장소를 담습니다.](https://wave-barrier-free-gyeongnam.vercel.app/planner#conditions) 날짜는 나중에 정해도 됩니다. 필요한 시설·활동만 선택하며 확인·미확인·부재를 구분합니다.
2. [내 일정에서 날짜·시각·순서와 이동을 정하고 저장하거나 공유합니다.](https://wave-barrier-free-gyeongnam.vercel.app/planner#itinerary) 출발 전 확인을 펼치면 날씨·관광 집중률·교통·장소 근거를 다시 볼 수 있습니다.

나루 캐릭터를 눌러 같은 여행을 대화로 편집할 수도 있습니다. 구체적인 변경은 되돌릴 수 있고, 새로운 장소나 전체 일정 제안은 검토한 뒤 적용합니다.

| 여행자의 질문 | W.A.V.E가 보여 주는 것 | 판단 기준 |
| --- | --- | --- |
| 내가 이용하기 편한가? | 선택한 조건과 장소별 공식 편의정보 | 확인됨·불일치·미확인을 구분하고 긍정 근거가 없는 장소는 일반 추천에서 제외 |
| 어떻게 이동하는가? | 자동차·대중교통 경로와 직선 연결 미리보기 | 실제 제공기관 경로와 추정값을 분리 |
| 어떤 순서가 나은가? | 날짜·시작 시각·장소 순서를 편집하는 일정 | 버튼과 키보드로도 같은 작업 제공 |
| 출발 전에 무엇을 더 확인해야 하는가? | 날씨·관광 집중률·교통·장소 정보의 조회 상태 | 실패·값 없음·예측을 확인 완료로 표시하지 않음 |
...
| 공유 여행 | `/trip/{id}` | 장소 식별자와 날짜별 일정을 30일간 공유. 새 링크는 같은 주소에서 갱신·종료하며 기존 링크는 기존 스냅샷·만료일을 유지 |
...
| 데이터 성격 | 화면 표시와 처리 |
| --- | --- |
| 공식 관광·무장애 정보 | 제공기관 원문과 확인됨·불일치·미확인을 구분. 확인됨은 공식 데이터에 기재됐다는 뜻이며 현장 접근성 인증이 아님 |
| W.A.V.E 추천 계산 | 중복 제거한 전체 선택 편의항목 중 긍정 근거의 비율로 정렬. 알려진 항목이 없으면 점수는 미확인으로 남김 |
| 조회 시각 | W.A.V.E가 정보를 조회한 시각이며 제공처의 시설 갱신일과 다름 |
| 교통 제공기관 경로 | 제공기관 응답이 실제 출발지→목적지 경로를 포함할 때만 실제 시간·거리·요금으로 표시 |
| 직선거리·기본 체류시간·일정 영향 | W.A.V.E의 계산 또는 규칙에 따른 결과임을 표시하며 제공기관의 실제 시간표·공식 권고와 구분 |
| 관광 집중률 | 한국관광공사 예측 데이터로 기준일과 함께 표시하며 실시간 방문자 수로 표현하지 않음 |
| 여행자 현장 제보 | 작성자 한 명의 경험으로 표시하고 공식 정보에 근거한 W.A.V.E 추천 계산에 합산하지 않음 |
...
한 구간의 경로 응답은 일정 전체의 이동 확인을 뜻하지 않습니다. 저상버스·경사·승강기 운휴 등
전 구간의 접근성 정보가 모두 제공되는 것은 아니며, 사진·인기·후기도 편의 근거의 계산을 바꾸지 않습니다.
ODsay 제공처 제한은 별도의 외부 운영 상태로 관리하며, 해제 확인 전에는 대중교통 전체 정상화를
주장하거나 성공 화면을 얻기 위한 반복 조회를 하지 않습니다.
...
- 공개 서비스는 한국어·밝은 화면입니다. 영어·어두운 화면은 개발 검수에서만 활성화하며 전체 다국어 지원으로 안내하지 않습니다.
...
- 로컬 변경 후보: 현재 GPS는 사용자 요청 뒤 공개 출발지까지의 직선거리를 기기에서만 계산합니다. 지도 SDK·타일·주변 검색·경로·나루·저장·공유에는 넣지 않습니다. 외부 지도는 공개 장소 또는 직접 고른 지점만 사용합니다. 사진 GPS도 묶기에만 사용하며 조회 지역은 직접 선택합니다.
- 새 공유 링크에는 지역·일정·공식 장소 식별자를 저장하며 개인 편의 조건·메모·GPS 출발지는 포함하지 않습니다.
  소유자가 같은 링크를 갱신하거나 종료할 수 있습니다. 30일 뒤 조회에서 제외하고 매일 예약 정리하며 기존 링크의 스냅샷과 만료일은 유지합니다.
