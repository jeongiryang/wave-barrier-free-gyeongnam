...
| 현장 대응 | 현재 날씨·7일 예보, 관광 집중률, 축제·행사·숙박 정보 | 운영 중 |
| 접근성 | 키보드 탐색, 반응형 화면, 다크 모드, 동작 감소, 파동 효과 설정 | 운영 중 |
| 다국어 | 한국어와 영어·일본어·중국어·프랑스어·스페인어 소개·주요 조작 | 비한국어 Beta |
| 계정·저장 | Neon Auth와 Postgres 연결 코드 | 공개 로그인 진입점은 현재 비활성 |

> 계정 인프라는 유지하지만 공개 로그인 UI는 Google Safe Browsing 오탐 검토가
> 끝날 때까지 노출하지 않습니다. 관광 탐색과 여행 설계는 로그인 없이 사용할 수 있습니다.
...
flowchart TB
  ...
  B --> C[장소 적합도와 추천 근거 구성]
  C --> D[교통·날씨·혼잡·행사 교차 확인]
  D --> E[일정·지도·공유 가능한 여행안]
```
...
- Neon에는 공유 여행과 접근성 제보 등 서비스가 직접 만든 데이터만 저장합니다.
- 공유 여행 데이터는 운영 정책에 따라 30일 뒤 만료됩니다.
- 사용자의 GPS 좌표는 브라우저 안에서만 사용하고 서버·DB에 저장하지 않습니다.
- 공식 인증, 제공기관 응답, W.A.V.E 적합도 계산을 화면에서 구분합니다.
...
기본 개발 주소는 `http://localhost:5173`입니다. 포트가 이미 사용 중이면
...
worker/index.ts         관광·교통·날씨 데이터 오케스트레이션
public/                 정적 SVG와 파비콘
...
3. PR마다 담당자·리뷰어·라벨과 `docs/ai-logs/PR-xxx.md`를 기록합니다.
4. 최신 `main` 대비 diff, 충돌, 리뷰 의견과 새 CI 결과를 확인합니다.
5. 실패·대기 중 검사는 우회하지 않고 성공한 경우에만 병합합니다.
