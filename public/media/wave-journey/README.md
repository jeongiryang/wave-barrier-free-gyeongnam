...
실제 공개 Production 화면을 정수 영역으로 자른 무손실 WebP입니다. UI 내용·시설 상태·날짜·워터마크를 합성하거나 변경하지 않았습니다. **원본 촬영 당시 배포와 이 파일을 사용하는 소개 UI의 배포 상태를 구분합니다.** 현재 새 날짜·지도3상태 소개는 커밋된 미배포 구현이며 Production 검증은 아직 없습니다.
