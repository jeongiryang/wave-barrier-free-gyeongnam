...
2026-09-20 현장 음성 글자 표시: #528의 직원 답변 화면에서만 선택적으로 `SpeechRecognition`/`webkitSpeechRecognition`을 시작한다. 시작 전 고정 고지와 매번 확인을 요구하며, 브라우저에 따라 음성이 제조사 서버에서 처리될 수 있다. WAVE 서버·나루·DB·로그·URL·`localStorage`·`sessionStorage`에는 음성이나 인식 결과를 보내거나 저장하지 않는다. 결과는 최대 2,000자의 React 메모리 상태에만 두고 닫기·언마운트 시 폐기한다. 화면 숨김·닫기·언마운트 시 인식을 중지하고 이벤트를 해제한다. 위치 API, `MediaRecorder`, 서버 handler, 분석 이벤트, 환경 변수, 추가 네트워크 요청은 사용하지 않는다. 미지원 브라우저에서는 진입점을 숨기고 기존 의사소통판과 직접 입력을 유지한다.
...
2026-09-20 인구감소지역 안내: 행정안전부 「인구감소지역 지정 고시」(행정안전부고시 제2021-66호, 2021-10-19)와 현재 지정 현황을 확인해 경남 11개 시군 이름만 정적으로 표시한다. 2024년 변경 고시는 군위군의 시도 편입만 반영해 경남 목록에는 영향이 없다. 인구 수·비율·위험 지수를 포함하지 않고, 목록 순서 필터는 브라우저 메모리에서만 동작한다. 서버 action·API·DB·환경 변수·네트워크 요청·위치 권한을 추가하지 않는다. [고시 원문](https://www.mois.go.kr/frt/bbs/type001/commonSelectBoardArticle.do?bbsId=BBSMSTR_000000000016&nttId=90651), [현재 지정 현황](https://www.mois.go.kr/frt/sub/a06/b06/populationDecline/screen.do)을 2026-09-20 확인했다.
2026-09-20 공공 동행 지원 안내: 보건복지부 장애인활동지원 공식 안내와 경상남도 복지포털을 확인했으나, 여행 목적 이용 가능 여부와 개별 신청 조건을 공식 문서만으로 확정하지 못했다. 저장소 책임자의 제도·링크·신청 문구 확인 전까지 `companionSupportPrograms`는 빈 배열이고 출발 전 카드는 렌더링하지 않는다. 새 API·서버 action·DB·환경 변수·분석 이벤트·개인정보 입력·위치 권한·네트워크 요청을 추가하지 않는다. [장애인활동지원 공식 안내](https://www.ableservice.or.kr/regm/info/getOHGC0006M0.do), [경상남도 장애인복지 서비스](https://www.gn.go.kr/welfare/contents.do?key=6510)를 2026-09-20 확인했다.
2026-09-20 기능 후보45: 공공데이터포털에서 `가로 휴지통`·`쓰레기통`·`가로쓰레기통 설치 현황`·`재활용 분리수거함`과 경남 시군을 조합해 읽기 전용으로 조사했다. 확인 가능한 위치 데이터는 [광주 광산구 쓰레기통 현황](https://www.data.go.kr/data/15108027/fileData.do)(91행, 주소·세부위치·위경도·종류)과 [대구 달서구 가로휴지통 설치 현황](https://www.data.go.kr/data/15127543/fileData.do)(9행, 주소·세부위치·종류)이었으며 둘 다 경남 표본은 0건이다. 검색에서 경남 위경도 표본을 가진 단일 공식 데이터셋은 확인하지 못했다. 따라서 실제 위치를 저장소에 넣거나 다른 지역 레코드를 경남으로 표시하지 않는다.

사용자의 구현 지시에 따라 현재 승인 상태를 차단 사유로 삼지 않고 `/api/wave?action=trash-bin&contentId=…` 계약을 완성했다. 운영자는 공공데이터포털에서 승인된 **경남 지자체 JSON API**의 HTTPS 주소를 `WASTE_BIN_API_URL`, 정확한 데이터셋 이름을 `WASTE_BIN_API_SOURCE`에 연결하고 기존 `TOUR_API_SERVICE_KEY_ENCODED`를 재사용한다. 서버는 KTO `detailCommon2`로 경남 공개 관광지 ID·좌표를 다시 확인하고, 제공처 원본 중 실제 위경도·기준일·경남 근거가 모두 있는 레코드만 거리순 최대15개로 정규화한다. 주소만 있는 데이터의 지오코딩, 사용자 좌표, 하드코딩 위치, 자동 재시도는 없다. 성공 응답만 인스턴스 메모리에24시간/최대100개, 동시50개까지 보관한다. 이 작업 환경에는 승인된 경남 endpoint가 없어 실제 제공처 호출 성공과 경남 표본 건수는 **미확인**이며, 합성 fixture 성공과 구분한다.
