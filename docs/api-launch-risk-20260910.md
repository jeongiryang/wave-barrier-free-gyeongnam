...
2026-09-12 후보24: 음성 명령은 브라우저 제공 음성 인식을 선택적으로 사용하며 WAVE에 새 과금 API 계약을 추가하지 않는다. [SpeechRecognition](https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition)은 브라우저별 지원이 다르고 일부는 서버 인식과 인터넷 연결이 필요하다. 마이크·서비스 권한, 네트워크, 제공처 가용성에 따라 실패할 수 있어 문자 입력을 항상 유지한다. 자동 재시도·음성 저장 없음. 합성 인식 이벤트로 조작 경계를 확인하며 실제 사용자의 마이크 음질·인식 정확도를 보증하지 않는다.

2026-09-12 후보22: A/B 분리·합류 시간 계산과 선택한 오프라인 요약은 새 제공처 호출이나 유료 서비스 없이 동작한다. 이미 확인한 동일 경로 외 이동은 추정이고 복귀 이동이 미확인이면 그 한계를 유지한다. 저장 공간·브라우저 설정·다른 화면의 동시 변경은 저장 실패로 안내하며 원래 일정은 변경하지 않는다.
