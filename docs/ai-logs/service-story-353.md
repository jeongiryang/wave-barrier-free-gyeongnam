...
### CI859 시간 한도와 기존 두 runner의 분배 보완

- [CI859](https://github.com/jeongiryang/wave-barrier-free-gyeongnam/actions/runs/34246685238)는 root가 취소하지 않았다. quality/boundary/desktop 성공 후 mobile job이 **25분 실행 한도**로 플랫폼에서 취소됐고, 기존 `validate`는 정상적으로 FAIL했다. API annotation이 `The job has exceeded the maximum execution time of 25m0s`를 확인했다. 실패를 성공으로 소급 덮어쓰지 않는다.
- 기본 전체 `--shard=n/2`의 실제 분포는 desktop411 / mobile411이었다. desktop은 **411 PASS, 16.6분**(job17분34초). mobile은 마지막 **411/411** 테스트 시작 **1.75초** 뒤 취소되어 전체 PASS가 아니다. 로그에는 assertion 실패·재시도 흔적이 없지만 미완료 검사를 통과로 계산하지 않는다. 지연은 여러 기존 mobile 파일에 분포했으며, 단일 새 테스트 실패로 오인하지 않는다.
- #373/Fast Gate·새 runner·sandbox 연구를 가져오지 않고, 현재 후보 검증을 막는 분배만 조정했다. 기존 **2 runner / 각2 workers / job25분 / test45초 / expect8초 / 기존retry1·failOnFlaky**를 모두 유지한다. 각 runner가 Playwright의 기본 `--project=desktop-chromium --shard=n/2`와 `--project=mobile-chromium --shard=n/2`를 차례로 실행한다. 먼저 실패해도 취소되지 않았다면 mobile 검사를 수행하며 어느 쪽 실패도 job/validate 실패로 유지한다.
- 각 프로젝트의 HTML 보고서와 test-results를 별도 하위 경로에 보존하므로 두 번째 실행이 첫 번째의 증거를 지우지 않는다. 변경 전후 YAML 구조·보안 경계·나머지 명령의 정확한 동등성 계약을 유지하고 두 실행 명령/조건/보고서 경로를 추가 검증했다. 과거 sandbox archive는 수정하지 않았다.
- 새 후보의 같은 CLI `--list --reporter=json` 실제 수집에서 **unsharded828 = desktop207×2 + mobile207×2**, 누락0/중복0/추가0을 확인했다(`story353-shard-coverage.json`). 처음 보조 집계의 파일/행/짧은 제목 키는 서로 다른 describe를 구분하지 못해 중복 오류를 냈으며 공식 Playwright spec ID로 정확히 대조했다. 테스트나 skip을 제거한 것이 아니다. 변경된 CI 계약4 PASS. 실제 hosted 시간·전체 결과는 다음 exact HEAD CI에서 별도로 확인한다.
- 두 관련 파일을 같은 순차 CLI/project/shard 방식으로 확인해 **desktop8 PASS(14.6s) → mobile8 PASS(16.7s)**, 두 HTML 보고서가 모두 남음을 확인했다(`story353-two-phase-*`). 기존 로컬 서버를 사용하는 제한된 CLI/보고서 검증이며 hosted 서버 lifecycle 성공으로 과장하지 않는다. 마지막 lint/typecheck 성공, unit696 PASS/0 skip. 독립 QA도 828개 ID 집합·실패 전파·분리 보고서·불변 한도를 별도로 확인했다. 로그 기반 테스트 구간 예상치는 runner별20분05초/20분37초이며 실제 완료시간은 이후 결과로 판정한다.
