...
## Hosted CI #435 correction

CI 34429308090 on 90744097c1d8c4826cae15a7f72a80d1726cacec failed four English light/dark cases across both device projects at the first 320px date-select text-fit assertion. Its trace and screenshot were inspected; the screenshot also showed a compressed native time input. Other quality, boundary and browser groups passed. This was treated as a product layout failure, not bypassed or hidden with retries.

Mobile date editing now spans the full stop width. The native time input has a usable flex basis and wraps with its button when necessary. The unchanged text-fit assertion reports the label and available/needed widths when it fails. Fresh local lint/typecheck, all 762 unit/contract tests, Vercel build and performance budgets passed (Planner 269.34/270 KiB; CSS 69.90/70 KiB), as did all 60 related browser cases in 2.4 minutes. Fresh 320px English, 960px Korean light and 1440px English dark renders were inspected. Local native time formatting follows the browser/OS locale. New exact-head hosted CI and this PR's own main/CD/Production verification remain required. Predecessor #431 has now completed its deployment and eight canonical read-only browser checks; the separate ODsay #372 quota preflight remains held.
