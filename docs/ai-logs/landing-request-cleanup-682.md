...
- 추가 검증: 전체 1,623개 단위·계약 테스트, 관련 데스크톱 22개, 모바일 13개, 테마 대비 3개, 타입 검사, Vercel 빌드와 성능 예산을 통과했다. 당시 성능은 CSS gzip 84.86/85 KiB, 랜딩 초기 JS gzip 144.20/155 KiB, 플래너 초기 JS gzip 264.24/270 KiB다.
- 1440px 실제 렌더 캡처에서 지역 카드 크기, 세 칸 높이, 아이콘 중앙 정렬, 나루 즉시 노출과 마지막 사진 배경을 확인했다.
- 최신 main 통합 뒤 브라우저 재실행은 Windows 프로세스 생성 오류(`spawn UNKNOWN`)로 시작되지 않았다. 제품 assertion 실패가 아니며, 통합 전 동일 모바일 13개는 통과했다.

## 2026-09-22 경계 출처 표시 후속 조정

- 사용자 캡처에 표시된 `통계청 SGIS 2020 · 경계 단순화 / StatGarten` 문구를 서비스 소개 지도에서 숨겼다.
- 일정 설계기의 지도 표기와 저장소 라이선스·출처 문서는 유지해 자산 출처 기록을 보존했다.
- 타입 검사와 변경 파일 ESLint를 통과했다. Playwright 재실행은 Windows `spawn UNKNOWN`으로 브라우저 프로세스가 시작되지 않았다.
