...
## 2026-09-22 mobile evidence readability follow-up

- Root correctly identified the 390px crop's launcher overlap as directly affecting this PR's source/date readability requirement. The excluded assistant implementation was not a reason to leave map text occluded; the preceding observation is resolved by this follow-up.
- Added a real text-range regression with the legend near the fixed launcher's vertical position. Before the CSS change, the desktop valid-date case failed at 390px: both `한국관광공사 예측` and `실시간 인원·대기시간 아님` intersected the launcher rectangle. Evidence/trace retained externally in `pr673-overlap-before`.
- One mobile CSS rule moves `.map-crowd-evidence` to columns 2 through the last column at max-width 620px, aligning it with the existing title instead of placing source text beneath the left icon. It keeps the legend's bottom position and width, wraps naturally, and avoids moving the entire overlay farther into the map. No launcher, assistant, model, or data code changed.
- The browser spec checks actual source/date/disclaimer text rectangles against the unopened fixed launcher, preserves overflow checks, and requires the legend to occupy no more than 40% of map height. Widths: desktop 1440 -> 960 -> 390 -> 320 -> 1440; mobile 390 -> 320.
- Same command and executable override as above, with output in `pr673-overlap-after`: **12 passed (40.2s)**, workers 2, retries 0, repeat-each 2. Baseline negative case: 1 expected failure (390px). Final 320/390/960 screenshots visually inspected: source, date, and disclaimer readable; decorative icon may share the floating control's area, while evidence text does not.
- TypeScript, changed-spec ESLint, `git diff --check`, `npm run build:vercel`, and `npm run check:performance`: passed. CSS gzip **84.75/85 KiB**, landing initial JS gzip **144.30/155**, planner **264.47/270**, largest chunk **67.57/110**. Budgets and CI gates unchanged. Full build log: external `pr673-overlap-build.log`.
- Ready for independent QA and root's latest-main coordination; no remote push or merge performed in this follow-up.

## 2026-09-22 existing service-story CI readiness correction

- Root authorized one test-only follow-up for the same run's existing mobile shard 7 flaky story-tab case. Independent trace analysis by community_demo found the tab click completed at 196024ms, before LandingChapters' effect-driven lazy region/map request at 196075ms. The shell's `__VINEXT_HYDRATED_AT` records scheduling hydrateRoot, not chapter commit; no post-commit reset/error was observed, and retry passed.
- Reused the existing `landing-design-b-flow.spec.ts` UI readiness contract only in the affected service-story case: scroll `#story` into view, wait for its effect-driven `여행 지역 선택` group, click the facilities tab, and verify `aria-pressed=true` before the unchanged explanatory-copy checks. No product, timeout, retry or CI gate changed; 998e140's product CSS is identical.
- `E2E_BASE_URL=http://127.0.0.1:4191 node node_modules/@playwright/test/cli.js test e2e/service-story.spec.ts --grep 'ko: the service explains' --workers=2 --retries=0 --repeat-each=3`: **6 passed (19.6s)**, desktop/mobile repeated three times. This Korean case navigates to `/planner` and does not open Naru or call a model. The English case that opens its dialog was not run. An initial grep with a leading `^` selected no tests; corrected the selector before validation.
- Changed-file ESLint and diff check passed. Browser override remains Chromium1223 above. Product/build contents remain the already validated 998e140 tree; no duplicate build was needed for this test/log-only change.
- Root's follow-up review caught that the affected case loops over both locales: the lazy region group is `Choose a region` in English. The final selector now uses the actual locale-specific accessible name; all existing assertions remain. A temporary bounded English probe reproduced only chapter readiness -> facilities-tab pressed state -> explanatory text, without executing any assistant action. Desktop/mobile, workers 2, retries 0, repeat-each 2: **4 passed (10.3s)**. Probe preserved externally as `service-story-en-ready-probe.spec.ts` and removed from the worktree; changed-spec lint/diff check passed again. The full English assistant-opening flow remains intentionally unexecuted locally.
