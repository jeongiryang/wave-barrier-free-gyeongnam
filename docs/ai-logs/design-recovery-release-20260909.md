...
### Hosted image-export hover repair

Run `34354186039` passed quality (including the unchanged performance budgets), sandbox boundary and seven browser shards. Desktop shard 2 failed the English light/dark image-export cases at the final 1440px resize. Both retries showed the same problem. The downloaded traces show the PNG button alternating between its normal and blue hover surfaces: the inherited `translateY(-2px)` moved its lower edge away from the stationary pointer and immediately moved it back when hover ended. This kept the target unstable. The local Windows run passed both unchanged cases, so local success alone did not establish a fix.

Keep the export buttons stationary on hover while retaining color, border and shadow feedback. No global scroll/focus behavior, test assertion, timeout, retry or shard coverage changes are needed. The unchanged 24-case image export/share suite passes on desktop and mobile locally. Hosted exact-head CI and Production verification remain required.
