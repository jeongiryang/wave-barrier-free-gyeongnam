...
The next hosted run34420260747 passed every job except mobile3, where photo-accessibility recorded two requests for content1001 and therefore failed the existing fail-on-flaky gate. Its retry passing is not a pass for that head. Two deterministic hook cases reproduce the underlying races: an early decode error is overwritten by the initialization frame, and the original-image8.5s timer starts another fallback while the first request is pending.

The photo hook now retains one keyed fallback attempt, preserves an already-started fallback through the initialization frame, and aborts/ignores obsolete requests during cleanup or input changes. The original8.5s/12s deadlines, UI assertions and CI retry/flaky policy are unchanged. Baseline8a5991a has both failing cases; both pass with the fix, including unmount abort and late-response rejection. Fresh full lint/typecheck,755 unit/contract tests, Vercel build/performance PASS (Planner269.08KiB,CSS69.90KiB). All54 related browsers PASS1.9min: photo accessibility24, integrity16, region boundary14. The same query-count assertion remains exactly one. Fresh960px Korean/light and1440px English/dark recovery captures were reviewed; the English first photo remains a normal loading placeholder in that capture. #428 stays open until exact new-head CI and the Production impact are verified.
