...
function FestivalCard({ festival, selectedProfiles, onOpen }: { festival: Festival; selectedProfiles: string[]; onOpen: (item: Festival, date: string, fresh: boolean, askNaru?: boolean) => void }) {
  ...
  return <article className="festival-card"><button type="button" className="festival-card-photo festival-card-open" onClick={() => setDetailOpen(true)} aria-label={`${festival.name} 축제 상세 보기`}>{festival.image && !failedImage ? <Image src={festival.image} alt={`${festival.name} 관광정보 사진`} fill sizes="25vw" unoptimized onError={() => setFailedImage(true)} /> : <div className="festival-photo-fallback"><NaruAvatar large /><span>사진 없이도 행사 정보를 확인할 수 있어요.</span></div>}<span className="festival-state">{festival.state === 'ongoing' ? '진행 중' : festival.state === 'upcoming' ? `D-${Math.max(0, Math.ceil((Date.parse(festival.startDate) - Date.parse(today())) / 86400000))}` : '종료'}</span></button>
    <div className="festival-card-copy"><small>{festival.city}</small><h2><button type="button" className="festival-title-open" onClick={() => setDetailOpen(true)}>{festival.name}</button></h2><p className="festival-period">{festival.startDate} – {festival.endDate}</p><p>{festival.address || '상세 행사장 주소 확인 필요'}</p>
      ...
      {visitActions}
    </details><FestivalAmenities place={festival}/>{detailOpen && <FestivalDetailDialog festival={festival} websiteUrl={sources.websiteUrl} onClose={() => setDetailOpen(false)} visitActions={visitActions} />}</article>;
}
...
export default function FestivalExplorer() {
  ...
  return <main className="festival-page wave-night"><SkipLink href="#festival-results">축제 목록으로 바로가기</SkipLink><NightScene kind="festival"><WaveHeader current="festivals" />
    ...
    <section className="festival-filters" aria-label="축제 찾기">
      ...
      <div className="night-festival-dates"><NightIcon name="calendar"/><span>날짜 범위<span className="night-date-pair"><WaveDatePicker label="언제부터" value={start} onChange={setStart} /><span>~</span><WaveDatePicker label="언제까지" value={end} min={start} onChange={setEnd} /></span></span></div>
      ...
      <button className="primary night-festival-search" type="button" onClick={() => setReload(current => current + 1)}>축제 검색하기 </button>
    </section>
    ...
    <section id="festival-results" className="festival-results" tabIndex={-1} aria-busy={pending}>{pending && <LoadingState>행사 날짜와 관광정보를 확인하고 있어요.</LoadingState>}{notice && <p className="result-notice" role="alert">{notice}</p>}{failure && <div className="result-notice error" role="alert"><p>{failure}</p><button type="button" onClick={() => setReload(current => current + 1)}>다시 조회</button></div>}
      ...
      {!pending && !failure && current && data && !shown.length && <div className="travel-book-empty"><h2>{eventState === 'ended' ? '이 기간에는 종료된 축제가 없어요.' : '이 조건의 축제는 아직 확인하지 못했어요.'}</h2><p>{eventState === 'ended' ? '종료 여부는 오늘을 기준으로 표시해요. 과거 날짜를 선택해 지난 축제를 살펴보세요.' : '편의 조건은 그대로 두고 지역이나 날짜를 넓혀보세요.'}</p><div className="travel-book-actions"><button type="button" onClick={() => { setRegion('경남 전체'); setQuery(''); setKeyword('전체'); }}>경남 전체 보기</button><button type="button" onClick={() => { if (eventState === 'ended') { setStart(offsetTripDate(today(), -30)); setEnd(offsetTripDate(today(), -1)); } else { const next = validTripDate(end) ? offsetTripDate(end, 1) : today(); setStart(next); setEnd(offsetTripDate(next, 30)); } }}>{eventState === 'ended' ? '지난 한 달 보기' : '다음 한 달 보기'}</button><Link href="/planner?assistant=naru">나루에게 다른 여행 물어보기</Link></div></div>}
    ...
    </div><NightScene kind="festival" closing><section className="night-festival-courses"><header><NightIcon name="pin"/><h2>축제와 함께 여행을 떠나보세요</h2><p>축제 주변의 여행지를 일정으로 이어보세요.</p><Link href="/planner">전체 여행 코스 보기</Link></header><div><Link className="night-course-map" href="/planner"><NightIcon name="map" size={52}/><strong>축제가 있는<br/>더 넓은 경남</strong><span>경남 전체 지도 보기</span></Link>{shown.slice(0,3).map(item => <Link className="night-course-card" key={item.id} href={`/planner?region=${encodeURIComponent(item.city)}`}><span>{item.image && <Image src={item.image} alt="" fill sizes="120px" unoptimized/>}</span><div><strong>{item.city} 축제와 주변 여행</strong><p>{item.name}</p><small>주변 여행지 둘러보기</small></div></Link>)}{!shown.length && <Link className="night-course-empty" href="/planner">여행 지역을 고르고 나에게 맞는 코스를 만들어보세요 </Link>}</div></section>
