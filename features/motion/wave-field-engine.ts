 ...
 *
 * 인트로 모드에서는 물결 위로 네 개의 형상이 차례로 떠오른다.
 * 여행 경로 → 축제 → 커뮤니티 → WAVE 워드마크. 서비스가 무엇을 위한 것인지를
...
export interface WaveFieldRendererOptions {
  tone: "deep" | "light";
  mode: "intro" | "ambient";
  wordmark: string;
  motion: "full" | "calm";
...
export function startWaveFieldRenderer(
  canvas: HTMLCanvasElement,
  { tone, mode, wordmark, motion }: WaveFieldRendererOptions,
) {
    ...
    function draw(now: number) {
      ...
      if (!reduced && inViewport && !document.hidden && (mode !== "intro" || elapsed < 9.8)) frame = window.requestAnimationFrame(draw);
    }
    ...
    const observer = new ResizeObserver(() => {
      ...
      if (mode === "intro" && start && performance.now() - start >= 9800) draw(performance.now());
    });
    ...
    function onPointerMove(event: PointerEvent) {
      ...
      if (time - lastRipple > 0.16 && ripples.length < 6) {
        lastRipple = time;
        ripples.push({ x, y, born: time });
      }
    }
    function onPointerLeave() { pointer = { x: -9999, y: -9999, active: false }; }
    function onPointerDown(event: PointerEvent) {
      const rect = canvas.getBoundingClientRect();
      ripples.push({ x: event.clientX - rect.left, y: event.clientY - rect.top, born: (performance.now() - start) / 1000 });
    }
