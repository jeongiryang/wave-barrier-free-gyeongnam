...
export function explainVisitOrder(ordered, origin, locale = "ko") {
  const withCoordinates = ordered.filter((place) => coordinates(place)).length;
  if (locale === "en") {
    if (ordered.length < 2) return "Add more places to get a suggested visit order.";
    if (!withCoordinates) return "Coordinates are unavailable, so the recommendation order is unchanged.";
    return `The order considers straight-line distances for ${withCoordinates} places${coordinates(origin) ? " from the starting point" : ""} and facility information coverage. Check actual travel times in the route view.`;
  }
