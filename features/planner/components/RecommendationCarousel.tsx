...
export default function RecommendationCarousel({ t, region, activePlaces, planController, route, tripSelection, onGenerate, onSelectPlace }: {
  ...
  return <>
    <div className="workspace-heading journey-subheading" data-reveal><div><span>추천</span><h2>{t("placesTitle", "추천 여행지")}</h2></div><div className="carousel-actions"><button type="button" onClick={() => scrollCards(-1)} aria-label="이전 여행지">←</button><button type="button" onClick={() => scrollCards(1)} aria-label="다음 여행지">→</button></div></div>
