...
export default function RecommendationCarousel({ region, activePlaces, planController: control, tripSelection: trip, onGenerate, onMore, onSelectPlace }: {
  ...
  return <section className="simple-results" aria-label={say("여행지 검색 결과", "Place search results")} aria-busy={loading}>
    ...
    {exploration.length > 0 && <Suspense fallback={<p><Spinner />{say("다른 장소를 불러오고 있어요.", "Loading other places.")}</p>}><ExplorationPlaces places={exploration} region={region} saved={trip.saved} en={en} onToggle={place => trip.toggleSaved(place.id, place)} onSelectPlace={onSelectPlace} /></Suspense>}
