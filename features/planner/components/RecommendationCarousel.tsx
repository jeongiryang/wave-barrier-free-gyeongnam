...
export default function RecommendationCarousel({ t, region, activePlaces, planController, route, tripSelection, onGenerate, onSelectPlace }: {
  ...
  return <>
    ...
    {!loading && explorationPlaces.length > 0 && <section className="exploration-places" aria-labelledby="exploration-places-title">
      <header><div><span>추가 탐색</span><h4 id="exploration-places-title">공식 편의근거를 더 확인해야 하는 장소</h4></div><p>일반 추천과 자동 일정에는 넣지 않았습니다. 상세 정보와 운영기관을 확인한 뒤 경로만 살펴볼 수 있습니다.</p></header>
      <div>{explorationPlaces.map((place) => <article key={`explore-${place.id}`}>
        ...
        <span className={place.score === 0 ? "mismatch" : "unknown"}>{place.score === 0 ? "선택 조건 일치 0%" : "공식 편의근거 미확인"}</span>
        <div><button type="button" onClick={() => onSelectPlace(place)}>근거 상세 확인</button><button type="button" onClick={() => { void loadRoutes(place); scrollToSection("navigation"); }}>경로만 확인</button></div>
      </article>)}</div>
    </section>}
  </>;
}
