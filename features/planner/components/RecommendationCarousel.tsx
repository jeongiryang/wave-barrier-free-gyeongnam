...
export default function RecommendationCarousel({ region, activePlaces, planController: control, tripSelection: trip, weather, weatherDate, onGenerate, onMore, onSelectPlace }: {
  ...
  return <section className="simple-results" aria-label={say("여행지 검색 결과", "Place search results")} aria-busy={loading}>
    <div className="simple-results-heading"><h2>{region} {say("여행지", "places")}</h2>{plan && <span>{say(`조건 충족 ${activePlaces.length}곳 · 추가 확인 ${exploration.length}곳 · 조건 불일치 ${excluded.length}곳`, `${activePlaces.length + exploration.length + excluded.length} places loaded`)}{plan.pagination?.hasMore ? say(' · 더 보기 가능', ' · more available') : ''}</span>}</div>
