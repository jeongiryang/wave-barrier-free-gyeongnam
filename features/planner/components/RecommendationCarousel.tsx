...
function PlaceChoiceCard({ place, region, index, saved, current, en, onToggle, onDetails, compare }: {
  place: Place; region: string; index: number; saved: boolean; current: boolean; en: boolean;
  onToggle: () => void; onDetails: () => void;
  compare: { selected: boolean; disabled: boolean; toggle: () => void } | null;
}) {
  const id = useId();
  ...
  return <article className="place-card" data-result-current={current} aria-labelledby={`${id}-title`}>
    ...
    <div className="place-content">
      ...
      {compare && <button type="button" className="place-compare-choice" aria-pressed={compare.selected} disabled={compare.disabled} onClick={compare.toggle} aria-label={`${place.name} ${say("편의 비교 선택", "select for facility comparison")}`}><span aria-hidden="true">{compare.selected ? "✓" : "+"}</span>{compare.selected ? say("비교에 담았어요", "Selected for comparison") : say("비교에 담기", "Select for comparison")}</button>}
...
export default function RecommendationCarousel({ region, activePlaces, planController, tripSelection, onGenerate, onSelectPlace }: {
  ...
  const { loading, planError, plan, dirty, resultCurrent } = planController;
  ...
  return <>
    ...
    {dirty && <div className="result-notice" role="status"><strong>{say("조건이 변경됐어요.", "Your preferences have changed.")}</strong><p>{say("아래는 이전 조건의 결과예요. 다시 찾기 전에는 새 일정에 추가할 수 없습니다.", "These are previous results. Search again before adding places.")}</p><button type="button" aria-busy={loading} disabled={loading} onClick={() => void onGenerate(false)}>{say("변경한 조건으로 다시 찾기", "Search with new preferences")}</button></div>}
    {planError && <div className="result-notice error" role="alert"><strong>{planFailureHeadings[planError][en ? 1 : 0]}</strong><p>{say("선택한 조건과 기존 일정은 유지됩니다. 잠시 후 다시 시도해 주세요.", "Your choices and existing itinerary are kept. Please try again shortly.")}</p><button type="button" aria-busy={loading} disabled={loading} onClick={() => void onGenerate(false)}>{say("다시 시도", "Try again")}</button></div>}
    <PlaceComparison key={`${region}:${plan?.generatedAt}:${activePlaces.map(place => place.id).join(",")}`} places={activePlaces} requiredKeys={plan?.criteria?.facilityKeys || []} saved={saved} current={resultCurrent} en={en} onToggle={place => toggleSaved(place.id)}>{compare => <div className="place-carousel" aria-busy={loading}>
      ...
      {!loading && !activePlaces.length && !planError && !incomplete && <div className="place-empty" role="status"><h3>{!plan ? say("어떤 곳으로 떠나볼까요?", "Where will your next trip take you?") : say("선택한 조건에 맞는 여행지를 찾지 못했어요.", "No places match these preferences yet.")}</h3><p>{say("필요한 편의는 유지한 채 지역이나 여행 취향을 바꿔보세요.", "Keep your facility needs and try another region or interest.")}</p><a href="#conditions">{say("여행 조건 다시 선택", "Review preferences")}</a><button type="button" onClick={() => void onGenerate(false)}>{say("여행지 찾기", "Find places")}</button></div>}
