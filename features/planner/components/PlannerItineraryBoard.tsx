      ...
      <section className="reference-day-list" aria-label="날짜별 여행 일정">
        ...
        <ol>{active?.entries.map((entry, index) => {
          ...
          return <li key={entry.place.id}><article className="reference-stop">
            ...
            <div className="reference-stop-copy"><button type="button" onClick={() => onSelectPlace(entry.place)}>{entry.place.name}</button><small>체류 약 {entry.visitMinutes}분</small><PlaceFacilitySummary place={entry.place} en={false} /></div>
            <div className="reference-stop-actions"><button type="button" aria-label={`${entry.place.name} 같은 날 앞 순서로 이동`} disabled={!movement.up} onClick={() => trip.movePlace(entry.place.id, "up")}>↑</button><button type="button" aria-label={`${entry.place.name} 같은 날 뒤 순서로 이동`} disabled={!movement.down} onClick={() => trip.movePlace(entry.place.id, "down")}>↓</button><details><summary aria-label={`${entry.place.name} 일정 수정`}>⋮</summary><div><label>방문 날짜<select aria-label={`${entry.place.name} 여행 날짜`} value={trip.scheduleAssignments[entry.place.id] || trip.tripDays[0]} onChange={event => trip.assignPlaceToDay(entry.place.id, event.target.value)}>{trip.tripDays.map(day => <option key={day} value={day}>{day}</option>)}</select></label><button type="button" onClick={() => trip.toggleSaved(entry.place.id)}>일정에서 제거</button></div></details></div>
