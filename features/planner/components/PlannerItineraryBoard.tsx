      ...
      <section lang="ko" className="simple-timeboard" aria-label="날짜별 여행 일정">
        ...
        <ol className="simple-stops">{active?.entries.map(entry => {
          ...
          return <li key={entry.place.id} id={`itinerary-stop-${entry.place.id}`} data-selected={entry.place.id === focusedPlaceId}>
            <div className="simple-stop"><time>{entry.startsAtLabel}</time><div className="night-stop-photo"><SmartSpotImage src={entry.place.image} title={entry.place.name} region={entry.place.city} contentId={entry.place.id} tag="관광" rank={0} showMeta={false} /></div><div className="simple-stop-copy"><h3><button type="button" onClick={() => onSelectPlace(entry.place)}>{entry.place.name}</button></h3><p>{entry.visitMinutes}분 머묾 · {entry.visitEndsAtLabel}까지</p><PlaceFacilitySummary place={entry.place} en={false} /></div><button className="simple-edit-stop" type="button" onClick={() => setEditing(entry.place)} aria-label={`${entry.place.name} 일정 수정`} title="수정"><ActionIcon label="수정" /></button></div>
