...
export default function PlannerConditionsPanel(props: PlannerConditionsPanelProps) {
  ...
  return <div className="journey-workspace-block journey-conditions" id="conditions">
    ...
    {question !== 3 && <div className="condition-actions reference-bottom-bar">
      ...
      {guided && question < 2 ? <button type="button" disabled={question + 1 > lastAvailableQuestion} onClick={() => go(question + 1)}>{en ? "Continue" : `다음: ${labels[question + 1]}`} →</button> : <button type="button" disabled={!region || !themes.length || !selected.length} aria-disabled={loading || undefined} aria-busy={loading || undefined} onClick={() => { if (!loading) void props.onGenerate(); }}>{loading ? en ? "Finding places…" : "여행지 찾는 중…" : en ? "Find places" : "여행지 찾기"} →</button>}
    </div>}
