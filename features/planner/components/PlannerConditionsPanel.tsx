...
export default function PlannerConditionsPanel(props: PlannerConditionsPanelProps) {
  ...
  return <div className="journey-workspace-block journey-conditions" id="conditions" data-compact={guided}>
    ...
    <div className="condition-inputs" inert={!props.planController.criteriaReady || !props.tripSelection.storageReady} key={guided ? question : "overview"}>
      ...
      {guided && question === 0 && <div className="condition-first-details"><PlannerThemeDates t={props.t} planController={props.planController} tripSelection={props.tripSelection} part="dates" /><PlannerThemeDates t={props.t} planController={props.planController} tripSelection={props.tripSelection} part="themes" /></div>}
    ...
    {question !== 3 && <div className="condition-actions reference-bottom-bar">
      <div><strong>{en ? regionNames[region] || "Gyeongnam trip" : region || "경남 여행"}</strong><small>{selected.length ? en ? `${selected.length} facilities` : `편의 ${selected.length}개` : region ? en ? "Choose facilities" : "필요한 편의를 선택해 주세요" : en ? "Choose a region" : "지역을 선택해 주세요"}{themes.length ? en ? ` · ${themes.length} activities` : ` · 활동 ${themes.length}개 선택` : ""}</small></div>
      {guided && question > 0 && <button type="button" className="secondary" onClick={() => go(0)}>{en ? "Previous" : "이전"}</button>}
