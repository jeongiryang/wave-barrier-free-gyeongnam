...
export default function PlannerConditionsPanel(props: PlannerConditionsPanelProps) {
  ...
  return <div className="journey-workspace-block journey-conditions" id="conditions">
    ...
    {question !== 3 && <div className="condition-actions reference-bottom-bar">
      <div><strong>{en ? regionNames[region] || "Gyeongnam trip" : region || "경남 여행"}</strong><small>{selected.length ? en ? `${selected.length} facilities` : `편의 ${selected.length}개` : en ? "Choose a region" : "지역을 선택해 주세요"}{themes.length ? en ? ` · ${themes.length} activities` : ` · 활동 ${themes.length}개 선택` : ""}</small></div>
