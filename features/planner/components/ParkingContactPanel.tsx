...
export default function ParkingContactPanel({ id, parkingName, phoneNumber, placeName, onClose }: ParkingContactPanelProps) {
  ...
  return <section id={id} className="parking-contact-panel" aria-labelledby={`${id}-title`} style={{ background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: 16, marginTop: 16, overflowWrap: 'anywhere', padding: 18 }} onKeyDown={event => {
    ...
    {telHref && question ? <>
      ...
      {copyState === 'manual' && <div className="parking-manual-copy" style={{ display: 'grid', gap: 8, marginTop: 14 }}>
