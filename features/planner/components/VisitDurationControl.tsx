...
export default function VisitDurationControl({ name, value, defaultMinutes = 90, en = false, onChange }: {
  ...
  return <div className="auth-field">
    ...
    {editing && <div className="auth-field">
      ...
      <div className="travel-book-actions" style={{ gridTemplateColumns: "1fr 1fr" }}><button className="primary" type="button" disabled={!valid} onClick={apply}>{c("시간 적용", "Apply")}</button><button type="button" onClick={closeEditor}>{c("취소", "Cancel")}</button></div>
    </div>}
  </div>;
}
