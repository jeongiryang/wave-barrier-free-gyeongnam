...
export default function CourseExpansion({trip,region,themes,profiles,plan,current,onSelectPlace}:Props){
 ...
 return <details className="place-evidence" onToggle={event=>{if(!event.currentTarget.open)search.cancel();}}><summary>한 장소에서 코스 이어 담기</summary>
  <section aria-label="한 장소에서 코스 확장" style={{...courseCard,marginTop:12}}>
   ...
   <div className="travel-book-actions" style={courseActions}><button type="button" disabled={search.busy||!profiles.length||!requestedThemes} onClick={()=>{setNotice('');void search.search();}}>{search.busy?'가까운 후보를 찾는 중…':data?'같은 조건으로 후보 다시 찾기':'같은 편의로 후보 찾기'}</button>{search.busy&&<button type="button" onClick={search.cancel}>찾기 중단</button>}</div>
