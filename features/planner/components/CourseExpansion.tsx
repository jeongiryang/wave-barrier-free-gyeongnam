...
export default function CourseExpansion({trip,region,themes,profiles,plan,current,onSelectPlace}:Props){
 ...
 // 안내 문구와 빈 상태에만 말투를 적용한다. 버튼·실패 안내·수치는 표준말로 고정한다.
 ...
 return <details data-planner-tool="course" className="place-evidence" onToggle={event=>{if(!event.currentTarget.open)search.cancel();}}><summary>한 장소에서 코스 이어 담기</summary>
  <section aria-label="한 장소에서 코스 확장" style={{...courseCard,marginTop:12}}>
   <h3 style={{margin:0,fontSize:24}}>좋아하는 곳에서, 다음 풍경으로</h3><p style={courseCopy}>{say(courseGuideTones.intro)}</p>
   ...
   <p style={courseCopy} role="status">{notice||search.notice||(data?`반경 안에서 비교할 후보 ${candidates.length}곳`:say(courseGuideTones.idle))}</p>
   ...
   {data&&!candidates.length&&<p style={courseCopy}>{say(courseGuideTones.empty)}</p>}
