...
export function DayDeadlineSummary({ entries, value, en = false }: { entries: Entry[]; value?: DayDeadline; en?: boolean }) {
  ...
  return <div className="reference-tint day-deadline-summary" role="status" style={{ marginBlock: 12 }}>
    ...
    {!result ? <p>{en ? "Add places to compare the end of this day." : "장소를 담으면 이 날의 예상 종료와 비교해요."}</p> : <>
      ...
      <p className="modal-note">{en ? "An estimate for your plan. " : "일정의 예상 시간으로 비교해요. "}{!result.allLegsVerified ? en ? "Some travel legs are unverified. " : "확인하지 않은 이동 구간이 있어요. " : ""}{en ? "Allow for service changes and waiting." : "교통편 변동과 대기시간도 고려해 주세요."}</p>
    </>}
  </div>;
}
...
export default function DayDeadlineControl({ day, value, entries, onChange, en = false }: {
  ...
  return <div className="day-deadline" style={{ width: "100%", minWidth: 0 }}>
    <details ref={details} className="place-evidence" onToggle={event => { if (event.currentTarget.open) reset(); }}>
      <summary aria-label={`${day} ${en ? "return deadline" : "귀가·약속 시간"}`}>{en ? "Return / appointment time" : "귀가·약속 시간"}{value ? ` · ${value.time}` : ""}</summary>
      <div className="modal-data" onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); reset(); close(); } else if (event.key === "Enter" && event.target instanceof HTMLInputElement) { event.preventDefault(); apply(); } }}>
        ...
        <div className="auth-field"><label>{en ? "Extra buffer" : "추가 여유시간"}<select value={buffer} onChange={event => setBuffer(event.target.value)}>{[0, 15, 30, 60, 90, 120].map(minutes => <option key={minutes} value={minutes}>{minutes}{en ? " min" : "분"}</option>)}</select></label></div>
