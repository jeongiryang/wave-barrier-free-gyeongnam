...
export default function TripComfortPlan({ trip, coverage, schedule, en = false }: {
  ...
  return <details className="place-evidence trip-comfort-plan" style={{ marginBlock: 12 }}>
    ...
    <div className="modal-data">
      ...
      <p>{trip.comfort.maxWalkMinutes ? say(`나의 연속 걷기 기준은 ${trip.comfort.maxWalkMinutes}분입니다.`, `Your continuous walking limit is ${trip.comfort.maxWalkMinutes} minutes.`) : say("연속 걷기 기준은 검색 화면의 필요한 편의에서 정할 수 있어요.", "Set a walking limit in the facilities step.")}</p>
      ...
      <div className="travel-book-actions"><button type="button" aria-busy={coverage.loading} disabled={coverage.loading || !coverage.legs.length} onClick={() => void coverage.checkRoutes()}>{coverage.loading ? say("이동 구간 확인 중…", "Checking travel legs…") : say("이동 구간 확인", "Check travel legs")}</button>{coverage.loading && <button type="button" onClick={coverage.cancel}>{say("확인 중단", "Stop checking")}</button>}</div>
