...
import ParkingContactPanel from './ParkingContactPanel';
...
export default function ParkingAlternatives({ place }: { place: Place }) {
  ...
  const [contactParkingId, setContactParkingId] = useState('');
  const request = useRef<AbortController | null>(null);
  const contactButtons = useRef(new Map<string, HTMLButtonElement>());
  ...
  useEffect(() => {
    if (previousPlace.current !== place.id) {
      ...
      setData(null); setExpanded(false); setSelected(''); setReference('place'); setDevicePoint(null); setContactParkingId('');
      setState('cancelled');
    }
  ...
  const closeContact = (id: string) => {
    setContactParkingId('');
    requestAnimationFrame(() => contactButtons.current.get(id)?.focus());
  };
  ...
  return <section className="parking-alternatives" aria-label="주변 주차장">
    ...
    {expanded && data && <div className="parking-results" aria-live="polite">
      ...
      {data.status === 'empty' ? <p>공식 데이터에서 조건에 맞는 주변 주차장을 찾지 못했어요.</p> : <div className="parking-list">{sorted.map(item => <article key={item.id}>
        ...
        <small>{caution}</small>
        <div className="parking-actions"><button type="button" aria-pressed={selected === item.id} onClick={() => setSelected(item.id)}>도착지로 선택</button><button type="button" style={{ background: 'var(--white)', border: '1px solid var(--line)', color: 'var(--ink)' }} ref={node => { if (node) contactButtons.current.set(item.id, node); else contactButtons.current.delete(item.id); }} aria-expanded={contactParkingId === item.id} aria-controls={`parking-contact-${item.id}`} onClick={() => contactParkingId === item.id ? closeContact(item.id) : setContactParkingId(item.id)}>주차장에 문의하기</button><a target="_blank" rel="noopener noreferrer" href={`https://map.kakao.com/link/map/${encodeURIComponent(item.name)},${item.destination.latitude},${item.destination.longitude}`}>지도에서 보기</a></div>
        {selected === item.id && <p role="status">일정과 시간은 바꾸지 않고 도착 참고정보로 선택했어요.</p>}
        {contactParkingId === item.id && <ParkingContactPanel parkingName={item.name} phoneNumber={item.phoneNumber} placeName={place.name} panelId={`parking-contact-${item.id}`} onClose={() => closeContact(item.id)} />}
      </article>)}</div>}
    </div>}
