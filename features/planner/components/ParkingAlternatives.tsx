...
export default function ParkingAlternatives({ place }: { place: Place }) {
  ...
  useEffect(() => {
    if (previousPlace.current !== place.id) {
      ...
      setData(null); setExpanded(false); setSelected(''); setReference('place'); setDevicePoint(null); setOpenContactId(null);
      setState('cancelled');
    }
  ...
  const load = async () => {
    if (data) { setExpanded(value => { if (value) setOpenContactId(null); return !value; }); return; }
    request.current?.abort();
