...
export default function TripBudgetEntry({trip,coverage,region}:{trip:ReturnType<typeof useTripSelection>;coverage:ReturnType<typeof useItineraryRoutes>;region:string}){
 ...
 return <details className="place-evidence" onToggle={event=>{if(event.currentTarget.open)setOpened(true);}}>
  <summary>여행비 계획하기</summary>
  {opened&&<Suspense fallback={<p role="status">여행비 계획을 준비하고 있어요.</p>}><TripBudget trip={trip} routes={coverage.costByPlaceId} region={region}/></Suspense>}
 </details>;
}
