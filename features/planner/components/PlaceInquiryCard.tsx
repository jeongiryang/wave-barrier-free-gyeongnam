...
export default function PlaceInquiryCard({ place, en }: { place: Place; en: boolean }) {
  ...
  return <section className="place-inquiry-entry">
    ...
    {open && <Suspense fallback={<LoadingState>{en ? "Preparing your card…" : "문의 카드를 준비하고 있어요…"}</LoadingState>}><InquiryDialog place={place} en={en} selected={selected} extra={extra} onSelection={setSelected} onExtra={setExtra} onClose={close} /></Suspense>}
  </section>;
}
