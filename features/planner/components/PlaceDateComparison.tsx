...
export default function PlaceDateComparison({ place, trip, onMoved }: { place: Place; trip: ReturnType<typeof useTripSelection>; onMoved: () => void }) {
  ...
  return <section className="account-settings">
    ...
    <div className="travel-book-actions"><button type="button" aria-busy={loading} disabled={loading} onClick={() => void load()}>{loading ? "날짜별 예측 확인 중…" : data ? "날짜별 예측 다시 확인" : "날짜별 예측 확인"}</button>{loading && <button type="button" onClick={() => { pending.current?.abort(); setNotice("확인을 중단했어요."); }}>확인 중단</button>}</div>
    {data && <>
    ...
    {pinned && <p>고정한 장소의 날짜는 유지합니다. 옮기려면 고정을 해제하세요.</p>}
