...
function PlaceReturnTransport({ place, targets }: { place: Place; targets: Place[] }) {
  ...
  const directionHeading = useRef<HTMLHeadingElement>(null), focusDirection = useRef(false);
  ...
  useEffect(() => {
    if (!info?.direction || !focusDirection.current || !directionHeading.current) return;
    focusDirection.current = false;
    directionHeading.current.focus({ preventScroll: true });
    directionHeading.current.scrollIntoView({ block: 'start', behavior: 'instant' });
  }, [info]);
  ...
  async function load(stop?: PublicStop, routeId?: string) {
    ...
    setBusy(true); setError(''); focusDirection.current = Boolean(routeId);
  ...
  return <section aria-label="장소에서 돌아가는 교통" aria-busy={busy} style={{ display: 'grid', gap: 16, marginTop: 20 }}>
    <div className="travel-book-actions" style={actionsStyle}><button type="button" disabled={busy} onClick={() => void load()}>{busy ? '교통 정보 확인 중…' : info ? '주변 정류장 다시 확인' : '주변 정류장 확인'}</button><a href={`https://map.kakao.com/link/search/${encodeURIComponent(place.name + ' ' + (place.address || ''))}`} target="_blank" rel="noreferrer">카카오맵에서 교통편 보기</a></div>
    ...
    {info && <>
      ...
      {info.selected && <section aria-label="선택한 정류장의 도착 정보" style={{ display: 'grid', gap: 12, marginTop: 8 }}>
        ...
        <div className="travel-book-actions" style={actionsStyle}><button type="button" disabled={busy || (info.arrivalCheckedAt ? now-Date.parse(info.arrivalCheckedAt)<15000 : false)} onClick={() => void load(info.selected)}>도착 정보 다시 확인</button></div>
        ...
        <div style={{ ...gridStyle, marginTop: 16 }}>{(allRoutes ? info.routes : info.routes.slice(0,6)).map(route => <article key={route.routeId} style={cardStyle}>
          ...
          <div className="travel-book-actions" style={actionsStyle}><button type="button" disabled={busy} aria-pressed={info.routeId === route.routeId} onClick={() => void load(info.selected, route.routeId)}>이 노선의 진행 방향 확인</button></div>
        </article>)}</div>
        {info.routes.length > 6 && <div className="travel-book-actions" style={actionsStyle}><button type="button" aria-expanded={allRoutes} onClick={() => setAllRoutes(!allRoutes)}>{allRoutes ? '가까이 도착할 노선 6개만 보기' : `확인한 노선 ${info.routes.length}개 모두 보기`}</button></div>}
      </section>}
