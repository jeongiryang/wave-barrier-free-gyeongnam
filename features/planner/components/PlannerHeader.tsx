...
export function PlannerHeader({ t, scrolled, hidden, savedCount, onNavigate }: {
  ...
  return <header className={`site-header ${scrolled ? "scrolled" : ""} ${hidden ? "hidden" : ""}`}>
    ...
    ]} /><HelpCenter /><PreferenceControls /><AccountMenu loginHref="/login?next=%2Fplanner" /><button className="header-action" type="button" onClick={() => onNavigate("itinerary")}>내 일정 <b>{savedCount}</b><span aria-hidden="true">↗</span></button></div>
  </header>;
}
