...
export function PlannerHeader({ t, scrolled, hidden, savedCount }: {
  ...
  return <header className={`site-header ${scrolled ? "scrolled" : ""} ${hidden ? "hidden" : ""}`}>
    ...
    <nav aria-label="주요 메뉴"><a href="#planner">{t("conditions", "내 여행 만들기")}</a><a href="#journey">추천·일정</a><a href="#navigation">{t("route", "길찾기")}</a><Link href="/photo-course">사진 코스</Link><Link href="/community">커뮤니티</Link></nav>
