  ...
  return <section className="day-planner" data-reveal aria-label="날짜별 여행 일정">
    ...
    <div className="day-planner-grid">{schedule.map(({ day, entries }, dayIndex) => <article key={day}>
      ...
      <ol>{entries.map((entry, index) => { const movement = movementFor(entry.place.id); return <li key={entry.place.id}>
        <span>{index + 1}</span>
        <div className="day-place-copy"><b>{entry.startsAtLabel} · {entry.place.name}</b><small>{entry.travelSource === "route" ? "확인된 경로" : entry.travelSource === "estimate" ? "직선거리 기반 추정" : "경로 미확인 · 임시"} 이동 {entry.travelMinutes}분 · 체류 {entry.visitMinutes}분 · {entry.endsAtLabel} 종료</small><small>{entry.place.address || entry.place.city}</small><small className="day-evidence">{typeof entry.place.score === "number" && entry.place.score > 0 ? `확인된 편의시설 ${entry.place.score}% · 공식 항목 ${entry.place.knownFields || 0}개` : "편의시설 정보 확인 필요"}</small>{entry.crossesDateBoundary && <em>일정이 다음 날로 이어집니다.</em>}</div>
