...
export default function TripDayPlanner({ plan, tripSelection, route, audioGuide, participation, archiveContext, itineraryRouteMinutes = {} }: {
  ...
  return <section className="day-planner" data-reveal aria-label={c("날짜별 여행 일정", "Itinerary by date")}>
    ...
    <div className="itinerary-primary-actions">
      ...
      <button type="button" onClick={() => { if (plan && shareState !== "saving") void sharePlan(); }} disabled={!plan} aria-disabled={!plan || shareState === "saving"} aria-busy={shareState === "saving"}>{shareState === "saving" ? c("링크 만드는 중", "Creating link") : shareState === "done" ? c("링크 복사 완료", "Link copied") : c("공유 링크 만들기", "Create shared link")}</button>
