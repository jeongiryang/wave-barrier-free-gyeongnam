...
import { visitDurationFor, buildItinerarySchedule, routeMinutesForOriginLeg } from "../optimization/itinerary-schedule.js";
...
export default function TripDayPlanner({ plan, tripSelection, route, audioGuide, participation, archiveContext, itineraryRouteMinutes = {} }: {
  ...
  const {
    ...
    assignPlaceToDay, dayStartTime, setDayStartTime, visitMinutesByPlaceId, setVisitMinutes,
  ...
  return <section className="day-planner" data-reveal aria-label={c("날짜별 여행 일정", "Itinerary by date")}>
    ...
    {tripSelection.constraintNotice && <p role="status">{tripSelection.constraintNotice}</p>}
    ...
    {outsideDates.length > 0 && <section className="outside-trip-dates" aria-label={c("다른 날짜에 보관된 장소", "Places outside these dates")}><h3>{c(`이 기간 밖에 보관된 장소 ${outsideDates.length}곳`, `${outsideDates.length} places outside these dates`)}</h3><p>{c("날짜를 바꿔도 이전 일정은 자동으로 옮기지 않습니다. 이번 여행에 넣을 장소만 직접 이동하세요.", "Changing dates does not move earlier plans automatically. Move only the places you want in this trip.")}</p><ul>{outsideDates.map((place) => <li key={place.id}><span><span lang={originalLanguage(place.name)}>{place.name}</span> · {scheduleAssignments[place.id]}</span><select disabled={Boolean(tripSelection.fixedVisits[place.id])} aria-label={c(`${place.name} 이번 여행 날짜로 이동`, `${place.name} move to a date in this trip`)} value="" onChange={(event) => assignPlaceToDay(place.id, event.target.value)}><option value="" disabled>{c("날짜 선택", "Choose a date")}</option>{tripDays.map((day) => <option key={day} value={day}>{day}</option>)}</select><button type="button" disabled={Boolean(tripSelection.fixedVisits[place.id])} onClick={() => toggleSaved(place.id)}>{c("일정에서 제거", "Remove from itinerary")}</button>{tripSelection.fixedVisits[place.id] && <button type="button" onClick={() => tripSelection.setFixedVisit(place.id, null)}>{c("고정 해제", "Unpin")}</button>}</li>)}</ul></section>}
    <header><div><span>{c("내 일정", "My itinerary")}</span><h3>{c(`${orderedSavedPlaces.length - outsideDates.length}곳을 날짜별로 정리했어요.`, `${orderedSavedPlaces.length - outsideDates.length} ${orderedSavedPlaces.length - outsideDates.length === 1 ? "place" : "places"} arranged by date.`)}</h3></div><div className="day-start-control"><label htmlFor="day-start-time">{c("하루 시작", "Day starts at")}</label><div className="day-time-entry"><input ref={startTimeInput} id="day-start-time" type="time" value={dayStartTime} onChange={(event) => setDayStartTime(event.target.value)} /><button type="button" onClick={() => { startTimeInput.current?.focus(); try { startTimeInput.current?.showPicker?.(); } catch { /* Native text entry remains available when a picker cannot open. */ } }}>{c("시간 선택", "Choose time")}</button></div><small>{c("확인한 실제 경로가 있으면 이동시간에 반영합니다.", "Checked routes are used for travel times when available.")}</small></div></header>
    <div className="day-order-toolbar"><div><strong>{orderMode === "manual" ? c("내가 정한 순서", "Your visit order") : c("추천 순서", "Suggested order")}</strong><p className="day-planner-explanation">{orderExplanation} {c("날짜와 순서는 저장한 일정과 공유 일정에 반영됩니다.", "Dates and order are included in saved and shared itineraries.")}</p></div>{orderMode === "manual" && <button type="button" onClick={() => { restoreAutoOrder(); setEdit(null); }}>{c("추천 순서로 정렬", "Restore suggested order")}</button>}</div>
