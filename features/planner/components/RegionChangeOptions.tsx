...
export default function RegionChangeOptions({ en, error, onAdd, onNew }: { en: boolean; error: boolean; onAdd: () => void; onNew: () => void }) {
  return <>
    ...
    <p>{en ? "Starting a new trip clears your current itinerary and route results. Trips already saved in your travel book are kept." : "새 여행을 시작하면 현재 일정과 경로 결과를 비웁니다. 여행집에 따로 보관한 여행은 유지합니다."}</p>
    {error && <p role="alert">{en ? "Could not save this change. The current trip is kept. Check storage permissions and try again, or cancel." : "변경 내용을 저장하지 못했어요. 현재 여행은 유지됩니다. 저장 권한을 확인한 뒤 다시 시도하거나 취소해 주세요."}</p>}
