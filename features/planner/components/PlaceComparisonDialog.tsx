...
export default function PlaceComparisonDialog({ places, requiredKeys, saved, current, en, onClose, onToggle }: {
  ...
  const labels = en ? { confirmed: "Reported available", negative: "Reported unavailable", unknown: "Not reported" } : { confirmed: "확인됨", negative: "불일치", unknown: "미확인" };
  return <dialog ref={dialog} className="region-change-dialog place-comparison-dialog" aria-labelledby="place-comparison-title">
    ...
    <p>{en ? "These are official records from this search. Unreported information does not mean a facility is absent. Scroll sideways on a small screen." : "이번 검색의 공식 기록을 비교합니다. 미확인은 시설이 없다는 뜻이 아니에요. 작은 화면에서는 표를 좌우로 넘겨보세요."}</p>
    ...
    <div className="place-comparison-scroll" tabIndex={0} role="region" aria-label={en ? "Facility comparison table" : "장소별 편의 비교표"}>
      <table><caption className="sr-only">{en ? `${places.length} places` : `${places.length}곳 편의 비교`}</caption><thead><tr><th scope="col">{en ? "Facility" : "살펴볼 편의"}</th>{places.map(place => <th scope="col" key={place.id}><span lang={originalLanguage(place.name)}>{place.name}</span><small>{place.city}</small></th>)}</tr></thead>
    ...
    <p className="modal-note">{en ? "The record does not certify current access. Use the inquiry card in a place's visitor information to check before visiting." : "현재 접근 가능성을 보장하는 정보는 아니에요. 장소의 이용 정보에서 문의 카드를 만들어 방문 전에 확인해 보세요."}</p>
  </dialog>;
}
