...
export default function TravelComfortChoices({ selected, comfort, onSelected, onComfort, en = false }: {
  ...
  return <details className="travel-profile-card trip-comfort-choices" ref={details}>
    <summary onClick={() => { if (!details.current?.open) setDraft(comfort); }}><span>{say("걷기·휴식·동행 조건", "Walking, rests and companions")}</span><b aria-hidden="true">+</b></summary>
    <div className="travel-profile-content" onKeyDown={event => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); setDraft(comfort); close(); } }}>
      ...
      <div className="travel-profile-actions"><button type="button" onClick={() => { onComfort(sanitizeComfort(draft)); setNotice(say("선택한 기준을 준비했어요. 아래 적용을 누르면 저장돼요.", "Preferences are ready. Apply below to save.")); }}>{say("이 기준 선택", "Choose preferences")}</button><button type="button" onClick={() => { setDraft(comfort); close(); }}>{say("취소", "Cancel")}</button></div>
      <details className="place-evidence" style={{ background: "var(--paper)" }}><summary>{say("동행자 조건 함께 모으기", "Combine companion needs")}</summary><div className="modal-data">
        ...
        <div className="travel-profile-actions"><button type="button" disabled={members.length >= 8} onClick={() => { const id = nextMember.current++; setMembers(current => [...current, { id, profiles: [], maxWalkMinutes: null }]); }}>{say("동행자 추가", "Add companion")}</button><button type="button" onClick={() => { const result = combineCompanionNeeds(members, selected, draft); onSelected(result.selected); onComfort(result.comfort); setDraft(result.comfort); setNotice(say(`편의 ${result.selected.length}개${result.comfort.maxWalkMinutes ? ` · 연속 걷기 ${result.comfort.maxWalkMinutes}분` : ""}을 합쳤어요. 아래 적용을 누르면 저장돼요.`, "Combined facilities and walking limit applied.")); }}>{say("동행 조건 합치기", "Combine needs")}</button></div>
