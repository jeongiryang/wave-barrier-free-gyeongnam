...
import { savedSplitReunionLines } from '../../../lib/split-reunion.js';
...
export default function OfflineTripPack({trip,coverage,origin,region,progressMemory}:{trip:ReturnType<typeof useTripSelection>;coverage:ReturnType<typeof useItineraryRoutes>;origin:RoutePoint;region:string;progressMemory:TripProgressMemory}) {
  ...
  return <section className="account-settings" aria-label="여행 요약 파일">
    ...
    <label className="departure-review-check"><input type="checkbox" checked={includeSplit} onChange={event=>setIncludeSplit(event.target.checked)}/>저장한 A·B 합류 약속 포함</label>
    {!complete&&<p role="status">여행 기간 밖에 있는 장소의 날짜를 확인해 주세요</p>}<div className="travel-book-actions"><button type="button" style={{fontSize:14}} aria-busy={Boolean(loading)} disabled={!complete||!trip.orderedSavedPlaces.length||Boolean(loading)} onClick={()=>download('html')}>여행 요약 파일 저장</button><button type="button" style={{fontSize:14}} aria-busy={Boolean(loading)} disabled={!complete||!trip.orderedSavedPlaces.length||Boolean(loading)} onClick={()=>download('txt')}>텍스트로 저장</button></div>
