...
export default function TransportDatasetPanel({ activePlaces, route }: { activePlaces: Place[]; route: ReturnType<typeof useRoutePlanning> }) {
  ...
  const { privateOrigin, routeStart, routeStartLabel, routeStartIsPrivate, routeTravelMode, setRouteTravelMode, routeDestination, routeLoading, routeFailed, transportContext, loadRoutes, selectedDataset, selectedTransportDataset } = route;
  const id = selectedDataset?.id || selectedTransportDataset;
  const destination = routeDestination || activePlaces[0];
  const privateStart = routeStart ? routeStartIsPrivate : privateOrigin;
  const unavailable = privateStart || !destination;
  const transitSelected = routeTravelMode === "transit";
  ...
  async function retry() {
    ...
    if (!transitSelected) { setRouteTravelMode("transit"); return; }
    pending.current = true;
    try { await loadRoutes(destination, routeStart ?? undefined, privateStart, routeStart ? routeStartLabel : undefined); } finally { pending.current = false; }
  }
  return <section className="transport-data-panel" aria-labelledby="transport-data-title" {...focus}>
    <div className="transport-data-heading"><div><span>{english ? "Transport information" : "교통정보"}</span><h3 id="transport-data-title">{datasetName(id, english, selectedDataset?.name)}</h3><p>{datasetDescription(id, english)}</p></div><button type="button" onClick={() => void retry()} disabled={unavailable} aria-disabled={unavailable || routeLoading} aria-busy={routeLoading}>{routeLoading ? (english ? "Checking…" : "확인 중") : !transitSelected ? (english ? "Select public transport" : "대중교통으로 확인") : (english ? "Check these conditions again" : "현재 조건 다시 확인")}</button></div>
    {english && <p className="transport-source-note">Stop, route and station names are shown in their original language.</p>}
    {id === "bus-arrival" && transitSelected && !routeLoading && !privateStart && result === "data" && transportContext && <p className="transport-source-note"><ArrivalRetrievedAt value={transportContext.arrivalRetrievedAt} english={english} /></p>}
    <div className="transport-data-results" aria-busy={routeLoading} aria-live="polite">
      {routeLoading ? <p className="wave-loading-inline" role="status">{english ? "Checking current transport information…" : "현재 교통정보를 확인하고 있습니다…"}</p> : privateStart ? <p>{english ? "Choose a public departure point to request transport information, or check directly in the map provider's app." : "교통정보를 조회하려면 공개 출발 거점을 선택하거나 지도 앱에서 직접 확인해 주세요."}</p> : !transitSelected ? <p>{english ? "Select public transport to check bus and rail information for this journey." : "이 구간의 버스·철도 정보를 확인하려면 대중교통을 선택하세요."}</p> : result !== "data" ? <div className="transport-data-empty"><strong>{selectedDataset?.failure ? providerFailureMessage(selectedDataset.failure, english) : resultMessage(result, id, english)}</strong><span>{english ? "Check again after reviewing your departure and destination, or use official booking." : "출발지·목적지를 확인해 다시 조회하거나 공식 예매에서 확인하세요."}</span></div> : transportContext && <>
