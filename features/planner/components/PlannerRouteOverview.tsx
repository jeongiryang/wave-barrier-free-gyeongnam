...
export default function PlannerRouteOverview({ plan, region, theme, selectedProfileIds, liveCount, audioGuide, participation }: PlannerRouteOverviewProps) {
  ...
  return <div className={`route-section journey-workspace-part ${plan ? "revealed" : ""}`} id="route">
    <div className="route-intro" data-reveal>
      <div><p className="section-kicker">YOUR W.A.V.E ROUTE</p><span className={`route-status ${plan?.mode ?? "fallback"}`}><i />{plan?.mode === "live" ? "최신 여행정보 반영" : plan?.mode === "partial" ? "확인된 정보 우선 반영" : "코스 미리보기"}</span><h2>{activeProfiles.map((item) => item.label).slice(0, 2).join("·")} 조건으로 찾은<br />{region}의 하루</h2><p>{region} · {activeTheme} · {activeProfiles.map((item) => item.label).join(" · ")}</p></div>
