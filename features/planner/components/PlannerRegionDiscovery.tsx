...
export default function PlannerRegionGallery({ value, onChange, full = false }: { full?: boolean; value: string; onChange: (region: string) => void; onInterest: (theme: string) => void; onFacilities: () => void }) {
  ...
  return <div className="simple-region-discovery">
    <div className="simple-region-grid" id="planner-region-options">{names.map(name => {
      ...
      return <article className="simple-region" key={name}>
        <button type="button" className="simple-region-link" lang="ko" onClick={() => onChange(name)} aria-label={`${name} 지역 선택`} aria-pressed={name === value}>
    ...
    <button type="button" className="simple-show-regions" aria-expanded={expanded} aria-controls="planner-region-options" onClick={() => setExpanded(!expanded)}>{expanded ? en ? "Show fewer" : "접기" : en ? "All 18 regions" : "전체 18개 지역"}<span aria-hidden="true">{expanded ? "−" : "+"}</span></button>
  </div>;
}
