...
export default function PlannerRegionDiscovery({ value, onChange }: { value: string; onChange: (region: string) => void }) {
  ...
  return <div className="planner-region-discovery">
    <figure className="planner-destination-image">
      {failedImage !== photo.image && <img key={photo.image} src={photo.image} width="1000" height="800" loading="lazy" decoding="async" alt={`${name} · ${photo.title}`} onError={() => setFailedImage(photo.image)} />}
      <div className="planner-destination-caption">
        ...
        <strong>{value === "경남 전체" ? en ? "All of Gyeongnam" : "경남 전체" : en ? regionNames[name] : name}</strong>
        <p lang="ko">{photo.title}</p>
      </div>
      <figcaption><span lang="ko">{photo.photographer || "한국관광공사"}</span> · <a href={regionPhotoSource(photo).href} target="_blank" rel="noopener noreferrer">{en ? "Original photo ↗" : "사진 원본 ↗"}</a>{failedImage === photo.image && <span> · {en ? "Photo unavailable" : "사진을 불러오지 못했어요"}</span>}</figcaption>
    </figure>
