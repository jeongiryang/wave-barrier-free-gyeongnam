...
export default function TripDayTools(props:{trip:ReturnType<typeof useTripSelection>;coverage:ReturnType<typeof useItineraryRoutes>;origin:RoutePoint;region:string;onSelectPlace:(place:Place)=>void}) {
 const [mode,setMode]=useState<'progress'|'pack'|'transport'|null>(null);
 ...
 return <section className="account-settings">
  ...
  {mode&&<Suspense fallback={<p role="status">여행 정보를 준비하고 있어요.</p>}>{mode==='progress'?<OnTripGuide {...props} progressMemory={progressMemory} onProgressChange={rememberProgress}/>:mode==='transport'?<ReturnTransport trip={props.trip}/>:<OfflineTripPack {...props} progressMemory={progressMemory}/>}</Suspense>}
 </section>;
}
