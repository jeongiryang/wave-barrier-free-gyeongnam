...
export default function TripDayTools(props:{trip:ReturnType<typeof useTripSelection>;coverage:ReturnType<typeof useItineraryRoutes>;origin:RoutePoint;region:string;onSelectPlace:(place:Place)=>void}) {
 ...
 return <section className="account-settings">
  ...
  <div className="travel-book-actions" style={courseActions}><button type="button" aria-pressed={mode==='progress'} onClick={()=>setMode(mode==='progress'?null:'progress')}>여행 당일 진행</button><button type="button" aria-pressed={mode==='pack'} onClick={()=>setMode(mode==='pack'?null:'pack')}>여행 요약 챙기기</button><button type="button" aria-pressed={mode==='transport'} onClick={()=>setMode(mode==='transport'?null:'transport')}>돌아가는 교통 확인</button><button type="button" aria-pressed={mode==='split'} onClick={()=>{setSplitMounted(true);setMode(mode==='split'?null:'split');}}>동행과 합류 계획</button></div>
  ...
  {mode&&mode!=='split'&&<Suspense fallback={<p role="status">여행 정보를 준비하고 있어요.</p>}>{mode==='progress'?<OnTripGuide {...props} progressMemory={progressMemory} onProgressChange={rememberProgress}/>:mode==='transport'?<ReturnTransport trip={props.trip}/>:<OfflineTripPack {...props} progressMemory={progressMemory}/>}</Suspense>}
  {splitMounted&&<div hidden={mode!=='split'}><Suspense fallback={<p role="status">합류 계획을 준비하고 있어요.</p>}><SplitReunion trip={props.trip} coverage={props.coverage} origin={props.origin}/></Suspense></div>}
 </section>;
}
