...
export default function PlaceComparison({ places, requiredKeys, saved, current, en, onToggle, children }: {
  ...
  return <>
    ...
    {active && open && selected.length >= 2 && <Suspense fallback={<p role="status">{en ? "Preparing comparison…" : "편의를 나란히 정리하고 있어요…"}</p>}><ComparisonDialog places={selected} requiredKeys={requiredKeys} saved={saved} current={current} en={en} onClose={close} onToggle={onToggle} /></Suspense>}
  </>;
}
