...
import RestroomAlternativeCards from "./RestroomAlternativeCards";
...
export default function RestStopFinder({ trip, places, requiredKeys, onSelectPlace, en = false }: {
  ...
  useEffect(() => { const open = (event: Event) => { const id = (event as CustomEvent<{ contentId?: string }>).detail?.contentId || ''; const target = dayPlaces.find(place => place.id === id); setPurpose('restroom'); if (target) setChoice(target.id); else setNotice('먼저 이 장소를 일정에 담은 뒤 주변 공중화장실을 찾아보세요.'); if (panel.current) { panel.current.open = true; panel.current.scrollIntoView({ block: 'start' }); } }; window.addEventListener('wave:open-restroom-finder', open); return () => window.removeEventListener('wave:open-restroom-finder', open); }, [dayPlaces]);
  return <details ref={panel} id="restroom-alternatives-entry" className="place-evidence rest-stop-finder" style={{ marginBlock: 12 }}>
    ...
    <div className="modal-data">
      ...
      {anchor ? <>
        ...
        <h3>{say("일정에 있는 장소", "Places in the itinerary")}</h3><p role="status">{say(`조건에 맞는 가까운 곳 ${candidates.length}곳`, `${candidates.length} nearby places in these results`)}</p>
