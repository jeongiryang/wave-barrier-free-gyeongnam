...
export default function TripPointPicker({ activePlaces, route, locationSearch, onChoosePoint, onClose }: {
  ...
  return <section id="trip-point-picker" className="trip-point-picker" aria-labelledby={headingId}>
    <header><div><small>{pointPicker === "origin" ? "출발지 선택" : "도착지 선택"}</small><strong id={headingId}>{pointPicker === "origin" ? "어디에서 출발할까요?" : "어디로 이동할까요?"}</strong></div><button type="button" onClick={onClose} aria-label="선택 창 닫기">×</button></header>
    ...
    <form onSubmit={(event) => { event.preventDefault(); void searchLocations(); }}><input ref={searchInputRef} value={placeQuery} onChange={(event) => setPlaceQuery(event.target.value)} placeholder="장소명·역·터미널·관광지를 직접 입력" aria-label="장소 검색" /><button type="submit" disabled={placeSearchLoading || placeQuery.trim().length < 2}>{placeSearchLoading ? "검색 중" : "검색"}</button></form>
    <div className="trip-point-list">
      {pointPicker === "origin" && departurePresets.map((item) => <button type="button" key={item.id} onClick={() => { updateOrigin(item.point, item.name); if (routeDestination || activePlaces[0]) void loadRoutes(routeDestination || activePlaces[0], item.point, false, item.name); onClose(); }}><i aria-hidden="true">S</i><span><strong>{item.name}</strong><small>{item.detail}</small></span></button>)}
      {activePlaces.slice(0, 8).map((place, index) => <button type="button" key={`${pointPicker}-${place.id}`} onClick={() => choosePoint(place)}><i aria-hidden="true">{index + 1}</i><span><strong>{place.name}</strong><small>{place.address || place.summary}</small></span>{index === 0 && <em>W.A.V.E 추천</em>}</button>)}
      {placeSearchResults.map((item) => <button type="button" key={`search-${item.id}`} onClick={() => choosePoint(searchableToPlace(item))}><i aria-hidden="true">⌕</i><span><strong>{item.name}</strong><small>{item.address || item.category}</small></span><em>직접 검색</em></button>)}
