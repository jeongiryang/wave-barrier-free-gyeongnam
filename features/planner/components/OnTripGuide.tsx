...
import { cleanOnTrip, easyOnTripSteps, onTripIdentity, readOnTrip, remainingOnTrip, saveOnTrip, type TripProgress, type TripProgressMemory } from '../../../lib/on-trip.js';
...
function DayGuide({ trip, coverage, origin, onSelectPlace, progressMemory, onProgressChange }: {trip:ReturnType<typeof useTripSelection>;coverage:ReturnType<typeof useItineraryRoutes>;origin:RoutePoint;onSelectPlace:(place:Place)=>void;progressMemory:TripProgressMemory;onProgressChange:(identity:string,value:TripProgress,unsaved?:boolean)=>void}) {
  ...
  const [undo, setUndo] = useState<TripProgress|null>(null), [resetting, setResetting] = useState(false), [view, setView] = useState<'basic'|'easy'>('basic');
  ...
  useEffect(() => {
    const load = (external = false) => {
      ...
      try {
        ...
        else if(external)setNotice('다른 창에서 바뀐 진행 기록을 불러왔어요.');
  ...
  function persist(value:TripProgress) {
    ...
    catch {memory.current={...memory.current,[identity]:{value:clean,unsaved:true}};onProgressChange(identity,clean,true);setNotice('진행 기록을 저장하지 못했어요. 이 화면에서는 계속 이용할 수 있어요.');return false;}
  }
  function mark(state:'done'|'skipped') {
    ...
    if(saved) {
      const following=remaining.entries[1]?.place.name;
      setNotice(view==='easy' && state==='done' && following ? `${next.name}을 완료했어요. 다음은 ${following}이에요.` : `${next.name} ${state==='done'?'방문을 완료했어요.':'방문을 건너뛰었어요.'} 원래 계획은 보관됩니다.`);
    }
    if(state==='done')vibrate(saved?'confirm':'alert',haptics==='on');
  }
  const restoreUndo=()=>{if(!undo)return;if(persist(undo))setNotice('직전 진행 표시를 되돌렸어요.');setUndo(null);};
  return <section className="account-settings" aria-label="여행 당일 진행">
    <p className="section-kicker">여행을 이어서</p>
    <div className="easy-trip-toggle" role="group" aria-label="여행 당일 보기"><button type="button" aria-pressed={view==='basic'} onClick={()=>setView('basic')}>기본 보기</button><button type="button" aria-pressed={view==='easy'} onClick={()=>{setView('easy');setRunning(true);}}>쉬운 보기</button></div>
    ...
    {undo&&<div className="travel-book-actions"><button type="button" style={{fontSize:14}} onClick={restoreUndo}>직전 진행 되돌리기</button></div>}
