...
export default function PlannerFooter() {
  ...
  return <footer className="simple-footer">
    <div className="brand footer-brand"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>WAVE</span></div>
    <div className="footer-notes"><p>{en ? "A Gyeongnam travel guide for everyone's access needs" : "누구나 원하는 곳으로, 경남 무장애 여행 길잡이"}</p><p className="trust-notice">{en ? "WAVE is an independent service using public tourism information. It is not operated by the Korea Tourism Organization or Gyeongsangnam-do." : "WAVE는 공공 관광정보를 연결해 제공하는 독립 서비스이며 한국관광공사·경상남도의 공식 운영 서비스가 아닙니다."}</p><PolicyFooterLinks /></div>
