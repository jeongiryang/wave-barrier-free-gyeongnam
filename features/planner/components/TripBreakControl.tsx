...
export default function TripBreakControl({ name, value, purpose, onChange, onPurpose, en = false }: {
  ...
  return <div className="auth-field trip-break-control" style={{ width: "100%", minWidth: 0 }}>
    ...
    {onPurpose && <label>{say("이곳에 들르는 이유", "Purpose of this stop")}<select aria-label={say(`${name} 방문 목적`, `${name} stop purpose`)} value={purpose ?? ""} onChange={event => onPurpose(event.target.value ? event.target.value as StopPurpose : null)}><option value="">{say("둘러보기", "Visit")}</option><option value="rest">{say("쉬어 가기", "Rest stop")}</option><option value="restroom">{say("화장실 이용", "Restroom stop")}</option><option value="nap">{say("낮잠", "Nap")}</option><option value="nursing">{say("수유", "Nursing")}</option></select></label>}
...
export function TripBreakSummary({ minutes, purpose, start, end, en = false, inheritColor = false }: { minutes?: number; purpose?: StopPurpose; start?: string; end?: string; en?: boolean; inheritColor?: boolean }) {
  ...
  return <p className="modal-note" style={{ marginBlock: 8, color: inheritColor ? "inherit" : undefined }}>{purpose && <b>{purpose === "nap" ? en ? "Nap" : "낮잠" : purpose === "nursing" ? en ? "Nursing" : "수유" : purpose === "restroom" ? en ? "Restroom stop" : "화장실 이용" : en ? "Rest stop" : "쉬어 가는 곳"} · </b>}{minutes ? <>{en ? `Rest ${minutes} min after visiting` : `방문 뒤 ${minutes}분 휴식`}{start && end ? ` · ${start}–${end}` : ""}</> : en ? "Check facilities before visiting" : "방문 전 시설 확인"}</p>;
}
