...
import { readTripValue, CURRENT_TRIP_KEY, CURRENT_TRIP_VALUE_KEYS } from '../../../lib/current-trip-storage.js';
...
export default function TripShareMenu({ trip, participation, region }: { trip: ReturnType<typeof useTripSelection>; participation: ReturnType<typeof usePlannerParticipation>; region: string }) {
  ...
  async function calendar() {
    ...
    try {
      const shareUrl = await participation.ensureShareUrl();
      const { buildTripCalendarIcs } = await import('../../../lib/trip-calendar.js');
      download(buildTripCalendarIcs({ travelStart: trip.travelStart, travelEnd: trip.travelEnd, dayStartTime: trip.dayStartTime, travelMode: trip.travelMode, title: `${region} 여행`, region, placeNames: trip.orderedSavedPlaces.map(place => place.name), shareUrl }), 'text/calendar;charset=utf-8', 'wave-trip.ics');
