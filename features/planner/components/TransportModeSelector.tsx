  ...
  return <>
    ...
    */}
    ...
    {transportContext && (transportContext.nearbyStops.length > 0 || transportContext.arrivals.length > 0 || transportContext.korail.length > 0) && <div className="transport-live-rail" aria-live="polite">
      <div><span>가까운 정류장</span><strong>{transportContext.nearbyStops.slice(0, 3).map((item) => item.name).join(" · ") || "확인 중"}</strong></div>
      ...
      <div><span>열차</span><strong>{transportContext.korail.length ? "운행정보 확인됨" : "운행정보 확인 필요"}</strong></div>
    </div>}
  </>;
}
