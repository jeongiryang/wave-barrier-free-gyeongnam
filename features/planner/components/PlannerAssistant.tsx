...
export default function PlannerAssistant(props: Props) {
  ...
  const [starterDone, setStarterDone] = useState(() => storedNaruStarter() === 'done'), [starterOpen, setStarterOpen] = useState(() => storedNaruStarter() !== 'done'), [starterSelected, setStarterSelected] = useState<string[]>([]);
  ...
  const { books: savedTravelBooks } = useTravelBook();
  ...
  return <dialog ref={dialogRef} lang="ko" className={`naru-panel naru-${size}`} aria-label="WAVE 여행 가이드 나루와 대화" onCancel={event => { event.preventDefault(); close(); }} >
    <div className="naru-conversation">
      ...
      <div className="naru-log" ref={log} role="log" aria-live="polite" aria-relevant="additions" onScroll={() => { if (log.current) { follow.current = log.current.scrollHeight - log.current.scrollTop - log.current.clientHeight < 100; scrollPosition.current = log.current.scrollTop; } }}>
        ...
        {starterDone && !starterOpen && messages.length === 1 && <section className="naru-prompt-starters" aria-labelledby="naru-prompt-title"><h2 id="naru-prompt-title">{say(naruGuideTones.promptTitle)}</h2>{starterPrompts.map(prompt => <button type="button" key={prompt} onClick={() => { setInput(prompt); inputRef.current?.focus(); }}>{prompt}</button>)}</section>}
        ...
        {streamText && <div className="naru-message assistant" data-streaming="true">
          ...
          <button type="button" className="naru-readback" onClick={() => { if ('speechSynthesis' in window) { window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(streamText); utterance.lang = 'ko-KR'; utterance.onend = utterance.onerror = () => setSpeaking(false); setSpeaking(true); window.speechSynthesis.speak(utterance); } }}>답변 읽어주기</button>
        </div>}
      ...
      <NaruHelpHub context={hubContext} canTalk={available !== false} onOpenTool={tool => goToTool(tool)} onTalk={() => inputRef.current?.focus({ preventScroll: true })} />
