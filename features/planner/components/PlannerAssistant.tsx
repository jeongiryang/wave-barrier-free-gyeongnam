...
const toolGroups = [
  ...
  { title: '여행 중 활용', items: [['experience','페이스·감각지도·여행여권'],['audio','오디오 가이드·후기'],['coordinates','장소 좌표 복원'],['route-check','이동 구간 확인']] },
...
export default function PlannerAssistant(props: Props) {
  ...
  // Ordinary result actions still use the stricter search-aware revision above.
  const journeyRevision = JSON.stringify([plan.region, plan.themes, plan.selected, trip.voiceRevision, props.routeRevision]);
  const liveJourneyRevision = useRef(journeyRevision);
  useLayoutEffect(() => { liveJourneyRevision.current = journeyRevision; }, [journeyRevision]);
  ...
  useEffect(() => {
    ...
    // Opening on a phone should reveal the conversation, not summon the keyboard.
    const newPrompt = props.launchRequest?.prompt && props.launchRequest.id !== focusedLaunch.current;
    focusedLaunch.current = props.launchRequest?.id;
    if (media.matches && !newPrompt) dialog?.querySelector<HTMLButtonElement>('#naru-tab-conversation')?.focus({ preventScroll: true });
  ...
  async function send(value = input) {
    ...
    if (direct) {
      setInput(''); setMessages(current => [...current, { id: ++messageId.current, role: 'user', text: originalText }]);
      await apply({ id: ++messageId.current, role: 'assistant', text: '', requestText: originalText, proposal: direct, revision }); return;
    }
    ...
    try {
      ...
      if (proposal && ['create-itinerary', 'adapt-itinerary'].includes(proposal.action)) {
        ...
        if (id !== sequence.current) return;
        const draft: NaruJourney = { ...prepared, ...(prepared.restOnly ? { adjustment: gentleTripPlan({places:trip.orderedSavedPlaces,days:trip.tripDays,assignments:trip.scheduleAssignments,fixed:trip.fixedVisits,breaks:trip.breakMinutesByPlaceId,visits:trip.visitMinutesByPlaceId,origin:props.origin,targetDay:prepared.restDay,startTime:trip.dayStartTime}) } : {}), stops: prepared.stops.map(stop => stop.replaces ? { ...stop, minutes: trip.visitMinutesByPlaceId[stop.replaces] ?? stop.minutes, breakMinutes: Math.max(trip.breakMinutesByPlaceId[stop.replaces] ?? 0, stop.breakMinutes) } : stop) };
        const unchanged = draft.outcome?.kind === 'unchanged';
        append(unchanged ? '현재 장소 모두 공식 소개에서 실내 공간을 확인했어요. 기존 일정을 그대로 유지했어요.' : draft.stops.length || draft.restOnly ? '실제 관광 정보로 일정안을 준비했어요. 확인할 편의와 변경 내용을 살펴보고 적용해 주세요.' : '조건을 유지하며 찾아봤지만 바로 적용할 일정안을 만들지 못했어요. 아래에서 다음 방법을 골라주세요.', { source: 'AI 요청 · 공공 관광 데이터', draft, revision: journeyRevision });
      ...
      if (proposal && canRunConversationAction(text, proposal, reference.placeId || '', known)) {
        await apply({ id: ++messageId.current, role: 'assistant', text: '', requestText: originalText, proposal, revision }, true);
      } else if (proposal) append('이렇게 변경할까요?', { proposal, revision, requestText: originalText });
      ...
      if (proposal.action !== 'help' && canRunConversationAction(text, proposal, reference.placeId || '', known)) {
        ...
        await apply({ id: ++messageId.current, role: 'assistant', text: '', requestText: originalText, proposal, revision }, true, true);
  ...
  function applyDraft(message: Message) {
    ...
    if (message.revision !== liveJourneyRevision.current) { append('그동안 여행이 바뀌었어요. 현재 내용으로 다시 요청해 주세요.'); return; }
  ...
  async function apply(message: Message, automatic = false, navigateTool = false) {
    ...
    // Use only the user turn that produced this action, never the model's reply or an older request.
    const requested = message.requestText || '';
    const excludesIndoor = /실내(?:는|를|가|도|로)?\s*(?:말고|제외|빼|싫|아니|원하지|필요\s*없)|야외(?:로|를)?\s*(?:원해|찾아|추천)/.test(requested);
    const indoor = !isChangeNegated(requested) && !excludesIndoor && (/실내/.test(requested) || /(?:비가?\s*(?:와|오(?:니|면|는|니까)|올)|우천|rain(?:y|ing)?)/i.test(requested));
    ...
    if (action.action === 'tool' || action.action === 'save-trip') {
      ...
      committed();
      if (tool === 'alternatives' && indoor) {
        const target = trip.orderedSavedPlaces.find(item => item.id === focusedPlace.current) || trip.orderedSavedPlaces[0];
        if (target) { props.onAlternative(target.id, 'indoor'); return; }
      }
...
<a href="/guide#naru-guide" onClick={close}>사용 방법</a>{speaking && <button type="button" onClick={() => { speechSynthesis.cancel(); setSpeaking(false); }}>읽기 중단</button>}<p>대화와 사진은 AI 서버에서 처리하지만 서버에 보관하지 않아요. 저장을 누른 대화는 이 기기에 보관되며 사진 원본은 저장하지 않습니다. 사진은 위치정보를 제거한 뒤 보냅니다.</p></div></details>
      ...
      <div className="naru-log" ref={log} role="log" aria-live="polite" aria-relevant="additions" onScroll={() => { if (log.current) { follow.current = log.current.scrollHeight - log.current.scrollTop - log.current.clientHeight < 100; scrollPosition.current = log.current.scrollTop; setShowLatest(!follow.current); } }}>
        {!workRequestOpen && <div className="naru-workspace-actions" aria-label="여행의 다음 단계">
          ...
          {trip.saved.length>0 && <button type="button" disabled={busy} onClick={()=>{setReviewTrip(true); setReviewHours(false);follow.current=true;append('시간·휴식·귀가 점검 결과입니다.');}}>내 일정 점검</button>}
        </div>}
      ...
      <div role="tabpanel" id="naru-panel-tools" aria-labelledby="naru-tab-tools" hidden={workspaceTab !== 'tools'}><section hidden={workspaceTab !== 'tools'} className="naru-workspace-content" aria-label="모든 여행 도구"><h2>{toolSurfaceGroup(internalTools.request.id) ? toolLabel(internalTools.request.id) : "여행 도구"}</h2>{toolSurfaceGroup(internalTools.request.id) && <button type="button" onClick={()=>internalTools.open("")}>← 모든 여행 도구</button>}<div hidden={Boolean(toolSurfaceGroup(internalTools.request.id))} className="naru-tool-catalog"><label htmlFor="naru-tool-search">무엇을 확인할까요?</label><div className="naru-tool-search"><input id="naru-tool-search" type="search" value={toolQuery} onChange={event=>setToolQuery(event.target.value)} placeholder="막차, 휴식, 영수증…"/>{toolQuery && <button type="button" onClick={()=>setToolQuery('')}>검색 지우기</button>}</div>{toolQuery && <p role="status">{filteredTools.reduce((count,group)=>count+group.items.length,0)}개 도구{!filteredTools.length && ' · 다른 말로 찾아보거나 검색을 지워 모든 도구를 볼 수 있어요.'}</p>}<div className="naru-tools">{filteredTools.map(group => <div key={group.title}><h3>{group.title}</h3><div>{group.items.map(([id, label]) => <button key={id} type="button" onClick={() => goToTool(id)}>{label}</button>)}</div></div>)}</div><NaruHelpHub context={hubContext} canTalk={available !== false} onOpenTool={goToTool} onTalk={() => focusPrompt('')} /></div>{!trip.travelStart && internalTools.request.id && <p role="status">{trip.saved.length ? '여행 날짜를 정하면 이 도구로 일정을 준비할 수 있어요.' : '여행지를 일정에 담으면 이 도구로 여행을 준비할 수 있어요.'} <button type="button" onClick={()=>goToTool(trip.saved.length ? "dates" : plan.region ? "places" : "conditions")}>{trip.saved.length ? '날짜·출발지 정하기' : '여행지 찾기'}</button></p>}<PlannerToolSurfaces visible={props.open && workspaceTab === 'tools'}/></section></div>
