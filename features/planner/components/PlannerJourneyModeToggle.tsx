...
export default function PlannerJourneyModeToggle({ view, interactive, onChange }: PlannerJourneyModeToggleProps) {
  return <section className="journey-mode-toggle" aria-labelledby="journey-mode-title" aria-busy={!interactive}>
    ...
    <div role="group" aria-label="여행 설계 보기 방식">
      <button type="button" disabled={!interactive} aria-pressed={view === "guided"} onClick={() => onChange("guided")}><i aria-hidden="true">1</i> 한 단계씩</button>
      <button type="button" disabled={!interactive} aria-pressed={view === "overview"} onClick={() => onChange("overview")}><i aria-hidden="true">4</i> 전체 보기</button>
    </div>
  </section>;
}
