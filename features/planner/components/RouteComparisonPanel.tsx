  ...
  return <aside className="route-compare-panel">
    ...
    */}
    ...
    <div className="route-options" aria-busy={routeLoading}>
      ...
      {!routeLoading && configuredRoutes.map((item, index) => { const payment = paymentDetails(item); return <button type="button" key={item.id} className={(activeRoute?.id === item.id ? "active " : "") + "route-option"} aria-pressed={activeRoute?.id === item.id} onClick={() => setActiveRouteId(item.id)}>
        <span className="route-option-rank" aria-hidden="true">{activeRoute?.id === item.id ? "✓" : String(index + 1).padStart(2, "0")}</span><div><strong>{item.label}</strong>{modeNote(item) && <small>{modeNote(item)}</small>}{item.provider === "ODsay" && <small className="route-provider-attribution">powered by www.ODsay.com</small>}</div><dl><div><dt>예상 시간</dt><dd>{item.totalTime}분</dd></div><div><dt>{payment.label}</dt><dd>{payment.value}</dd></div><div><dt>환승</dt><dd>{`${item.transfers}회`}</dd></div><div><dt>도보</dt><dd>{`${item.totalWalk}m`}</dd></div></dl>
