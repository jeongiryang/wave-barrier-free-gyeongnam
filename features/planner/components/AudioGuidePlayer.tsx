...
export default function AudioGuidePlayer({ audio, controller }: {
  ...
  return <aside className="guide-player" aria-label={c("관광지 오디오 해설", "Place audio guide")}>
    ...
    <div className="guide-art"><span aria-hidden="true" className={playing ? "sound playing" : "sound"}><i /><i /><i /><i /><i /></span><strong lang={audio?.audioTitle ? originalLanguage(audio.audioTitle) : undefined}>{audio?.audioTitle || c("여행지 이야기를\n음성과 대본으로", "Place stories in audio and text")}</strong><small>{audio ? c("한국관광공사 오디 해설", "Korea Tourism Organization audio; the original recording may be in Korean.") : c("해설이 있는 관광지를 선택하면 연결됩니다.", "Choose a place with an audio guide to listen here.")}</small></div>
    <audio ref={audioRef} src={audio?.audioUrl || undefined} onError={controller.handleAudioError} onLoadedMetadata={handleLoadedMetadata} onPlay={handlePlay} onPause={handlePause} onEnded={handlePause} onTimeUpdate={handleTimeUpdate} />
    ...
    <button className="transcript-button" type="button" aria-expanded={transcriptOpen} onClick={toggleTranscript}>{transcriptOpen ? c("전체 대본 접기", "Hide transcript") : c("전체 대본 보기", "Show transcript")}<span>{c("청각 정보 지원", "Text alternative")}</span></button>
    {transcriptOpen && <div className="transcript" tabIndex={0} lang={audio?.script ? originalLanguage(audio.script) : undefined}>{audio?.script || c("현재 선택한 여행지의 오디 해설 대본이 없습니다. 실시간 검색 결과에서 해설이 확인되면 이곳에 전체 대본이 표시됩니다.", "No transcript was supplied for this place. Available guide text will appear here.")}</div>}
  </aside>;
}
