...
export function PlannerWeatherCard({ weather, loading, region }: { weather: WeatherData | null; loading: boolean; region: string }) {
  return <section className="reference-weather" aria-label="여행 날씨"><small>{region} 여행 날씨</small>{weather ? <><strong>{weather.current.label}<b>{weather.current.temperature}°</b></strong><WeatherVisual code={weather.current.code} /><p>강수 {weather.current.precipitation}mm · 바람 {weather.current.wind}m/s</p><span>현재 날씨 · {weather.updatedAt ? new Date(weather.updatedAt).toLocaleDateString("ko-KR") : weather.source}</span></> : <><strong>{loading && <Spinner />}{loading ? "날씨 확인 중" : "날씨 확인 필요"}</strong><p>{loading ? "여행 지역의 날씨를 불러오고 있어요." : "날씨가 도착하면 이곳에서 확인할 수 있어요."}</p></>}</section>;
}
...
export default function PlannerTripOverview({ trip, participation, coverage, origin, weather, weatherLoading, region, theme, profiles, onEdit, onMap, onDetails, onSelectPlace }: {
  ...
  return <>
    ...
    <div className="reference-overview-grid">
      ...
      <aside><PlannerWeatherCard weather={weather} loading={weatherLoading} region={region} /><section className="reference-info-card"><h3>이동 경로 요약</h3><p role="status">{coverage.loading && <Spinner />}{coverage.loading ? "이동 구간을 확인하고 있어요." : `${coverage.legs.length}개 구간 중 ${coverage.readyCount}개 확인`}</p><button type="button" onClick={onMap}>지도에서 경로 보기 →</button></section><section className="reference-info-card"><h3>접근성 정보</h3><div className="reference-tags">{profiles.map(profile => <span key={profile}>{profile}</span>)}</div><p>선택한 편의 조건입니다. 장소별 확인 결과와 미확인 항목을 확인하세요.</p><button type="button" onClick={onDetails}>날씨·접근성 정보 자세히 보기 →</button></section></aside>
    </div>
    <div className="reference-bottom-bar reference-final-actions"><div><strong>{region} · 여행지 {inPeriod.length}곳</strong><small>{canShare ? "저장한 뒤에도 일정을 다시 수정할 수 있어요." : "기간 밖 여행지의 방문 날짜를 확인해 주세요."}</small></div><button type="button" className="reference-secondary" aria-busy={participation.shareState === "saving"} disabled={!canShare || participation.shareState === "saving"} onClick={() => void participation.sharePlan()}>{participation.shareState === "saving" ? "링크 만드는 중" : "공유하기"}</button><Suspense fallback={<LoadingState>저장 기능을 준비하고 있어요.</LoadingState>}><TravelBookArchiveAction compact places={trip.orderedSavedPlaces} region={region} theme={theme} profiles={profiles} travelStart={trip.travelStart} travelEnd={trip.travelEnd} dayStartTime={trip.dayStartTime} travelMode={trip.travelMode} scheduleAssignments={trip.scheduleAssignments} visitMinutesByPlaceId={trip.visitMinutesByPlaceId} fixedVisits={trip.fixedVisits} dayDeadlines={trip.dayDeadlines} comfort={trip.comfort} breakMinutesByPlaceId={trip.breakMinutesByPlaceId} restPurposeByPlaceId={trip.restPurposeByPlaceId} /></Suspense></div>
