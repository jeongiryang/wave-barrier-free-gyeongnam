...
export default function PlaceInquiryDialog({ place, en, selected, extra, onSelection, onExtra, onClose, onHelpRequest, startCommunication = false }: {
  place: Place; en: boolean; selected: string[]; extra: string;
  onSelection: (ids: string[]) => void; onExtra: (text: string) => void; onClose: () => void; onHelpRequest?: () => void; startCommunication?: boolean;
}): ReactNode {
  ...
  const [communicating, setCommunicating] = useState(startCommunication);
  ...
  const initialTopic: CommunicationTopic = selected.includes("ordering") ? "payment" : selected.includes("stepfree") ? "step_free_entrance" : selected.includes("toilet") ? "restroom" : selected.includes("elevator") ? "elevator" : selected.includes("guidance") ? "assistance" : "assistance";
