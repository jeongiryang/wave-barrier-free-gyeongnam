...
export default function DirectPlaceSearch({ region, trip, onRegionSelect, officialPlaces = [], profiles = [], onPlace }: {
  ...
  return <section className="simple-direct-search" aria-labelledby="direct-place-search-title">
    ...
    <form className="simple-search-bar simple-direct-search-form" role="search" onSubmit={submit}>
      <label htmlFor="direct-place-query"><NightIcon name="search" size={22} /><span>여행지 검색</span><input ref={inputRef} aria-describedby={queryNotice ? "direct-place-query-notice" : undefined} id="direct-place-query" type="search" role="combobox" aria-label="여행지 검색" value={search.placeQuery} placeholder="예: 통영 케이블카, 창원 카페" autoComplete="off" aria-autocomplete="list" aria-controls="direct-place-results" aria-expanded={showResults} aria-activedescendant={active >= 0 ? `direct-result-${active}` : undefined} onChange={event => { search.setPlaceQuery(event.target.value); setQueryNotice(""); setActive(-1); }} onKeyDown={keydown} /></label>
    ...
    {queryNotice && <p id="direct-place-query-notice" role="status">{queryNotice}</p>}
    ...
    {showResults && <div id="direct-place-results" className="simple-place-list" role="list" aria-label="직접 검색 결과">
      {results.map((result, index) => result.kind === "region" ? <article ref={node => { resultNodes.current[index] = node; }} id={`direct-result-${index}`} key={result.id} className="simple-result-notice simple-direct-region" role="listitem" data-active={active === index}>
        ...
        <button type="button" className="simple-place-add" disabled={!trip.storageReady} aria-pressed={trip.saved.includes(result.place.id)} onClick={() => trip.toggleSaved(result.place.id, result.place)} aria-label={`${result.item.name} ${trip.saved.includes(result.place.id) ? "담았음 · 되돌리기" : "일정에 담기"}`}><span aria-hidden="true">{trip.saved.includes(result.place.id) ? "↶" : "+"}</span></button>
      </article>)}
    </div>}
  </section>;
}
