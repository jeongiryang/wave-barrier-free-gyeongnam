...
export function useAlternativePool({ region, themes, profiles, places, current, requiredKeys, indoor }: {
  ...
  async function search(indoor = false) {
    if (pending.current || !nearbyAlternativeRegions(region).includes(selectedRegion)) return;
    ...
    try {
      const params = new URLSearchParams({ action: "plan", region: selectedRegion, themes: requestedThemes, profiles: profiles.join(","), facilityKeys: resolveFacilityKeys({ profiles }).join(","), locale: "ko" });
      ...
      const expected = resolveFacilityKeys({ profiles }).sort();
      const actual = [...(data.criteria?.facilityKeys || [])].sort();
      if (JSON.stringify(actual) !== JSON.stringify(expected)) throw Error("Mismatched facility criteria");
  ...
  const pool = relevantExtra ? [...relevantExtra.data.places, ...(relevantExtra.data.explorationPlaces || [])] : usableCurrent ? places : [];
