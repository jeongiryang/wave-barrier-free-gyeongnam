...
export default function AccountMenu({ loginHref = "/login", initialOpen = false }: { loginHref?: string; initialOpen?: boolean }) {
  ...
  if (!session?.user) return <a ref={(element) => { entry.current = element; }} className="account-button" href={loginHref}>{en ? "Log in" : "로그인"}</a>;
  ...
  return (
    <details className="account-menu" open={initialOpen || undefined}>
      <summary ref={(element) => { entry.current = element; }} role="button" aria-label={en ? `${label} account menu` : `${label} 계정 메뉴`}><span>{label}</span><i aria-hidden="true">⌄</i></summary>
