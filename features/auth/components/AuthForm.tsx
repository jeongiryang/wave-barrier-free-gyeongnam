...
export default function AuthForm({ mode, returnTo }: { mode: AuthMode; returnTo?: string }) {
  ...
  return (
    <section className="auth-card" aria-labelledby="auth-title" aria-busy={auth.submitting}>
      ...
      <form onSubmit={auth.submit} noValidate>
        {auth.registering && <div className="auth-field"><label htmlFor="auth-name">표시 이름</label><input id="auth-name" name="name" autoComplete="name" minLength={2} maxLength={40} required aria-describedby="auth-name-help" /><small id="auth-name-help">게시글과 댓글에 표시되며 이메일은 공개하지 않습니다.</small></div>}
        ...
        <div className="auth-field"><label htmlFor="auth-password">비밀번호</label><div className="password-field"><input id="auth-password" name="password" type={auth.showPassword ? "text" : "password"} autoComplete={auth.registering ? "new-password" : "current-password"} minLength={8} maxLength={128} required aria-describedby="auth-password-help auth-message" /><button type="button" aria-pressed={auth.showPassword} onClick={auth.togglePassword}>{auth.showPassword ? "숨기기" : "보기"}</button></div><small id="auth-password-help">8자 이상 입력해 주세요.</small></div>
        {auth.registering && <div className="auth-field"><label htmlFor="auth-confirm-password">비밀번호 확인</label><input id="auth-confirm-password" name="confirmPassword" type={auth.showPassword ? "text" : "password"} autoComplete="new-password" minLength={8} maxLength={128} required aria-describedby="auth-message" /></div>}
