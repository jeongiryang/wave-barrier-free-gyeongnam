...
export default function LandingJourneyStories() {
  return <>
    <article className="product-story plan-story" data-land-reveal>
      <div className="product-story-copy"><p className="section-kicker">03 · PLAN</p><h2>관광지 목록을<br /><em>하루의 순서로.</em></h2><p>선택한 장소를 날짜에 배치하고 방문 순서를 정리합니다. 이동 경로를 확인한 뒤 일정에 반영하고, 상황이 달라지면 같은 편의조건을 유지하는 대안을 검토합니다.</p><a href="/planner#itinerary">일정 구성해 보기 <span>→</span></a></div>
      <div className="product-preview plan-preview" aria-label="여행 일정 화면 미리보기"><PreviewNote /><header><span><small>이 기기 일정</small><strong>오늘의 이동 흐름</strong></span><b>추가한 장소로 직접 구성</b></header><ol><li><i>1</i><div><small>첫 방문</small><strong>선택한 관광지</strong><p>편의시설과 운영 정보를 방문 전에 확인</p></div><span>이용 정보</span></li><li className="transfer"><i aria-hidden="true">↳</i><p>지도에서 자동차·대중교통 이동 비교</p></li><li><i>2</i><div><small>다음 방문</small><strong>동선에 맞춘 다음 장소</strong><p>이동 부담과 일정 적합성을 함께 검토</p></div><span>순서 변경</span></li></ol></div>
    </article>
