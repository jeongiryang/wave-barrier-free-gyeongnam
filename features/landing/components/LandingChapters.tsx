...
const chapters = [
  { label: "조건 고르기", title: ["가고 싶은 마음부터", "여행은 시작되니까"], copy: "경남 18개 시·군의 바다와 문화, 숲길과 산책 중 마음이 향하는 지역과 활동을 골라보세요", en: ["Start with a place", "you want to go."], enCopy: "Explore Gyeongnam’s 18 regions. Choose where to go and the activities you enjoy.", tags: ["여행 지역", "여행 날짜", "하고 싶은 활동"], enTags: ["Region", "Dates", "Activities"] },
  { label: "편의 살피기", title: ["좋아하는 곳에,", "필요한 편의까지"], copy: "접근로, 화장실, 승강기처럼 내게 필요한 편의와 장소별 공식 정보를 함께 살펴요  확인되지 않은 정보도 따로 알려드려요", en: ["The places you love.", "The facilities you need."], enCopy: "Compare paths, toilets and lifts with official information. Details that still need checking are shown separately.", tags: ["접근로·승강기", "장애인 화장실", "확인 범위"], enTags: ["Paths and lifts", "Accessible toilets", "Evidence"] },
  { label: "일정 담기", title: ["하루의 순서는,", "내가 원하는 대로"], copy: "마음에 드는 장소를 담고 날짜와 방문 순서를 정해 나에게 맞는 여행을 만들어가요", en: ["Make room for", "your kind of day."], enCopy: "Save places, arrange dates and visiting order, then review your itinerary and journeys together.", tags: ["날짜별 일정", "방문 순서", "이동 확인"], enTags: ["Daily itinerary", "Visiting order", "Journeys"] },
];
...
export default function LandingChapters({ children }: { children?: ReactNode }) {
  ...
  return <section id="story" tabIndex={-1} className="horizon-how" aria-labelledby="horizon-how-title">
    <header className="horizon-section-heading" data-land-reveal>
      ...
      <p>{en ? "Start with a landscape. Find the information and the day that work for you." : "좋아하는 풍경에서 시작해 필요한 정보와 나의 일정으로 이어져요"}</p>
    </header>
    <div className="horizon-chapters" ref={root} data-active-chapter={active}>
      ...
      <div className="horizon-chapter-stream">{chapters.map((chapter, index) => <article key={chapter.label} className="horizon-chapter-copy" data-chapter={index} data-land-reveal>
