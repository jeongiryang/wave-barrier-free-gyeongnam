...
export default function LandingHero({ t }: { t: LandingTranslate }) {
  ...
  return <section className="landing-hero" id="top">
    ...
    <div className="landing-hero-copy" data-land-reveal>
      ...
      <button className="intro-replay" type="button" onClick={() => setSequence((value) => value + 1)}>{en ? "Replay intro" : "인트로 다시보기"} <span aria-hidden="true">↻</span></button>
      {motion === "calm" && <small className="intro-motion-note">{en ? "Reduced motion is on. The introduction stays still." : "움직임 줄이기 설정에 따라 정적인 소개를 보여드립니다."}</small>}
    </div>
    ...
    <div className="landing-scroll" aria-hidden="true">{en ? "Scroll to explore" : "아래로 내려 기능을 만나보세요"} <i /></div>
  </section>;
}
