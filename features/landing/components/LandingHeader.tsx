...
export default function LandingHeader({ scrolled, t }: { scrolled: boolean; t: LandingTranslate }) {
  return <header className={scrolled ? "landing-header scrolled" : "landing-header"}>
    ...
    <nav aria-label="주요 메뉴"><a href="#story">{t("service", "서비스 소개")}</a><Link href="/planner">여행 계획</Link><Link href="/community">여행 후기</Link></nav>
    <div className="landing-header-actions"><HelpCenter /><PreferenceControls /><AccountMenu /><Link className="landing-start" href="/planner">여행 계획 만들기 <span>↗</span></Link></div>
  </header>;
}
