...
export default function LandingSectionProgress() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const probe = bounds ? bounds.top + bounds.height / 2 : innerHeight / 2;
      const photograph = [...document.querySelectorAll(".landing-hero,.horizon-chapters,.horizon-account,.landing-cta")].some(node => { const rect = node.getBoundingClientRect(); return rect.top <= probe && rect.bottom > probe; });
      setSurface(photograph ? "dark" : "light");
    };
  ...
  return <nav ref={navigation} className="story-progress" data-surface={surface} hidden={intro} aria-label={en ? "Introduction sections" : "서비스 소개 페이지 탐색"}>
