...
export default function LandingRegionStory({ t, activeRegion, active, preview, regionPhotos, setPreviewRegion, loadRegionPhoto, selectRegion }: LandingRegionStoryProps) {
  ...
  const showPreview = (region: string) => {
  ...
  return <section className="region-story" id="regions">
    ...
    <div className="landing-region-map" data-land-reveal aria-label="지역 위치를 단순화한 경상남도 18개 시·군 선택 안내도">
      ...
      <div className="landing-region-map-scroll">
        <div className="landing-region-map-canvas" data-region-map-canvas role="group" aria-label="지도 위 지역 표식">
          ...
          {landingRegions.map((region, index) => <button
            ...
            onPointerEnter={() => showPreview(region.name)}
            onPointerLeave={() => setPreviewRegion(null)}
            onFocus={() => showPreview(region.name)}
      ...
      <div className="region-mobile-list" role="list" aria-label="경상남도 지역 목록">
        {landingRegions.map((region) => <span role="listitem" key={`mobile-${region.name}`}><button
          type="button"
          className={activeRegion === region.name ? "active" : ""}
          aria-pressed={activeRegion === region.name}
          onClick={() => selectRegion(region.name)}
          onFocus={() => showPreview(region.name)}
          onBlur={() => setPreviewRegion(null)}
        ><i aria-hidden="true" /><b>{region.name}</b><small>{region.story}</small></button></span>)}
      </div>
