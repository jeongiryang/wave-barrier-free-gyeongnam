      ...
      {regions.map(region => {
        ...
        return <article key={region.name} className={`region-landscape-card ${featured ? "region-featured-card" : "region-neighbour-card"}`}>
          ...
          <div id={featured ? "region-current" : undefined} className={featured ? "region-showcase-caption selected-region" : "region-neighbour-copy"} aria-live={featured && !running ? "polite" : "off"} aria-atomic={featured ? "true" : undefined}>
            ...
            {featured ? <><strong key={active.name} tabIndex={-1} lang="ko">{scene.title}</strong><p>{english ? "A landscape for your kind of day." : region.story}</p></> : <><h3 lang="ko">{scene.title}</h3><p>{english ? "A landscape for your kind of day." : region.story}</p><button type="button" onClick={() => { choose(region.name); requestAnimationFrame(() => stage.current?.querySelector<HTMLElement>(".selected-region strong")?.focus({ preventScroll: true })); }}>{english ? `Explore ${regionLabel(region.name)}` : `${region.name} 풍경 살펴보기`} <span aria-hidden="true">↗</span></button></>}
          </div>
          {featured && <>
