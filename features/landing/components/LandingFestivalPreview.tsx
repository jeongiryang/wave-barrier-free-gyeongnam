...
export default function LandingFestivalPreview(){
 ...
 return <div ref={root} className="night-festival-preview"><div className="night-festival-preview-cards">{items.map(item=><Link href="/festivals" key={item.id}>{item.image&&<img src={item.image} alt="" width="180" height="240" loading="lazy"/>}<div className="night-festival-preview-copy"><strong>{item.name}</strong><small>{item.startDate} — {item.endDate}</small><span>{item.city}</span></div></Link>)}</div>{!items.length&&<p role="status">{state==='loading'?'개최 예정인 축제를 확인하고 있어요.':state==='error'?'축제 목록에서 최신 일정을 확인하세요.':'다가오는 경남의 축제를 만나보세요.'}</p>}</div>;
}
