...
export default function LandingNeedsDemo() {
  ...
  return <div ref={root} className="needs-demo story-demo" data-demo="facilities" data-step={step} data-running={running} data-still={still}>
    ...
    <p className="sr-only">{en ? "Select access paths and lifts, then visual information support. Your selected facilities appear together." : "접근로와 승강기, 시각 정보 지원을 차례로 고르면 선택한 편의 조건을 함께 볼 수 있어요."}</p>
  </div>;
}
