    ...
    <details className="preference-controls" inert={!replayReady} aria-busy={!replayReady} suppressHydrationWarning>
      ...
      <div className="preference-panel">
        ...
        <label className="preference-row">
          <span><b>{t("language", "언어")}</b><small>{selectedLocale.beta ? en ? "Some pages are in Korean" : "핵심 화면 부분 번역" : en ? "Full Korean support" : "한국어 전체 지원"}</small></span>
        ...
        <p>{selectedLocale.beta ? en ? "Original place information and some features may appear in Korean. " : "관광지 원문과 일부 기능은 한국어로 표시될 수 있습니다. " : ""}{en ? "Your device's reduced motion preference is followed by default." : "운영체제의 동작 줄이기 설정을 기본으로 따릅니다."}</p>
      </div>
    </details>
  );
}
