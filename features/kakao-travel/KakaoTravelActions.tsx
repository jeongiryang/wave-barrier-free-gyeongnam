...
export function KakaoTravelShare({ trip }: { trip: AccountTripPayload }) {
  ...
  const timing = useTripTimingConfirmation(tripTimingWarnings({ ...trip, places: trip.placeIds.map((id, index) => ({ id, name: `여행지 ${index + 1}` })) }), snapshot);
  ...
  return <section aria-label="카카오톡 여행 카드">
    <p>공유 카드를 준비하면 장소와 날짜를 담은 공개 링크를 만들어요. 링크를 가진 누구나 30일 동안 볼 수 있으며, 편의 조건·메모·현재 위치는 포함하지 않아요.</p>
    <div className="travel-book-actions"><button type="button" className="kakao-action-button" aria-busy={busy} disabled={busy} onClick={() => timing.request(() => void prepare())}><KakaoIcon />{busy ? "공유 카드 준비 중…" : "카카오톡 공유 카드"}</button><Link href="/guide#kakao-share">공유 사용 방법</Link></div>
