...
export default function CommunityFieldReportEditor({ values, setValues }: {
  ...
  return <section className="editor-field-report" aria-labelledby="field-report-title">
    ...
    <label className="editor-visit-date">{values.visitPhotos?.length ? "방문일 (사진 첨부 시 필수)" : "방문일 (선택)"}<input type="date" required={Boolean(values.visitPhotos?.length)} max={communityToday()} value={values.visitDate} onChange={(event) => setValues((current) => ({ ...current, visitDate: event.target.value }))} /><small>미래 방문 계획은 후기로 게시할 수 없습니다. 사진을 첨부할 때는 실제 방문일을 입력해 주세요.</small></label>
