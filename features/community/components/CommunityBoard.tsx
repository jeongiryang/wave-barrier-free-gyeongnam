...
export default function CommunityPage({initialPlace=null,fieldReportsEnabled=false}:{initialPlace?:PlaceFilter|null;fieldReportsEnabled?:boolean}) {
 ...
 <div className="night-sortbar" id="night-all-stories"><div role="group" aria-label="게시글 정렬">{[['latest','최신순'],['popular','인기순'],['comments','댓글순']].map(([key,label])=><button key={key} type="button" aria-pressed={sort===key} onClick={()=>setSort(key)}>{label}</button>)}</div><button type="button" disabled={!ready} aria-pressed={savedOnly} onClick={()=>{if(!savedOnly)board.setPlaceFilter(null);setSavedOnly(!savedOnly);}}>저장한 이야기</button><div role="group" aria-label="게시글 보기 방식"><button type="button" aria-pressed={layout==='cards'} onClick={()=>setLayout('cards')}><NightIcon name="grid" size={17}/>카드형</button><button type="button" aria-pressed={layout==='compact'} onClick={()=>setLayout('compact')}>조밀한 카드</button><button type="button" aria-pressed={layout==='list'} onClick={()=>setLayout('list')}><NightIcon name="list" size={17}/>목록형</button></div></div>
