...
export default function CommunityEditor({ postId }: { postId?: string }) {
  ...
  return (
    <form className="community-editor" onSubmit={event => { if (photoBusy) event.preventDefault(); else void submit(event); }} aria-busy={state === "saving" || photoBusy}>
      ...
      {loginRequired && <a href={`/login?next=${encodeURIComponent(currentPath)}`} target="_blank" rel="noopener noreferrer">새 탭에서 로그인하기</a>}
      <div className="editor-actions"><Link href={postId ? `/community/${postId}` : "/community"}>취소</Link><button type="submit" disabled={state === "saving" || photoBusy}>{state === "saving" ? "저장하는 중…" : editing ? "수정 내용 저장" : "후기 등록"}</button></div>
    </form>
  );
}
