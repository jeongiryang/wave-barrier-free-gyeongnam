  ...
  return <>
    <div aria-live="polite" aria-busy={state === "loading"}>
      ...
      {state === "ready" && posts.length > 0 && <div className="community-list" data-layout={layout}>{posts.map((post) => <article key={post.id} data-category={post.category}>
        <a href={`/community/${post.id}`} aria-label={`${post.title} 게시글 읽기`}><div className="community-story-cover" aria-hidden="true"><SmartSpotImage title={post.placeName || regionShowcasePhotos[post.region || '통영']?.title || '경남 여행'} region={post.region || '경남'} src={post.placeId ? undefined : regionShowcasePhotos[post.region || '통영']?.image} contentId={post.placeId || undefined} tag={COMMUNITY_CATEGORY_LABELS[post.category]} rank={0} showMeta={false}/></div><div className="community-card-meta"><span className={`category-${post.category}`}>{COMMUNITY_CATEGORY_LABELS[post.category]}</span><time dateTime={new Date(post.createdAt).toISOString()}>{communityDate(post.createdAt)}</time></div><h3>{post.title}</h3><p>{post.content}</p>{(post.region || post.placeName) && <div className="community-place-tag"><span aria-hidden="true">⌖</span>{post.region}{post.placeName ? `${post.region ? " · " : ""}${post.placeName}` : ""}</div>}{(post.fieldReports?.length || 0) > 0 && <div className="community-report-tag">여행자 현장 제보 {post.fieldReports.length}개 · 공식 점수 미반영</div>}{(post.photoCount || 0) > 0 && <div className="community-report-tag">현장 사진 {post.photoCount}장 · 방문 {post.visitDate}</div>}<footer><span>{post.authorName}</span><span>좋아요 {post.likeCount} · 댓글 {post.commentCount}</span></footer></a>
      </article>)}</div>}
    </div>
