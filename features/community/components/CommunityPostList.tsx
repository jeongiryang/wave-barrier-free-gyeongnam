...
export default function CommunityPostList({ board }: { board: ReturnType<typeof useCommunityBoard> }) {
  ...
  return <>
    <div aria-live="polite" aria-busy={state === "loading"}>
      ...
      {state === "ready" && posts.length === 0 && <div className="community-state empty"><span aria-hidden="true">≈</span><b>{placeFilter ? "이 관광지의 첫 후기를 기다리고 있어요." : "아직 등록된 후기나 질문이 없습니다."}</b><p>직접 경험한 내용이나 궁금한 점을 가장 먼저 남겨 주세요. 예시 글을 실제 경험처럼 표시하지 않습니다.</p><a href={writeHref}>후기 작성</a></div>}
