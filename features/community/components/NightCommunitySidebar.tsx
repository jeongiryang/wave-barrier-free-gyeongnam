...
export default function NightCommunitySidebar({onRegion}:{onRegion:(name:string)=>void}) {
 ...
 <section><h2><NightIcon name="map"/>여행 준비 가이드</h2><p>처음이신가요? 여행 전 꼭 확인해보세요.</p>{[['access','장애인 이용 시설 찾기','/planner'],['bus','교통편 이용 가이드','/guide#transport'],['bed','숙박 시설 가이드','/planner?theme=stay'],['shield','여행 시 유용한 팁','/guide']].map(([icon,text,href])=><a key={text} className="night-guide-row" href={href}><NightIcon name={icon}/>{text}<span>›</span></a>)}</section>
