...
export default function CommunityDetail({ postId }: { postId: string }) {
  ...
  if (state === "error" || !post) return <div className="community-detail-state" role="alert"><b>게시글을 열지 못했습니다.</b><p>{message}</p><button type="button" onClick={() => void load()}>다시 시도</button><Link href="/community">목록으로 돌아가기</Link></div>;
