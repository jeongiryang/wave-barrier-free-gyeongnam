...
export default function LandingCommunityStory() {
  ...
  return <section className="landing-community community-chapter" id="community" tabIndex={-1} data-cinematic="left" aria-labelledby="community-story-title">
    <figure className="community-composer-visual" aria-labelledby="community-visual-caption">
      <div ref={root} className="community-demo story-demo" data-demo="community" data-step={step} data-running={running}>
        <div className="community-editor-preview" aria-hidden="true">
          <div className="community-preview-masthead"><span>W.A.V.E</span><span>{en ? "Writing example" : "작성 예시"}</span></div>
          <p className="section-kicker">{en ? "Write a question" : COMMUNITY_CATEGORY_LABELS.general}</p>
          <h3>{en ? "Before your trip." : "떠나기 전, 궁금한 점을."}</h3>
          ...
          <article className="demo-post-preview" data-shown={step >= 3}><small>{en ? "How the example reads · not published" : "작성 예시의 게시글 형태 · 미게시"}</small><h4>{title}</h4><p>{content}</p></article>
        </div>
        ...
        <div className="demo-foot"><small>{en ? "Title → content → example post" : "제목 → 내용 → 작성한 글"}</small><button className="story-motion-toggle" type="button" aria-label={still ? en ? "Static writing demonstration" : "작성 시연 정적 예시" : completed ? en ? "Replay writing demonstration" : "작성 시연 다시 보기" : en ? "Pause writing demonstration" : "작성 시연 멈추기"} aria-disabled={still} aria-pressed={completed || still ? undefined : paused} onClick={toggle}><span aria-hidden="true">{paused || completed || still ? "▶" : "Ⅱ"}</span>{still ? en ? "Still example" : "정적 예시" : completed ? en ? "Replay" : "다시 보기" : paused ? en ? "Paused" : "멈춤" : en ? "Pause" : "멈추기"}</button></div>
      </div>
      <figcaption id="community-visual-caption">{en ? "W.A.V.E question form · writing example, not published" : "W.A.V.E 질문 작성 예시 · 실제 게시된 글이 아닙니다"}</figcaption>
    </figure>
