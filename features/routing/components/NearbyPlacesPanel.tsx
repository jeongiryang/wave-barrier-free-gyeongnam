...
export default function NearbyPlacesPanel({ available, loading, activeCategory, categoryMessage, categoryPlaces, categoryState, onClose, onSearch, onChoosePlace, onRetry }: NearbyPlacesPanelProps) {
  ...
  return <section id="map-panel-nearby" className="map-tool-panel map-side-drawer map-nearby-panel" aria-label={english ? "Find nearby places" : "주변 장소 찾기"} tabIndex={-1} onFocusCapture={(event) => {
    ...
    {(categoryMessage || categoryPlaces.length > 0) && <div className="map-poi-results">
      ...
      <button type="button" aria-disabled={!available || categoryState === "loading"} onClick={() => { if (available && categoryState !== "loading") onRetry(); }}>{english ? "Search nearby again" : "주변 장소 다시 검색"}</button>
