...
export default function RoutePointPanel({ origin, pickMode, onClose, onCurrentLocation, onSelectMode }: RoutePointPanelProps) {
  return <section className="map-tool-panel map-side-drawer map-route-panel" aria-label="출발지와 목적지 설정">
    ...
    <div className="map-route-actions">
      ...
      <button type="button" className={pickMode === "origin" ? "active" : ""} onClick={() => onSelectMode("origin")}><i>S</i><strong>지도에서 출발지 선택</strong><small>지도 위 원하는 지점을 클릭</small></button>
      <button type="button" className={pickMode === "destination" ? "active" : ""} onClick={() => onSelectMode("destination")}><i>G</i><strong>지도에서 목적지 선택</strong><small>관광지 마커도 바로 선택 가능</small></button>
    </div>
