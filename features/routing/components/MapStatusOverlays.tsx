...
import { mapCrowdText } from "../map-status-copy";
...
export function MapCanvasStatusOverlays({ provider, roadviewOpen, roadviewMessage, roadviewLoading, onRetryRoadview, roadviewRef, crowd, crowdPlace, crowdVisual, onCloseRoadview }: MapCanvasStatusOverlaysProps) {
  ...
  return <>
    ...
    {crowdVisual && crowdText && crowd && crowdPlace && !roadviewOpen && <aside className={`map-crowd-legend crowd-${crowdVisual.level}`} style={{ "--crowd-color": crowdVisual.color, "--crowd-soft": crowdVisual.soft } as CSSProperties} aria-label={english ? `${crowdPlace.name} crowd forecast: ${crowdText.label}` : `${crowdPlace.name} 혼잡 예측 ${crowdText.label}`}>
      <span className="crowd-visual"><i /></span>
      <div><small>{english ? "30-day crowd forecast" : "30일 혼잡 예측"} · {crowdPlace.name}</small><strong>{crowdText.label}</strong><p>{crowdText.message}</p></div>
      <em>{crowd.rate.toFixed(1)}%</em>
    </aside>}
    {provider === "loading" && <div className="map-loading-skeleton" role="status" aria-label={english ? "Connecting map" : "지도 연결 중"}><div><i /><i /><i /><span /></div><p><b />{english ? "Connecting to Kakao Maps." : "카카오 지도를 안전하게 연결하고 있습니다."}</p></div>}
