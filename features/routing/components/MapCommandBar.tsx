...
export default function MapCommandBar({
  ...
  return <>
    <div className={`map-provider-badge ${provider}${layerRecovery ? " map-layer-status" : ""}`} role="status" aria-live="polite" aria-atomic="true">
      ...
      <strong style={actionNotice || layerRecovery ? { whiteSpace: "normal" } : undefined}>{statusMessage}</strong>
    ...
    <nav className="map-command-bar" aria-label="지도 기능">
      <div className="map-command-scroll">
        ...
        <button type="button" aria-expanded={toolPanel === "route"} aria-controls="map-panel-route" className={toolPanel === "route" ? "active" : ""} onClick={(event) => togglePanel("route", event.currentTarget)} disabled={!interactive || provider === "error"}>⇄ 출발·도착</button>
