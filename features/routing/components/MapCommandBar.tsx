  ...
  return <>
    ...
    <nav className="map-command-bar" aria-label="지도 기능">
      <div className="map-command-scroll">
        ...
        <button type="button" aria-expanded={toolPanel === "layers"} aria-controls="map-panel-layers" className={toolPanel === "layers" ? "active" : ""} onClick={(event) => togglePanel("layers", event.currentTarget)} disabled={!interactive || provider !== "kakao"}>▱ 레이어·측정</button>
