    ...
    <nav className="map-command-bar" aria-label="지도 기능">
      <div className="map-command-scroll">
        <div className="map-type-switch" aria-label="지도 유형">
          <button type="button" aria-pressed={baseMap === "roadmap"} className={baseMap === "roadmap" ? "active" : ""} onClick={() => onBaseMapChange("roadmap")} disabled={provider !== "kakao"}>지도</button>
          <button type="button" aria-pressed={baseMap === "skyview"} className={baseMap === "skyview" ? "active" : ""} onClick={() => onBaseMapChange("skyview")} disabled={provider !== "kakao"}>스카이뷰</button>
        </div>
        <button type="button" aria-expanded={toolPanel === "nearby"} aria-controls="map-panel-nearby" className={toolPanel === "nearby" ? "active" : ""} onClick={(event) => togglePanel("nearby", event.currentTarget)} disabled={provider !== "kakao"}>⌖ 주변</button>
        <button type="button" aria-expanded={toolPanel === "route"} aria-controls="map-panel-route" className={toolPanel === "route" ? "active" : ""} onClick={(event) => togglePanel("route", event.currentTarget)}>⇄ 출발·도착</button>
        <button type="button" aria-expanded={toolPanel === "layers"} aria-controls="map-panel-layers" className={toolPanel === "layers" ? "active" : ""} onClick={(event) => togglePanel("layers", event.currentTarget)} disabled={provider !== "kakao"}>▱ 레이어·측정</button>
        ...
        <button type="button" aria-expanded={toolPanel === "export"} aria-controls="map-panel-export" className={toolPanel === "export" ? "active" : ""} onClick={(event) => togglePanel("export", event.currentTarget)}>⇩ 이미지</button>
        <button type="button" onClick={onShare}>↗ 공유</button>
      </div>
      <button type="button" className="map-expand-button" aria-pressed={expanded} aria-controls="route-map-canvas" onClick={(event) => onToggleExpanded(event.currentTarget)}>{expanded ? "× 닫기" : "⛶ 전체보기"}</button>
    </nav>
  </>;
}
