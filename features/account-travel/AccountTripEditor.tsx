...
import TripBreakControl, { TripBreakSummary } from "../planner/components/TripBreakControl";
    ...
    {!trip || !draft ? <section className="travel-book-empty"><h2>{notice || "내 여행을 불러오고 있어요."}</h2><button type="button" onClick={() => window.location.reload()}>다시 불러오기</button></section> : <>
      <section className="account-settings" aria-labelledby="trip-edit-title"><h2 id="trip-edit-title">{trip.payload.title}</h2><p>{owner ? "내가 만든 여행 · 일정 편집과 동행자 초대" : "함께하는 여행 · 장소 투표와 의견 나누기"}</p>
        {owner && <><div className="auth-field"><label htmlFor="trip-title">여행 이름</label><input id="trip-title" value={draft.title} onChange={event => change({ title: event.target.value })} maxLength={80} /></div>
          <div className="account-trip-dates"><div className="auth-field"><label htmlFor="trip-start">시작 날짜</label><AccessibleDateInput id="trip-start" value={draft.travelStart} onChange={event => change({ travelStart: event.target.value })} /></div><div className="auth-field"><label htmlFor="trip-end">마지막 날짜</label><AccessibleDateInput id="trip-end" value={draft.travelEnd} onChange={event => change({ travelEnd: event.target.value })} /></div><div className="auth-field"><label htmlFor="trip-time">하루 시작 시간</label><input id="trip-time" type="time" value={draft.dayStartTime} onChange={event => change({ dayStartTime: event.target.value })} /></div></div>
        ...
        <OpenTripInPlanner revision={trip.revision} role={trip.role} id={trip.id} payload={draft} />
        ...
        <div className="travel-book-days">{draft.placeIds.map((placeId, index) => {
          ...
          return <section key={placeId}><header><strong>{index + 1}. {place?.name || `저장한 여행지 ${index + 1}`}</strong><small>{trip.votes.filter(vote => vote.placeId === placeId).length}명 선택</small></header>
            ...
            {owner && <TripBreakControl name={place?.name || `여행지 ${index + 1}`} value={draft.breakMinutesByPlaceId?.[placeId]} purpose={draft.restPurposeByPlaceId?.[placeId]} onChange={value => { const next = { ...draft.breakMinutesByPlaceId }; if (value) next[placeId] = value; else delete next[placeId]; change({ breakMinutesByPlaceId: sanitizeTripBreaks(next, draft.placeIds) }); }} onPurpose={value => { const next = { ...draft.restPurposeByPlaceId }; if (value) next[placeId] = value; else delete next[placeId]; change({ restPurposeByPlaceId: sanitizeStopPurposes(next, draft.placeIds) }); }} />}<TripBreakSummary minutes={draft.breakMinutesByPlaceId?.[placeId]} purpose={draft.restPurposeByPlaceId?.[placeId]} />
