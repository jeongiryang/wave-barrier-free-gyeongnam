...
export default function CloudSaveAction({ book }: { book: TravelBook }) {
  ...
  const key = JSON.stringify([userId, book.title, book.region, book.travelStart, book.travelEnd, book.dayStartTime, book.themes, book.theme, book.status, book.note, book.places.map(place => place.id), book.scheduleAssignments, book.visitMinutesByPlaceId, book.fixedVisits, book.dayDeadlines, book.breakMinutesByPlaceId, book.restPurposeByPlaceId]);
  async function save() {
