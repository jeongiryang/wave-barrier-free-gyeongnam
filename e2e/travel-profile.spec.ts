...
test("여행 프로필은 저장 뒤에도 자동 적용하지 않고 사용자가 선택해 적용·삭제한다", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /휠체어 이용/ }).click();
  await page.getByRole("button", { name: /청각 정보 지원/ }).click();
  ...
  await expect(page.getByRole("button", { name: /청각 정보 지원/ })).toHaveAttribute("aria-pressed", "false");
  ...
  await expect(page.getByRole("button", { name: /휠체어 이용/ })).toHaveAttribute("aria-pressed", "false");
  await expect(page.getByRole("button", { name: /청각 정보 지원/ })).toHaveAttribute("aria-pressed", "true");
