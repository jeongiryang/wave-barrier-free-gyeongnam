...
const plan={...alternativePlan,explorationPlaces:[{...alternativePlan.places[2],id:'1010',name:'편의 미확인 공원',accessibility:alternativePlan.places[2].accessibility?.map(field=>({...field,state:'unknown' as const}))}]};
async function setup(page:Page){await mockPublicShellApi(page);await mockPlannerApi(page,{preserveView:true,savedPlaces:[...plan.places,...plan.explorationPlaces]});await page.emulateMedia({reducedMotion:'reduce'});await page.route('**/api/wave?action=plan*',route=>route.fulfill({json:plan}));}

async function tripState(page:Page){return page.evaluate(()=>{
...
test('a short outing has explicit search, time limits, separate save and retained main itinerary',async({page},info)=>{
 await setup(page);await page.addInitScript(()=>{localStorage.setItem('wave-saved-places','["2758443"]');localStorage.setItem('wave-trip-schedule-v1',JSON.stringify({travelStart:'2026-10-02',travelEnd:'2026-10-03',dayStartTime:'09:30',scheduleAssignments:{'2758443':'2026-10-02'}}));});
 let calls=0;await page.route('**/api/wave?action=plan*',route=>{calls++;return route.fulfill({json:plan});});
 await page.goto('/outings');const facility=page.getByRole('button',{name:'접근로',exact:true});await expect(facility).toHaveAttribute('aria-pressed','false');await facility.click();await expect(page.getByRole('button',{name:'✓ 접근로',exact:true})).toHaveAttribute('aria-pressed','true');expect(calls).toBe(0);const before=await page.evaluate(()=>localStorage.getItem('wave-trip-schedule-v1'));
 await page.getByLabel('여행 날짜',{exact:true}).fill('2026-10-08');await page.getByRole('button',{name:'이 시간에 나들이 찾기',exact:true}).click();await expect(page.getByRole('heading',{name:'잠깐 머물고 싶은 곳',exact:true})).toBeFocused();expect(calls).toBe(1);
...
test('outing provider errors, changed preferences and blocked storage keep truthful state',async({page})=>{
 ...
 await page.route('**/api/wave?action=plan*',route=>route.fulfill({status:502,json:{error:'Unavailable'}}));await page.getByRole('button',{name:'이 시간에 나들이 찾기',exact:true}).click();await expect(page.getByRole('status')).toContainText('여행 후보를 불러오지 못했어요');await expect(page.getByRole('region',{name:'짧은 나들이 장소 고르기'})).toHaveCount(0);
 await page.route('**/api/wave?action=plan*',route=>route.fulfill({json:{...plan,statuses:[{id:'photo',name:'사진',role:'사진',state:'error',count:0,note:'Unavailable'}]}}));await page.getByRole('button',{name:'이 시간에 나들이 찾기',exact:true}).click();
 const choices=page.getByRole('region',{name:'짧은 나들이 장소 고르기',exact:true});await expect(choices.getByRole('status')).toContainText('일부 정보를 불러오지 못했어요');await expect(choices.getByRole('status')).toBeVisible();
