...
const basePost = {
  category: "review", content: "합성 데모 예시입니다. 짐을 줄여 보니 여행이 한결 편했어요.", authorName: "데모 여행자 01",
...
const demoPost = { ...basePost, id: "demo-post", title: "짐을 가볍게 꾸린 날", demoBatchId: "wave-community-demo-2026-v1" };
...
const comments = [
  { id: "demo-comment", content: "합성 데모 댓글입니다. 작은 가방이 편해 보여요.", authorName: "데모 답글 01", createdAt: demoPost.createdAt + 1_000, updatedAt: demoPost.createdAt + 1_000, isOwner: false, demoBatchId: demoPost.demoBatchId },
...
test("synthetic badges remain visible in list, detail and comments while ordinary content stays unlabelled", async ({ page }) => {
  await mockPublicShellApi(page);
  await page.route("**/api/auth/get-session", (route) => route.fulfill({ json: null }));
  await page.route("**/api/community/posts**", (route) => {
    const path = new URL(route.request().url()).pathname;
    return route.fulfill({ json: path === "/api/community/posts/demo-post" ? { post: demoPost, comments } : { posts: [demoPost, realPost], page: 1, hasMore: false } });
  });
  ...
  await expect(demoComment.locator(".community-demo-label")).toHaveText("합성 데모 댓글");
