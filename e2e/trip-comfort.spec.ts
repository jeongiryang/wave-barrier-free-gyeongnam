...
test("walking evidence, planned rests and nearby restroom stops stay integrated with saved schedules",async({page},info)=>{
  ...
  const board=page.locator('.simple-timeboard');const chat=await openNaruTool(page,'이동 부담·휴식'),comfort=chat.locator('.trip-comfort-plan'),finder=chat.locator('.rest-stop-finder');await comfort.locator('summary').click();
  ...
  await closeNaruTool(page);await timetable(page);await board.getByLabel('가까운 문화쉼터 일정 수정',{exact:true}).click();const editor=page.getByRole('dialog',{name:'가까운 문화쉼터 수정',exact:true});await editor.locator('.simple-stop-options > summary').click();await expect(editor.getByLabel('가까운 문화쉼터 방문 목적',{exact:true})).toHaveValue('restroom');await editor.getByLabel('가까운 문화쉼터 방문 뒤 휴식',{exact:true}).selectOption('5');await page.screenshot({path:info.outputPath('rest-menu.png')});await editor.getByRole('button',{name:'적용',exact:true}).click();
  ...
  { const create = page.getByRole('button', { name: '공개 링크 만들기', exact: true }); if (await create.isVisible() && await create.isEnabled()) { await create.click(); await acceptTripTimingWarning(page); } }const menu=page.getByRole('dialog',{name:'여행 공유',exact:true});await expect(menu.getByRole('link',{name:'공유 일정 보기',exact:true})).toBeVisible();expect(shares[0].breakMinutesByPlaceId).toEqual({'1001':15,'1003':5});expect(shares[0].restPurposeByPlaceId).toEqual({'1003':'restroom'});expect(shares[0].comfort).toBeUndefined();expect(shares[0].profiles).toEqual([]);await menu.getByRole('button',{name:'공유 닫기',exact:true}).click();
