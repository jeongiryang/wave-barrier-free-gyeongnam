...
for (const theme of ["light", "dark"] as const) test(`clipboard failure preserves a usable live link and an accessible retry in ${theme}`, async ({ page }) => {
  ...
  const copyFailure = panel.getByRole("status").filter({ hasText: /복사하지 못|직접 복사/ });
  await expect(copyFailure).toBeVisible();
  await expect(panel.getByRole("status").filter({ hasText: "현재 공개 중 · 일정 변경 자동 반영" })).toBeVisible();
  await expect(button).toBeFocused();
  await expect(copyFailure).not.toContainText("복사했어요");
  await expect(panel.getByRole("status").filter({ hasText: "공유 링크를 복사했어요" })).toHaveCount(0);
  await expect(panel.getByRole("link", { name: "공유 일정 보기", exact: true })).toHaveAttribute("href", url);
  ...
  await expect(panel.getByRole("status").filter({ hasText: "공유 링크를 복사했어요" })).toBeVisible();
