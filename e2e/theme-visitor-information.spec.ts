  ...
  test(`${mode} candidate keeps its description and opens same-place visitor information on demand`, async ({ page }, testInfo) => {
    ...
    await page.route('**/api/wave?*', route => {
      ...
      return route.fulfill({ json: {
        ...
        hours: '운영시간은 예약 프로그램에 따라 다름', restDays: '방문 전 문의', fees: '', phone: '055-123-4567',
      } });
    });
    ...
    await expect(hours.getByRole('link', { name: '055-123-4567', exact: true })).toHaveAttribute('href', 'tel:0551234567');
    ...
    await page.keyboard.press('Enter');
    await expect(hours.getByRole('link', { name: '055-123-4567', exact: true })).toBeVisible();
