...
test('device bookmarks agree between list and detail, survive reload and can be found and removed without publishing', async ({ page }) => {
  await setup(page);
  const writes: string[] = [];
  ...
  const axe = await new AxeBuilder({ page }).include('#community-saved-posts').analyze();
...
test('unavailable posts and provider failure keep the bookmark until explicit removal, retry reads fresh content', async ({ page }) => {
  await setup(page);
...
test('blocked storage never claims save success and a retry recovers without losing existing IDs', async ({ page }) => {
  await setup(page);
  await page.goto('/community');
  const card = page.locator('.community-list article').filter({ hasText: post.title });
