...
for (const locale of ["ko", "en"] as const) {
  ...
  test(`landing: ${locale} restored named sections explain places, itinerary and Naru in reading order`, async ({ page }) => {
    ...
    const names = en
      ? [/Explore Gyeongnam\s*Plan your trip/, /A journey that feels distant,\s*a little closer to you\./, /^WAVE로 할 수 있는 일$/, /^Explore Gyeongnam$/, /^Plan with Naru$/, /A lighter heart\.\s*One more check\./, /당신이 남긴 장면이\s*다음 여행의 시작/, /See you at\s*the next horizon\./]
      : [/경남 여행지를 찾고\s*일정을 짜보세요/, /멀게 느껴졌던 여행을,?\s*조금 더 가까이/, /^WAVE로 할 수 있는 일$/, /^지역으로 둘러보기$/, /^나루에게 말해보세요$/, /마음은 가볍게\s*준비는 한 번 더/, /당신이 남긴 장면이\s*다음 여행의 시작/, /다음 풍경에서\s*만나요/];
