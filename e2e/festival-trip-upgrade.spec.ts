...
test('축제 현장 정보는 쉬는 곳과 화장실 예시만 실제 지도에 표시한다', async ({ page }) => {
  ...
  await card.locator('.place-evidence > summary').click();
  const map = card.getByTestId('festival-amenity-map');
  await expect(map).toBeVisible();
  await expect(card.getByRole('button', { name: '쉬는 곳', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await expect(card.locator('[data-amenity-marker="rest"]')).toHaveCount(3);
  await expect(card.getByRole('button', { name: '쉬는 곳 입구 벤치', exact: true })).toBeVisible();

  await card.getByRole('button', { name: '화장실', exact: true }).click();
  await expect(card.getByRole('button', { name: '화장실', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await expect(card.locator('[data-amenity-marker="restroom"]')).toHaveCount(3);
  await expect(card.getByRole('button', { name: '화장실 컨테이너 화장실', exact: true })).toBeVisible();
  await expect(card.getByText('임의의 데이터를 사용하여 표시한 마크입니다. 실제 지도를 확인해 주세요.', { exact: true })).toBeVisible();
  await card.getByRole('button', { name: '쉬는 곳', exact: true }).focus();
  await expect(card.getByRole('button', { name: '쉬는 곳', exact: true })).toBeFocused();

  for (const removed of ['소리', '혼잡', '휠체어 이동', '빛', '현장 정보 새로 확인', '내 근처 일정 장소 찾기']) {
    await expect(card.getByRole('button', { name: removed, exact: true })).toHaveCount(0);
  }
  await expect(card.getByText('공식정보 재확인 목록', { exact: false })).toHaveCount(0);
  await expect(card.getByText('지금 이 장소에서 직접 본 정보 공유', { exact: true })).toHaveCount(0);
  expect((await new AxeBuilder({ page }).include('.place-evidence').analyze()).violations).toEqual([]);
