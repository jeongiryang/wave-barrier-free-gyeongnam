...
test("first visit stays neutral and later stages are locked without a search", async ({ page }) => {
  ...
  await expect(page.locator(".journey-rail").getByRole("progressbar")).toHaveAttribute("aria-valuenow", "0");
...
test("one saved place does not complete the trip and the dialog contains keyboard focus", async ({ page }) => {
  ...
  await expect(page.locator(".journey-rail").getByRole("progressbar")).toHaveAttribute("aria-valuenow", "50");
});
