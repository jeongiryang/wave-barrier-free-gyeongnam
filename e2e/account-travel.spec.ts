...
test("signed-out account travel remains private and help explains real entry points", async ({ page }) => {
  ...
  await expect(savedTravelGuide.locator("summary")).toHaveText("내 여행 저장과 이어하기");
  await page.getByRole("navigation", { name: "시작할 도움말" }).getByRole("link", { name: "저장과 공유", exact: true }).click();
  await expect(savedTravelGuide.locator("details")).toHaveAttribute("open", "");
  await expect(savedTravelGuide.getByRole("link", { name: "이 기기에 저장한 여행", exact: true })).toBeVisible();
  await expect(savedTravelGuide.getByRole("link", { name: "이 기기에 저장한 여행", exact: true })).toHaveAttribute("href", "/travel-book");
  await expect(savedTravelGuide.getByRole("link", { name: "계정에 저장한 여행", exact: true })).toBeVisible();
  await expect(savedTravelGuide.getByRole("link", { name: "계정에 저장한 여행", exact: true })).toHaveAttribute("href", "/my-trips");
  const companionsGuide = page.locator("#companions");
  await expect(companionsGuide.locator("summary")).toHaveText("동행자 초대");
  await companionsGuide.locator("summary").click();
  await expect(companionsGuide.locator("details")).toHaveAttribute("open", "");
  await expect(companionsGuide).toContainText("동행자는 로그인하고 표시 이름을 입력한 뒤 여행에 참여하기를 누릅니다.");
  expect(privateRequests).toBe(0);
  expect((await new AxeBuilder({ page }).include("#account-travel").analyze()).violations).toEqual([]);
});
