...
test('split/reunion saves an optional A/B plan, keeps original trip and exports explicit offline lines',async({page},info)=>{
 ...
 await page.reload();await openNaruTool(page, '이동 구간 확인');await page.getByRole('button',{name:'동행과 합류 계획',exact:true}).click();await expect(tool.getByRole('combobox',{name:'B 출발 장소에서 더 머무는 시간',exact:true})).toHaveValue('60');
});
test('late plans, storage conflicts and changed original schedules never claim a valid saved plan',async({page})=>{
 ...
 await closeNaruTool(page);await page.getByRole('button',{name:'용지호수공원 일정 수정',exact:true}).click();
 ...
 await page.reload();await openNaruTool(page, '이동 구간 확인');await page.getByRole('button',{name:'동행과 합류 계획',exact:true}).click();await expect(tool.getByRole('button',{name:'바뀐 일정으로 다시 계획',exact:true})).toBeVisible();await expect(tool.getByRole('button',{name:'합류 약속 저장',exact:true})).toHaveCount(0);
