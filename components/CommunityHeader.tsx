...
export default function CommunityHeader({ current = "community" }: { current?: "community" | "planner" }) {
  return (
    <header className="community-header">
      ...
      <nav aria-label="주요 메뉴"><Link href="/">서비스 소개</Link><Link href="/planner" aria-current={current === "planner" ? "page" : undefined}>여행 계획</Link><Link href="/community" aria-current={current === "community" ? "page" : undefined}>여행 후기</Link></nav>
