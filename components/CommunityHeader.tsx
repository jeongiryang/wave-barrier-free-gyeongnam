...
export default function CommunityHeader({ current = "community" }: { current?: "community" | "planner" | "travel-book" }) {
  return (
    <header className="community-header">
      ...
      <nav aria-label="주요 메뉴"><Link href="/">서비스 소개</Link><Link href="/planner" aria-current={current === "planner" ? "page" : undefined}>여행 계획</Link><Link href="/travel-book" aria-current={current === "travel-book" ? "page" : undefined}>내 일정</Link><Link href="/community" aria-current={current === "community" ? "page" : undefined}>여행 후기</Link></nav>
      <div className="community-header-actions"><PublicMobileNav links={[
        { href: "/", label: "서비스 소개" },
        { href: "/planner", label: "여행 계획", current: current === "planner" },
        { href: "/travel-book", label: "내 일정", current: current === "travel-book" },
