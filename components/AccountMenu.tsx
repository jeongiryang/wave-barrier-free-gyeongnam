...
export default function AccountMenu({ loginHref = "/login" }: { loginHref?: string }) {
  ...
  if (isPending) {
    return <span className="account-loading" role="status" aria-label="계정 상태를 불러오는 중"><i /></span>;
  }

  if (!session?.user) {
    return <a className="account-button" href={loginHref}>로그인</a>;
  }
