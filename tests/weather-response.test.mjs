...
const valid = () => ({ region: "창원", source: "Open-Meteo", updatedAt: "2026-09-06T00:00:00Z", current: { temperature: 0, apparent: -2, code: 0, label: "맑음", windMps: 0, precipitation: 0, isDay: true }, days: [{ date: "2026-09-06", code: 0, label: "맑음", min: -2, max: 3, rainProbability: 0, rain: 0, snow: 0, uv: 0, advice: [] }], advice: [] });

test("weather for another region cannot become evidence for the requested destination", () => {
  assert.equal(weatherResponse(valid(), "진주"), null);
  const data = valid();
  assert.equal(weatherResponse(data, "창원"), data);
  assert.equal(weatherResponse({ ...data, region: "경남 전체" }, "경남 전체").region, "경남 전체");
});
...
test("malformed weather is unavailable rather than crashing the planner or fabricating clear weather", () => {
  ...
  for (const field of ["temperature", "apparent", "code", "windMps", "precipitation"]) {
