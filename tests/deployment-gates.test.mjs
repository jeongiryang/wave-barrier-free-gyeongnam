...
test("fresh databases receive the complete idempotent migration chain in one transaction", async () => {
  ...
  assert.deepEqual(sources.map((source) => splitMigrationStatements(source).length), [9, 9, 5, 1, 4, 1, 2, 1, 9, 2, 5, 6, 3, 1, 2]);
  ...
  assert.doesNotMatch(sources[12], /DROP|DELETE|UPDATE/);
