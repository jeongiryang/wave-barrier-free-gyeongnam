...
test('invalid requests, missing key, unrelated stop and unconfirmed location do not fan out',async()=>{
 ...
 const legacy=harness({commonItems:[{...common,lDongRegnCd:undefined,areacode:'36'}]});assert.equal((await legacy.run()).body.status,'stops');assert.equal(legacy.calls.length,2);
