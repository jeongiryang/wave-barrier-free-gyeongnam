...
test("account trip storage has an explicit field allowlist and rejects corrupt dates/IDs", () => {
  ...
  assert.deepEqual(Object.keys(safe).sort(), ["version", "title", "region", "travelStart", "travelEnd", "dayStartTime", "travelMode", "themes", "profiles", "guidancePreferences", "comfort", "placeIds", "scheduleAssignments", "visitMinutesByPlaceId", "breakMinutesByPlaceId", "restPurposeByPlaceId", "fixedVisits", "dayDeadlines", "status", "note"].sort());
  assert.deepEqual(safe.profiles, ['wheelchair']);
  assert.deepEqual(accountTripPayload({ ...payload, profiles: ['wheel'] }).profiles, ['parking', 'route', 'wheelchair', 'elevator', 'restroom']);
