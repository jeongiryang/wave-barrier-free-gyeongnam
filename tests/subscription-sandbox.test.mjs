...
import { collectValidationShards, sandboxCall, sandboxConfiguration, wslPath } from "../scripts/subscription-sandbox.mjs";
...
const shardReceipt = shard => ({ result: "PASS", boundary: "linux-bwrap-v1", network: "isolated", filesystem: "isolated", shard, checks: ["outside-files", "home-appdata-userprofile", "external-network", "local-network", "host-mounts", "npm run lint", "npm run typecheck", "npm test", "npm run build:vercel", "npm run check:performance", `npm run test:e2e -- --shard=${shard}/4`] });

test("publication evidence requires every exact isolated shard once", async () => {
  const calls = [];
  const result = await collectValidationShards(async shard => { calls.push(shard); return shardReceipt(shard); });
  assert.deepEqual(calls, [1, 2, 3, 4]);
  assert.equal(result.shards.length, 4);
  assert.equal(result.checks.at(-1), "npm run test:e2e (all four shards)");
});

for (const invalid of [undefined, shardReceipt(1), { ...shardReceipt(2), checks: shardReceipt(2).checks.slice(0, -1) }, { ...shardReceipt(2), network: "shared" }, { ...shardReceipt(2), result: "FAIL" }]) test(`incomplete shard stops the next execution and cannot produce publication evidence ${JSON.stringify(invalid)}`, async () => {
  const calls = [];
  await assert.rejects(collectValidationShards(async shard => { calls.push(shard); return shard === 1 ? shardReceipt(1) : invalid; }), /BLOCKED_SANDBOX/);
  assert.deepEqual(calls, [1, 2]);
});

test("last shard failure cannot be replaced by earlier passing receipts", async () => {
  await assert.rejects(collectValidationShards(async shard => {
    if (shard === 4) throw new Error("BLOCKED_SANDBOX");
    return shardReceipt(shard);
  }), /BLOCKED_SANDBOX/);
});
