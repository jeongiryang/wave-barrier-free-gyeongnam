...
function harness({ places = [place], bins = [bin], providerStatus = 200, invalidJson = false, partial = false } = {}) {
  ...
  const env = { TOUR_API_SERVICE_KEY_ENCODED: "encoded-test-key", WASTE_BIN_API_URL: "https://official.example.test/bins", WASTE_BIN_API_SOURCE: "경남 공식 가로휴지통" };
