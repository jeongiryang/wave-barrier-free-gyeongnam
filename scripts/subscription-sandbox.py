...
def application_commands(shard=None):
    if shard is not None and (type(shard) is not int or shard not in range(1, 5)):
        fail()
    commands = [["run", "lint"], ["run", "typecheck"], ["test"], ["run", "build:vercel"], ["run", "check:performance"], ["run", "test:e2e"]]
    if shard is not None:
        commands[-1] += ["--", f"--shard={shard}/4"]
    return commands


def application_checks(shard=None):
    return ["npm " + " ".join(command) for command in application_commands(shard)]


def validate(config, archive, edits, shard=None):
    expected = application_checks(shard)
    workspace = pathlib.Path(tempfile.mkdtemp(prefix="wave-validation-", dir=fixed_path(config["scratch"])))
    request = json.dumps({"action": "validate", "config": config, "archive": archive, "edits": edits, "shard": shard})
    ...
    if receipt.get("result") != "PASS" or receipt.get("workspaceBytes") != WORKSPACE_BYTES or receipt.get("shard") != shard or receipt.get("checks") != expected:
        fail()
    for line in lines[:-1]:
...
def validate_in_workspace(config, archive, edits, workspace, shard=None):
    checks = application_commands(shard)
    ...
    for command in checks:
        ...
        name = command[1] if command[0] == "run" else command[0]
        (workspace.parent / (workspace.name + "-" + name.replace(":", "-") + ".log")).write_text(outcome.stdout + outcome.stderr)
...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1] == "--quota-workspace":
        ...
        if request["action"] != "validate" or workspace.parent != fixed_path(config["scratch"]):
            fail()
        checks = validate_in_workspace(config, request["archive"], request["edits"], workspace, request.get("shard"))
        print(json.dumps({"result": "PASS", "workspaceBytes": WORKSPACE_BYTES, "checks": checks, "shard": request.get("shard")}))
        return
    probe(config)
    checks = ["outside-files", "home-appdata-userprofile", "external-network", "local-network", "host-mounts"]
    if request["action"] == "validate":
        checks += validate(config, request["archive"], request["edits"], request.get("shard"))
    print(json.dumps({"result": "PASS", "boundary": "linux-bwrap-v1", "network": "isolated", "filesystem": "isolated", "checks": checks, "shard": request.get("shard")}))
