...
const cases = [
  ...
  ['festival', '아이와 갈 만한 경남 축제랑 주변 장소로 당일 코스를 만들어줘.', empty, ['create-itinerary'], 'baby'],
...
const results = [];
for (const [category, prompt, context, expected, profile] of cases) {
  if (process.argv[2] && !process.argv[2].split(',').includes(category)) continue;
  const started = Date.now();
  try {
    const response = await fetch(`${base}/api/assistant`, { method: 'POST', headers: { 'Content-Type': 'application/json', Origin: base }, body: JSON.stringify({ messages: [{ role: 'user', content: prompt }], context: { ...context, profiles: context.profiles.join(','), themes: context.themes.join(',') } }), signal: AbortSignal.timeout(50000) });
    const answer = await response.json();
    const result = { category, prompt, status: response.status, intentMs: Date.now() - started, answer, intentPass: response.ok && expected.includes(answer.proposal?.action) && (!profile || answer.proposal?.profiles?.includes(profile)) };
