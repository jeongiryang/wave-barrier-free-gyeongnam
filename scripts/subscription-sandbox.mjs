...
export function sandboxCall(action, config, extra = {}, run = spawnSync) {
  ...
  const command = windows ? "wsl.exe" : "/usr/bin/python3";
  const args = windows ? ["--exec", "/usr/bin/python3", "-B", wslPath(bridge)] : ["-B", bridge];
...
export async function validateInSandbox(directory, logRoot, beforeCheck, notify = console.log) {
  ...
  const receipt = sandboxCall("validate", config, { archive: process.platform === "win32" ? wslPath(archive) : archive, edits });
  await beforeCheck();
