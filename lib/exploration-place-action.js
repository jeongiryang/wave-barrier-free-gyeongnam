...
export function explorationPlaceAction({ place, plan, current, region, criteriaKey }) {
  ...
  const matches = [...(plan.explorationPlaces || []), ...(plan.excludedPlaces || [])].filter(candidate => candidate.id === place.id);
  ...
  if (!(plan.explorationPlaces || []).includes(place)) return blocked;
