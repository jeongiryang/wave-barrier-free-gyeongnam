...
import { approvedNeonAuthBaseUrl, securePostgresUrl } from "./environment-validation.js";
...
export function productionEnvironmentErrors(env) {
  ...
  const rawCronSecret = String(env.CRON_SECRET ?? "");
  const cronSecret = rawCronSecret.trim();
  if (rawCronSecret !== cronSecret) {
    errors.push("CRON_SECRET: 앞뒤 공백 또는 줄바꿈이 없어야 합니다.");
  }
  ...
  if (databaseUrl && !securePostgresUrl(databaseUrl)) {
    errors.push("DATABASE_URL: TLS가 강제된 postgres/postgresql 연결 문자열이어야 합니다.");
  }

  const authBaseUrl = String(env.NEON_AUTH_BASE_URL ?? "").trim();
  if (authBaseUrl && !approvedNeonAuthBaseUrl(authBaseUrl)) {
    errors.push("NEON_AUTH_BASE_URL: Neon이 발급한 HTTPS 주소여야 합니다.");
  }
