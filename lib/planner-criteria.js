...
/** UI/restoration keeps an empty selection; the legacy API default stays separate. */
export function selectedThemes(value) {
  const values = (Array.isArray(value) ? value : typeof value === "string" ? value.split(",") : [])
    .filter(item => typeof item === "string").map(item => item.trim());
  return THEME_IDS.filter(id => values.includes(id));
}

/** New multi-theme requests and older single-theme shared links use one parser. */
export function normalizeThemes(value, fallback = "") {
  ...
  return valid.length ? valid : [...THEME_IDS];
}

export function criteriaSignature({ region, themes, selected, locale }) {
  // An empty UI selection is distinct from the legacy API's default theme.
  const hasThemes = Array.isArray(themes) ? themes.length > 0 : Boolean(String(themes || "").trim());
  return JSON.stringify([region, hasThemes ? normalizeThemes(themes).sort() : [], [...new Set(selected)].sort(), locale]);
}
