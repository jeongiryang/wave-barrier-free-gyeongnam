...
/** Missing provider data is not evidence that a facility is absent. @returns {'confirmed'|'unknown'|'negative'} */
export function accessibilityFieldState(value) {
  const text = typeof value === "string" ? value.trim() : "";
  if (!text || /^(?:-|미제공|정보\s*없음|확인\s*필요|해당\s*없음|미확인|알\s*수\s*없음)(?:\s*[.(（]|$)/i.test(text)) return "unknown";
