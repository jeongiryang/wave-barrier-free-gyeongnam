...
export const voiceStateKey = state => JSON.stringify(stableValue({ ...state, activeDay: undefined }));
...
export function planVoiceEdit(state, action, place, day){
 ...
 if(action==='add'&&state.fixed[id])return{ok:false,reason:'이 장소에 고정 약속이 남아 있어요. 일정 편집에서 확인해 주세요.'};
 if(action==='add'&&state.assignments[id]&&!state.days.includes(state.assignments[id]))return{ok:false,reason:'이 장소의 이전 날짜가 현재 여행 기간 밖에 있어요. 일정 편집에서 방문 날짜를 확인해 주세요.'};
 if(action==='remove'){
  if(!state.days.includes(placeDay))return{ok:false,reason:'여행 기간 밖에 있는 장소예요. 방문 날짜를 확인해 주세요.'};
 ...
 if(state.fixed[id]||sameDay.slice(index+1).some(key=>state.fixed[key]))return{ok:false,reason:'고정한 장소와 그 앞의 일정은 음성으로 빼지 않아요. 일정 편집에서 고정 약속을 확인해 주세요.'};
 }
