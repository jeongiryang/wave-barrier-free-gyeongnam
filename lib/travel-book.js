...
export function createTravelBookSnapshot(input, now = new Date().toISOString()) {
  if (!validTripDate(input?.travelStart) || !validTripDate(input?.travelEnd)
  ...
  if (places.some(place => {
    const assigned = input.scheduleAssignments?.[place.id];
    return assigned !== undefined && (!validTripDate(assigned) || assigned < input.travelStart || assigned > input.travelEnd);
  })) return null;
