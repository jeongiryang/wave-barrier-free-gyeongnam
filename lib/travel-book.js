...
import { sanitizeComfort, sanitizeTripBreaks, sanitizeStopPurposes } from "./trip-comfort.js";
...
import { sanitizeTravelMode } from "./trip-travel-mode.js";
...
const legacyThemeIds = { "자연": "nature", "자연·휴양": "nature", "역사·문화": "history", "레포츠": "leisure", "음식": "food" };
...
function sanitizeAssignments(value, places, fallbackDate) {
  ...
  return Object.fromEntries(places.map((place) => {
    ...
    // Preserve a real historic date even when it falls outside the current period.
    return [place.id, assigned || fallbackDate];
  }));
}

export function travelBookRegions(places) {
  return [...new Set(places.map(place => text(place.city, 30)).filter(Boolean))];
}
...
export function sanitizeTravelBook(value, fallbackNow = new Date().toISOString()) {
  ...
  if (!travelStart && (value.travelStart || value.travelEnd)) return null;
  const travelEnd = requestedEnd && requestedEnd >= travelStart ? requestedEnd : travelStart;
  const placeRegions = travelBookRegions(places);
  const region = text(value.region, 30) || places[0].city || "경남";
  const regionLabel = placeRegions.join(" · ") || region;
  ...
  return {
    id: text(value.id, 100) || `book-${typeof value.tripId === "string" && /^[a-f\d]{8}-[a-f\d]{4}-[1-8][a-f\d]{3}-[89ab][a-f\d]{3}-[a-f\d]{12}$/i.test(value.tripId) ? value.tripId : fingerprint}`,
    fingerprint,
    tripId: typeof value.tripId === 'string' && /^[a-f\d]{8}-[a-f\d]{4}-[1-8][a-f\d]{3}-[89ab][a-f\d]{3}-[a-f\d]{12}$/i.test(value.tripId) ? value.tripId : undefined,
    identity: cleanIdentity(value.identity)?.id === value.tripId ? cleanIdentity(value.identity) : null,
    title: title && title !== `${region} ${places.length}곳 여행` ? title : `${regionLabel} ${places.length}곳 여행`,
    region,
    theme: text(value.theme, 40),
    themes: selectedThemes(value.themes ?? text(value.theme, 100).split(",").map(item => legacyThemeIds[item.trim()] || item.trim())),
    profiles: resolveFacilityKeys({ profiles: value.profiles }),
    guidancePreferences: sanitizeGuidancePreferences(value.guidancePreferences),
    travelStart,
    travelEnd,
    dayStartTime: time(value.dayStartTime),
    travelMode: sanitizeTravelMode(value.travelMode),
    createdAt,
    updatedAt,
    status: value.status === "visited" ? "visited" : "planned",
    note: text(value.note, 1200),
    places,
    scheduleAssignments: sanitizeAssignments(value.scheduleAssignments, places, travelStart),
    visitMinutesByPlaceId: sanitizeVisitDurations(value.visitMinutesByPlaceId, places.map(place => place.id)),
    fixedVisits: sanitizeFixedVisits(value.fixedVisits, places.map(place => place.id)),
    dayDeadlines: sanitizeDayDeadlines(value.dayDeadlines),
    comfort: sanitizeComfort(value.comfort),
    breakMinutesByPlaceId: sanitizeTripBreaks(value.breakMinutesByPlaceId, places.map(place => place.id)),
    restPurposeByPlaceId: sanitizeStopPurposes(value.restPurposeByPlaceId, places.map(place => place.id)),
  };
}
...
export function sanitizeTravelBooks(value) {
  ...
  return value.map((item) => sanitizeTravelBook(item)).filter(Boolean).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).filter((item) => {
    if (!item || fingerprints.has(item.tripId || item.fingerprint)) return false;
    fingerprints.add(item.tripId || item.fingerprint);
    return true;
  }).slice(0, TRAVEL_BOOK_MAX_ITEMS);
}

export function createTravelBookSnapshot(input, now = new Date().toISOString()) {
  const undated = input?.travelStart === '' && input?.travelEnd === '';
  if (!undated && (!validTripDate(input?.travelStart) || !validTripDate(input?.travelEnd)
    || boundedTripEnd(input.travelStart, input.travelEnd) !== input.travelEnd)) return null;
  const places = Array.isArray(input?.places) ? input.places.map(sanitizePlace).filter(Boolean).slice(0, TRAVEL_BOOK_MAX_PLACES) : [];
  if (places.some(place => input.scheduleAssignments?.[place.id] && !validTripDate(input.scheduleAssignments[place.id]))) return null;
...
export function upsertTravelBook(current, input, now = new Date().toISOString()) {
  ...
  const previous = books.find((book) => (incoming.tripId ? book.tripId === incoming.tripId || !book.tripId && book.id === incoming.id : !book.tripId && book.fingerprint === incoming.fingerprint));
  const next = previous
  ...
  return sanitizeTravelBooks([next, ...books.filter((book) => (incoming.tripId ? book.tripId !== incoming.tripId && (book.tripId || book.id !== incoming.id) : book.tripId || book.fingerprint !== incoming.fingerprint))]);
}
...
export function buildTravelBookPlannerHref(book) {
  ...
  return `/planner?${query.toString()}#itinerary`;
}
...
export function travelBookRestorePayload(book) {
  ...
  if (!safe) return null;
  return {
    identity: safe.identity, tripId: safe.tripId,
    savedPlaceIds: safe.places.map((place) => place.id),
    themes: safe.themes,
    profiles: safe.profiles,
    guidancePreferences: safe.guidancePreferences,
    savedPlaces: safe.places,
    schedule: {
      ...
      visitMinutesByPlaceId: safe.visitMinutesByPlaceId,
      fixedVisits: safe.fixedVisits, dayDeadlines: safe.dayDeadlines,
      comfort: safe.comfort, breakMinutesByPlaceId: safe.breakMinutesByPlaceId, restPurposeByPlaceId: safe.restPurposeByPlaceId,
    },
