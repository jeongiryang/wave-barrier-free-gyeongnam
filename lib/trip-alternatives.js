...
export function alternativeCandidates({ places = [], original, before, after, savedIds = [], requiredKeys = [], reason = 'distance', visitedIds = [], indoorById = {}, originalVisitMinutes, limit = 3 }) {
  ...
  return places.flatMap(place => {
    ...
    if (fields.some(field => field?.state !== 'confirmed')) return [];
    ...
    return [{ place, distanceKm: distance, travelMinutes: travel, travelDelta: travel === null || originalTravel === null ? null : travel - originalTravel, visitMinutes: minutes, visitDelta: minutes - baselineVisit, indoor, seen: visited.has(place.id) }];
  }).sort((a, b) => (reason === 'rest' ? a.visitMinutes - b.visitMinutes : 0) || (a.travelMinutes ?? Infinity) - (b.travelMinutes ?? Infinity) || a.distanceKm - b.distanceKm).slice(0, Math.min(3, Math.max(1, limit)));
}
