...
export function publicTravelBody(trip) {
  return { selections: { region: trip.region, theme: trip.themes.join(","), profiles: [], locale: "ko", travelStart: trip.travelStart, travelEnd: trip.travelEnd, dayStartTime: trip.dayStartTime, travelMode: sanitizeTravelMode(trip.travelMode), selectedPlaceIds: trip.placeIds, scheduleAssignments: trip.scheduleAssignments, visitMinutesByPlaceId: trip.visitMinutesByPlaceId, fixedVisits: trip.fixedVisits, dayDeadlines: trip.dayDeadlines, breakMinutesByPlaceId: trip.breakMinutesByPlaceId, restPurposeByPlaceId: trip.restPurposeByPlaceId }, origin: { label: "" } };
}
...
export async function sendKakaoMemo(accessToken, template, request = fetch) {
  ...
  } catch { throw new KakaoMessageError("전송 결과를 확인하지 못했어요. 카카오톡 나와의 채팅을 확인해 주세요. 중복을 막기 위해 1분 뒤 다시 보낼 수 있습니다."); }
