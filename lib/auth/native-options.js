...
import { NEW_PASSWORD_MAX, validNewUsername } from "./credential-policy.js";
...
export function createNativeAuth({ database, secret, clientId, clientSecret, sendMail, unlink, origin = AUTH_ORIGIN }) {
  ...
  const options = {
    ...
    emailAndPassword: {
      enabled: true,
      minPasswordLength: 8,
      maxPasswordLength: NEW_PASSWORD_MAX,
    ...
    hooks: {
      before: createAuthMiddleware(async (ctx) => {
        // New identifiers follow the current policy; legacy IDs still sign in.
        const newUsername = ctx.body?.username || (ctx.path === "/sign-up/email" ? ctx.body?.displayUsername : undefined);
        if (["/sign-up/email", "/update-user"].includes(ctx.path) && newUsername !== undefined && !validNewUsername(newUsername)) {
          throw new APIError("BAD_REQUEST", { message: "ID는 영문·숫자·밑줄·마침표 4–12자로 입력해 주세요." });
        }
        ...
        if (ctx.path === "/unlink-account") {
          ...
          if (!accounts.some((account) => account.providerId === "credential" && account.password)) {
            throw new APIError("BAD_REQUEST", { message: "이메일 비밀번호를 설정해 주세요." });
          }
        }
      }),
    },
