...
const PURPOSES = {
  verify: { title: "W.A.V.E 이메일 확인", action: "이메일 확인", path: "/api/auth/verify-email" },
  reset: { title: "W.A.V.E 비밀번호 재설정", action: "비밀번호 재설정", path: "/api/auth/reset-password/" },
  delete: { title: "W.A.V.E 계정 탈퇴 확인", action: "계정 탈퇴 확인", path: "/api/auth/delete-user/callback" },
};
...
export function accountEmail({ kind, to, url, origin = PRODUCTION_ORIGIN, allowLocalhost = false }) {
  ...
  const introduction = kind === "delete"
    ...
    : kind === "reset"
      ? "W.A.V.E 계정의 비밀번호 재설정을 요청하셨습니다. 아래 링크에서 새 비밀번호를 설정해 주세요."
      : "W.A.V.E 계정에 사용할 이메일 주소를 확인해 주세요.";
  return {
    to: { address: mailbox(to) },
    subject: purpose.title,
    text: `${purpose.title}\n\n${introduction}\n\n${purpose.action}: ${target.href}\n\n본인이 요청하지 않았다면 이 메일을 무시해 주세요. 링크는 한 번만 사용할 수 있으며, 만료된 경우 W.A.V.E에서 다시 요청할 수 있습니다.\n\nW.A.V.E`,
    html: `<div lang="ko" style="font-family:Arial,sans-serif;color:#183153;line-height:1.7;max-width:560px;margin:auto;padding:28px"><p style="font-weight:700">W.A.V.E</p><h1 style="font-size:24px">${purpose.title}</h1><p>${introduction}</p><p><a href="${escapeHtml(target.href)}" style="display:inline-block;padding:12px 20px;background:#174ea6;color:#fff;border-radius:8px">${purpose.action}</a></p><p>본인이 요청하지 않았다면 이 메일을 무시해 주세요. 링크는 한 번만 사용할 수 있으며, 만료된 경우 W.A.V.E에서 다시 요청할 수 있습니다.</p></div>`,
    disableFileAccess: true,
    disableUrlAccess: true,
  };
}
...
export function createAccountMailer(env = process.env, createTransport = nodemailer.createTransport) {
  ...
  return async (message) => {
    ...
    try {
      const receipt = await transporter.sendMail({ ...contents, from: { name: "W.A.V.E", address: configuration.auth.user } });
