...
import { NEW_PASSWORD_MAX, EXISTING_PASSWORD_MAX } from "./credential-policy.js";
...
export function checkAuthCredentials(mode, raw) {
  ...
  if (!looksLikeEmail(email) && !(mode === "login" && /^[a-zA-Z0-9_.]{3,30}$/.test(email))) {
    return { error: "이메일 형식을 확인해 주세요. 예: wave@example.com", field: "email" };
  }
  const passwordMax = mode === "register" ? NEW_PASSWORD_MAX : EXISTING_PASSWORD_MAX;
  if (password.length < PASSWORD_MIN || password.length > passwordMax) {
    return { error: `비밀번호는 ${PASSWORD_MIN}자 이상 ${passwordMax}자 이하로 입력해 주세요.`, field: "password" };
  }
