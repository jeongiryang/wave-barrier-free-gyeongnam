...
import { normalizeVisitPhotos } from "./visit-photos.js";
import { FIELD_REPORT_CATEGORY, FIELD_REPORT_CONTENT_ID_PATTERN } from "./field-report-board.js";

export const COMMUNITY_CATEGORIES = ["general", "place", "review", "tips", "together", "travel-talk", FIELD_REPORT_CATEGORY];
...
export function validatePostInput(input, now = Date.now(), options = {}) {
  const requestedTitle = cleanCommunityText(input?.title, 120);
  ...
  if (category === FIELD_REPORT_CATEGORY && options.fieldReportEnabled !== true) return { error: "현장 확인 정보 글쓰기는 현재 준비 중입니다." };
  ...
  if (category === FIELD_REPORT_CATEGORY) {
    if (!FIELD_REPORT_CONTENT_ID_PATTERN.test(placeId)) return { error: "공개 관광지 식별자를 다시 선택해 주세요." };
    if (!visitDate) return { error: "확인한 날짜를 입력해 주세요." };
    if (fieldReports.length || journalPlaces.length) return { error: "현장 확인 정보에는 별도 후기 항목을 함께 저장하지 않습니다." };
  }
  const title = category === FIELD_REPORT_CATEGORY
    ? cleanCommunityText(`${placeName} · ${visitDate} 현장 확인`, 120)
    : requestedTitle;
  ...
  const photoResult = normalizeVisitPhotos(input?.visitPhotos);
  if (photoResult.error) return { error: photoResult.error };
  if (category === FIELD_REPORT_CATEGORY && photoResult.photos.length) return { error: "현장 확인 정보에는 사진을 첨부할 수 없습니다." };
  if (photoResult.photos.length) {
    if (category !== "review" || !placeId || !visitDate) return { error: "현장 사진은 연결된 관광지와 실제 방문일이 있는 여행 후기에 첨부해 주세요." };
    if (input?.photoConsent !== true) return { error: "직접 촬영한 사진의 공개 여부를 확인해 주세요." };
  }
  return { value: {
    ...
    ...(input?.visitPhotos === undefined ? {} : { visitPhotos: photoResult.photos }),
  } };
}
...
export function communityListParams(url) {
  ...
  return { category: COMMUNITY_CATEGORIES.includes(category) ? category : "", search, placeId, page, limit, offset: (page - 1) * limit, ...(['popular', 'comments'].includes(sort) ? { sort } : {}), ...(placeId && url.searchParams.get('placePreview') === '1' ? { placePreview: true } : {}), ...(placeId && url.searchParams.get("history") === "1" ? { history: true } : {}) };
}
