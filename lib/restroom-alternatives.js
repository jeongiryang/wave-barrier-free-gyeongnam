...
export function normalizeRestroomAlternative(value) {
  ...
  const id = text(value.id, 80), name = text(value.name, 120), address = text(value.address).replace(/^(경상남도|경남)(?=창원시)/, '$1 '), destination = point(value.destination);
...
export function rankRestroomAlternatives(values, origin, limit = 3, radiusKm = 5) {
  ...
  return values.flatMap(value => {
    ...
    const distanceFromPlaceMeters = parkingDistanceMeters(start, item.destination);
    return distanceFromPlaceMeters <= radiusKm * 1000 ? [{ ...item, distanceFromPlaceMeters }] : [];
