...
export function visibleFacilityMarkers(selection, cap) {
  ...
  pool.sort(byDistance);
  if (!Number.isFinite(cap) || cap < 0) return pool;
  // 쓰레기통은 장소 수가 많은 보조 레이어다. 전체 상한을 넘을 때 기존 레이어를
  // 밀어내지 않도록 비-쓰레기통 마커를 먼저 보존하고 남은 자리에만 넣는다.
  const primary = pool.filter(marker => marker.layerId !== "trash-bin");
  const trashBins = pool.filter(marker => marker.layerId === "trash-bin");
  return [...primary.slice(0, cap), ...trashBins.slice(0, Math.max(0, cap - primary.length))];
}
...
function normalizeMarker(input, layerId) {
  ...
  return {
    ...
    ...(typeof marker.kind === "string" ? { kind: marker.kind } : {}),
    ...(typeof marker.locationNote === "string" ? { locationNote: marker.locationNote } : {}),
  };
}
