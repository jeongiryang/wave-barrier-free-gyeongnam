...
const facilities = value => Array.isArray(value) && value.length ? [...new Set(value.filter(item => typeof item === 'string' && item.trim()))].sort((a, b) => a.localeCompare(b, 'ko')).join(', ') || '정보 없음' : '정보 없음';
