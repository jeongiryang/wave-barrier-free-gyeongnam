/** JPEG·PNG·WebP·TIFF 컨테이너에서 촬영 시각과 좌표 EXIF만 읽는다. 사진은 브라우저 밖으로 보내지 않는다. */
const SOI = 0xffd8;
const APP1 = 0xffe1;
const SOS = 0xffda;
...
function isTiffStart(view, offset) {
  if (offset < 0 || offset + 4 > view.byteLength) return false;
  const order = view.getUint16(offset);
  if (order === 0x4949) return view.getUint16(offset + 2, true) === 0x002a;
  if (order === 0x4d4d) return view.getUint16(offset + 2, false) === 0x002a;
  return false;
}

function stripExifPrefix(view, offset) {
  return ascii(view, offset, 4) === "Exif" && offset + 6 <= view.byteLength ? offset + 6 : offset;
}
...
function findPngExifStart(view) {
  ...
  if (view.byteLength < 8 || signature.some((value, index) => view.getUint8(index) !== value)) return -1;
  let offset = 8;
  while (offset + 12 <= view.byteLength) {
    const length = view.getUint32(offset, false);
    const type = ascii(view, offset + 4, 4);
    const data = offset + 8;
    if (length > view.byteLength || data + length > view.byteLength) return -1;
    if (type === "eXIf") return stripExifPrefix(view, data);
...
function findExifStart(view) {
  if (isTiffStart(view, 0)) return 0;
  const jpeg = findJpegExifStart(view);
  if (jpeg >= 0) return jpeg;
  const png = findPngExifStart(view);
  if (png >= 0) return png;
  return findWebpExifStart(view);
}
...
export function readPhotoExif(buffer) {
  ...
  if (tiff < 0 || tiff + 8 > view.byteLength || !isTiffStart(view, tiff)) return empty;
