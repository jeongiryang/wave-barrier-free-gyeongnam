/**
 * JPEG EXIF에서 촬영 시각과 좌표만 읽는다. 외부 패키지를 쓰지 않는 이 저장소의
 * 방식을 따르고, 무엇보다 **사진이 브라우저 밖으로 나가지 않아야** 하므로
 * 판독은 전부 여기에서 끝난다.
 *
 * 필요한 것만 읽는다. 카메라 모델, 렌즈, 일련번호 같은 나머지 EXIF는 읽지도
 * 않으므로 실수로 어딘가에 실려 나갈 여지가 없다.
 */
...
/** "2026:08:14 09:31:02" -> { date: "2026-08-14", minutes: 571 } */
...
/**
 * 사진 한 장에서 촬영 시각과 좌표를 읽는다. 읽지 못하면 값을 지어내지 않고
 * 해당 항목을 `null`로 둔다.
 */
export function readPhotoExif(buffer) {
  ...
  if (Number.isFinite(gpsOffset)) {
    const gps = readIfd(view, tiff, tiff + gpsOffset, little, [
      TAG_GPS_LAT_REF, TAG_GPS_LAT, TAG_GPS_LNG_REF, TAG_GPS_LNG,
    ]);
