/** JPEG EXIF에서 촬영 시각과 좌표만 읽는다. 사진은 브라우저 밖으로 보내지 않는다. */
const SOI = 0xffd8;
const APP1 = 0xffe1;
const SOS = 0xffda;
...
function findExifStart(view) {
  ...
  while (offset + 4 <= view.byteLength) {
    ...
    if (marker === APP1 && offset + 10 <= view.byteLength) {
      const tag = String.fromCharCode(view.getUint8(offset + 4), view.getUint8(offset + 5), view.getUint8(offset + 6), view.getUint8(offset + 7));
