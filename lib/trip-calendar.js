...
export function buildTripCalendarIcs({
  ...
  if (!TIME_PATTERN.test(dayStartTime || "")) throw new Error("올바른 여행 시작 시각이 필요합니다.");
  const parsedShareUrl = shareUrl ? new URL(shareUrl) : null;
  if (parsedShareUrl && !/^https?:$/.test(parsedShareUrl.protocol)) throw new Error("공유 링크는 HTTP(S) 주소여야 합니다.");
  ...
  const lines = [
    ...
    "BEGIN:VEVENT", `UID:wave-${travelStart}-${travelEnd}-${stableTextHash(safeShareUrl || JSON.stringify([title, region, placeNames]))}@wave-barrier-free-gyeongnam`, `DTSTAMP:${utcStamp(createdAt)}`,
    ...
    `SUMMARY:${escapeIcsText(title)}`, `DESCRIPTION:${escapeIcsText(description)}`, ...(safeShareUrl ? [`URL:${safeShareUrl}`] : []),
