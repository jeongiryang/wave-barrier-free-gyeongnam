...
export function planTripCommand(state, command, places = []) {
  if (!state || !command || state.saved.length !== state.order.length || new Set(state.saved).size !== state.saved.length || state.saved.some(id => !state.order.includes(id))) return fail('저장한 장소를 먼저 불러와 주세요.');
  ...
  if (command.type === 'add') {
    ...
    if (state.fixed[id]) return fail('이 장소에 고정 방문이 남아 있어요. 기존 약속을 먼저 확인해 주세요.');
    if (state.assignments[id] && !state.days.includes(state.assignments[id])) return fail('이 장소에 여행 기간 밖의 방문일이 남아 있어요. 기존 날짜를 먼저 확인해 주세요.');
    ...
    if (sameDay.slice(sameDay.indexOf(id) + 1).some(key => state.fixed[key])) return fail('뒤에 고정한 방문이 있어요. 고정 약속을 먼저 확인해 주세요.');
