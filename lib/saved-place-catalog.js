...
function temporaryStop(value) {
  if (value?.kind !== "official-restroom") return undefined;
  const sourceId = text(value.sourceId, 80), referenceDate = text(value.referenceDate, 10);
  if (!sourceId || !/^\d{4}-\d{2}-\d{2}$/.test(referenceDate)) return undefined;
  const evidence = Object.fromEntries(["accessibleToilet", "entranceStep", "entranceDoor", "grabBars", "turningSpace", "sinkAccess", "elevatorRequired", "emergencyBell"].map(key => [key, text(value.evidence?.[key], 24) || "unknown"]));
  return { kind: "official-restroom", sourceId, referenceDate, evidence };
}
...
export function sanitizeSavedPlaceSnapshot(value) {
  ...
  const temporary = temporaryStop(value.temporaryStop);
  return {
    id,
    name,
    ...
    ...(temporary ? { temporaryStop: temporary } : {}),
  };
}
...
export function mergeSavedPlaceCatalog(current, places) {
  const stored = sanitizeSavedPlaceCatalog(current);
  const byId = new Map(stored.map(place => [place.id, place]));
  const incoming = sanitizeSavedPlaceCatalog(places).map(place => {
    const previous = byId.get(place.id);
    // A missing location is not a correction of an ID-bound public location.
    // Retain only coordinates, never older facility evidence or raw provider data.
    return !supportedPlacePoint(place.mapX, place.mapY) && previous && supportedPlacePoint(previous.mapX, previous.mapY)
      ? { ...place, mapX: previous.mapX, mapY: previous.mapY } : place;
  });
  return sanitizeSavedPlaceCatalog([...incoming, ...stored]);
}
