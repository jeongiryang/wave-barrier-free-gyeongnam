...
export function tripBoardCells(progress, stops) {
  if (!Array.isArray(stops) || !stops.length) return [];
  const ids = stops.map(stop => typeof stop?.id === 'string' ? stop.id : '').filter(Boolean);
  const clean = cleanOnTrip(progress, ids);
  const currentId = stops.find(stop => stop?.id && !clean.marks[stop.id])?.id || '';
  return stops.flatMap((stop, index) => {
    if (!stop || typeof stop.id !== 'string' || !stop.id || typeof stop.title !== 'string' || !stop.title.trim()) return [];
    const marked = clean.marks[stop.id]?.state;
    return [{ id: stop.id, order: index + 1, title: stop.title.trim(), state: marked === 'done' ? 'done' : marked === 'skipped' ? 'skipped' : stop.id === currentId ? 'current' : 'upcoming' }];
  });
}
