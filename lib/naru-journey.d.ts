...
export type NaruJourneyOutcome = { kind: 'proposal' | 'unavailable' } | { kind: 'unchanged'; reason: 'already-indoor'; kept: Array<{ id: string; name: string; date: string }> };
export type NaruJourney = { adjustment?: import('./naru-gentle-plan.js').GentleTripPlan; outcome?: NaruJourneyOutcome; removed?: Array<{ place: Place; date: string }>; restOnly?: boolean; restDay?: string; start: string; end: string; region: string; profiles: string[]; themes: string[]; transport: 'walk'|'bicycle'|'transit'|'car'; relaxed: boolean; originRegion?: string; stops: NaruStop[]; plan: PlanData; weather: WeatherData | null; warnings: string[]; generatedAt: string; action: AssistantAction['action'] };
export type ExistingStop = { id: string; date: string; fixed: boolean; place?: Place };
export function journeyOutcome(input: { action: AssistantAction['action']; indoor?: boolean; existing?: ExistingStop[]; indoorById?: Record<string, { state: string; detail?: string }>; stops?: NaruStop[]; removed?: Array<{ place: Place; date: string }>; restOnly?: boolean; providerFailed?: boolean; conditionsChanged?: boolean }): NaruJourneyOutcome;
