 ...
 * @returns {{ state: "connected" | "ready" | "error", detail: string, queryStatus: "success" | "error", resultCount: number | null } | null}
 */
export function odsayProviderStatus({ configured, error = null, failure = "", routeCount = 0 }) {
  if (!configured) return null;
  if (failure) return { state: "error", detail: failure.slice(0, 120), queryStatus: "error", resultCount: null };
  if (error) {
    if (["3", "4", "5", "6", "-98", "-99"].includes(error.code)) {
      return { state: "ready", detail: "이 구간에서 제공되는 대중교통 경로가 없습니다. 출발·도착 장소를 바꾸거나 외부 지도에서 확인해 주세요.", queryStatus: "success", resultCount: 0 };
    }
    if (error.code === "INTERCITY_INCOMPLETE") {
      return { state: "error", detail: "터미널 앞뒤 이동을 포함한 전체 경로는 아직 확인되지 않았습니다. 외부 지도에서 이어지는 이동을 확인해 주세요.", queryStatus: "error", resultCount: null };
    }
    ...
    if (Object.hasOwn(validationDetails, error.code)) {
      return { state: "error", detail: validationDetails[/** @type {keyof typeof validationDetails} */ (error.code)], queryStatus: "error", resultCount: null };
    }
    // Provider messages can echo request values. Never expose them or arbitrary codes.
    return { state: "error", detail: "대중교통 경로 응답을 확인하지 못했습니다. 잠시 후 다시 시도하거나 외부 지도에서 확인해 주세요.", queryStatus: "error", resultCount: null };
  }
  if (routeCount > 0) {
    return { state: "connected", detail: `대중교통 경로 ${routeCount}개를 확인했습니다.`, queryStatus: "success", resultCount: routeCount };
  }
  return { state: "ready", detail: "이 구간에서 제공되는 대중교통 경로가 없습니다.", queryStatus: "success", resultCount: 0 };
}
