import { resolveFacilityKeys, spokenFacilityKeys } from './facility-selection.js';
...
function withoutPreservedDates(text) {
  // An explicit preservation clause is not a date edit. Match only coordinated
  // trip nouns, so a date mutation before another item's "유지하고" stays visible.
  const keptDates = /(?:날짜|기간)(?:\s*(?:와|과|및|,)\s*(?:(?:기존|고정|담은|선택한)\s*)?(?:날짜|기간|장소|방문|체류\s*시간|휴식\s*시간))*(?:만|를|을|는|은|도)?\s*(?:(?:모두|전부|그대로)\s*)*(?:유지|보존)(?:하고|하며|하면서|하되)(?!\s*(?:싶|있|없|않|안|못))/g;
  return text.replace(keptDates, '');
}

function dateOnlyRequest(text, context) {
  const turn = withoutPreservedDates(text).trim().replace(/[.!。！]+$/, '').trim();
  const subject = '(?:(?:이번|현재)\\s*)?(?:여행\\s*)?(날짜|기간)(?:만|를|을|는|은)?';
  const prefix = '(?:(?:그럼|그러면|이번에는|이제)\\s*)?';
  // "동선을 조정해" does not contain the standalone command "정해". The
  // particle 로 or the date subject still permits compact date assignments.
  const decide = '(?:(?<![가-힣])|(?<=로)|(?<=(?:날짜|기간)(?:만|를|을|는|은)?))정해';
  const command = `(?:바꿔|변경해|설정해|${decide}|줄여|늘려)\\s*(?:줘|주세요)`;
  ...
  if (!expression) {
    ...
    if (/(?:날짜|기간)/.test(turn) && new RegExp(`바꿔|바꿀|바꾸|변경|설정|${decide}|줄여|늘려`).test(turn)
      || /\b(?:change|set)\b.*\b(?:dates?|period)\b/i.test(turn)) return null;
    return undefined;
  }
...
export function groundAssistantProposal(value, messages, context) {
  ...
  const latest = turns.at(-1) || '';
  if (value && isChangeNegated(latest) && !['search','details','readiness','compare','next','tool','help'].includes(value.action)) return null;
  const keepsDates = withoutPreservedDates(latest) !== latest;
  // Preserving the trip cannot authorize a model-created date edit or replacement
  // trip. Keep ambiguous/mixed date instructions on the clarification path too.
  if (keepsDates && ['set-dates', 'create-itinerary'].includes(value?.action)) return null;
  const dates = dateOnlyRequest(latest, context);
  if (dates !== undefined) return keepsDates ? null : dates;
  // Only the explicit date parser above can authorize a date assignment.
  if (value?.action === 'set-dates') return null;
  ...
  const action = { ...value };
  // Search output from a small local model can omit the visitor's exact criteria.
  // Repair only a positive current request, never old suggestions or departure cities.
  if (['search', 'settings'].includes(action.action) && !isChangeNegated(latest)
    && /추천|찾아|검색|둘러|보여|조건.*(?:적용|설정)|\b(?:find|search|show)\b/i.test(latest)) {
    const excluded = namedRegions.filter(region => new RegExp(`${region}(?:시|군)?(?:이나|나|과|와|,|\\s)*(?:(?:${namedRegions.join('|')})(?:시|군)?(?:이나|나|과|와|,|\\s)*)*(?:은|는|을|를)?\\s*(?:제외|말고|빼고)`).test(latest));
    const limit = latest.match(/(?<![가-힣\d])(\d{1,2}|열두|열한|열|아홉|여덟|일곱|여섯|다섯|네|세|두|한)\s*(?:곳|개)(?:만)?/);
    const numerals = { 한: 1, 두: 2, 세: 3, 네: 4, 다섯: 5, 여섯: 6, 일곱: 7, 여덟: 8, 아홉: 9, 열: 10, 열한: 11, 열두: 12 };
    const count = limit ? (numerals[limit[1]] || Number(limit[1])) : 0;
    const requestedCount = count >= 1 && count <= 12 ? count : undefined;
    const destinations = namedRegions.filter(region => {
      if (excluded.includes(region)) return false;
      const at = latest.indexOf(region);
      return at >= 0 && !/(?:출발지(?:는|를|가|:)?\s*)$/.test(latest.slice(0, at))
        && !/^(?:시|군)?(?:중앙)?(?:역)?에서\s*(?:출발|떠나)/.test(latest.slice(at + region.length));
    });
    // Excluding a city does not authorise searching that same city or silently
    // broadening the current region when no positive destination was supplied.
    if (!destinations.length && excluded.length) return null;
    const spoken = spokenFacilityKeys(latest);
    const themes = [['nature', /자연|공원|정원|숲/], ['history', /역사|문화|박물관|미술관|전시/], ['leisure', /레포츠|스포츠/], ['food', /음식|식당|먹거리/]]
      .filter(([, pattern]) => pattern.test(latest)).map(([key]) => key);
    if (destinations.length === 1 || spoken.length || themes.length || requestedCount) return {
      action: 'settings',
      ...(destinations.length === 1 ? { region: destinations[0] } : {}),
      ...(requestedCount ? { count: requestedCount } : {}),
      ...(spoken.length ? { profiles: resolveFacilityKeys({ facilityKeys: [...resolveFacilityKeys({ profiles: context.profiles }), ...spoken] }) } : {}),
  ...
  const asksForJourney = /만들|짜\s*(?:줘|주세요)|계획|(?:여행|일정|코스).*준비\s*해\s*(?:줘|주세요)|추천|찾아|검색|추가|넣어|대체|대안|교체|줄여|늘려|휴식|쉬|피곤|실내|비\s*(?:가|오는)|휴무|휴관|당일(?:치기)?(?:로)?\s*여행(?:을)?\s*하고\s*싶|(?:일정|장소|코스).*(?:바꿔|변경)|\b(?:plan|create|build|recommend|add|replace|itinerary)\b/i.test(latest);
  ...
  if (['settings', 'create-itinerary', 'adapt-itinerary'].includes(action.action)) {
    const existing = resolveFacilityKeys({ profiles: context.profiles });
    const spoken = spokenFacilityKeys(latest);
    const required = resolveFacilityKeys({ facilityKeys: [...existing, ...spoken] });
    if (required.length) action.profiles = required; else delete action.profiles;
  }
  ...
  // A weather/rest request targets a day of the existing trip, not a new period.
  // In particular "오늘 피곤해" must not move a saved future trip to today.
  if (action.action === 'adapt-itinerary' && context.days?.length && !/(?:기간|날짜).*(?:바꿔|변경|줄|늘)|[1-7]\s*일.*(?:줄여|늘려)/.test(latest)) {
    const target = action.date || action.start;
    if (target && context.days.includes(target)) action.date = target;
    else if (action.date) delete action.date;
    delete action.start; delete action.end;
  }
  if (keepsDates && action.action === 'adapt-itinerary') {
    // The current itinerary can have been edited after dates mentioned in chat.
    // A whole-trip preservation request must not restore or target those dates.
    delete action.start; delete action.end; delete action.date;
  }
