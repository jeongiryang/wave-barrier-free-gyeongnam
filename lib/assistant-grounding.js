...
export function groundAssistantProposal(value, messages, context) {
  ...
  const asksForJourney = /만들|짜\s*(?:줘|주세요)|계획|추천|찾아|검색|추가|넣어|대체|대안|교체|줄여|늘려|휴식|쉬|피곤|실내|비\s*(?:가|오는)|휴무|휴관|(?:일정|장소|코스).*(?:바꿔|변경)|\b(?:plan|create|build|recommend|add|replace|itinerary)\b/i.test(latest);
