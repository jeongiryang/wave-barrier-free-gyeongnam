...
export function validateJourneyApplication(draft, state) {
  ...
  if (draft.adjustment) {
    const proposal = draft.adjustment;
    const days = journeyDays(draft.start, draft.end);
    if (!draft.restOnly || draft.removed?.length || !Array.isArray(proposal.order) || !proposal.assignments || typeof proposal.assignments !== 'object' || proposal.order.length !== state.saved.length || new Set(proposal.order).size !== state.saved.length
      || proposal.order.some(id=>!state.saved.includes(id) || !days.includes(proposal.assignments[id]))
      || state.saved.some(id=>state.fixed[id] && (proposal.assignments[id] !== (state.assignments[id] || state.start) || proposal.order.indexOf(id)!==state.saved.indexOf(id)))) return '장소와 고정 방문을 모두 보존하는 조정안인지 다시 확인해 주세요.';
  }
  ...
  for (const stop of draft.stops) {
    ...
    if (stop.replaces) {
      if (!state.saved.includes(stop.replaces) || state.fixed[stop.replaces] || replaced.has(stop.replaces)
        || (state.assignments[stop.replaces] || state.start) !== stop.date) return '고정 방문이나 변경된 일정을 확인해 주세요.';
