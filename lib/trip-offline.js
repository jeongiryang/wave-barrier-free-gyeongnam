...
export function offlineTripText({ title, schedule, travelMode, info = {}, savedAt = new Date().toISOString(), progress = {}, budget, splitLines = [] }) {
  const lines = ['WAVE · ' + text(title, 100), `저장 ${validTimestamp(savedAt)}`, '이 파일은 저장 당시의 여행 요약입니다. 실시간 교통·운영 변경은 연결된 뒤 다시 확인하세요.', ''];
  if (travelMode !== undefined) lines.push(`선택한 이동수단: ${travelModeLabel(travelMode)}`, '');
  for (const { day, entries } of schedule.slice(0, 7)) {
    lines.push(day);
    if (!entries.length) lines.push('아직 방문 장소가 없는 날입니다.', '');
