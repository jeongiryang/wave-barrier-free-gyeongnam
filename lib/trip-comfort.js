...
export function sanitizeStopPurposes(value, allowedIds) {
  return Object.fromEntries(allowedEntries(value, allowedIds).filter(([, purpose]) => ["rest", "restroom", "nap", "nursing"].includes(purpose)).slice(0, 100));
}
...
export function combineCompanionNeeds(participants, selected = [], comfort = emptyComfort) {
  ...
  return { selected: resolveFacilityKeys({ profiles: [...choices] }), comfort: { ...current, maxWalkMinutes: limits.length ? Math.min(...limits) : null } };
}
