...
import { readTripValue, replaceCurrentTrip, tripStorageFailed, REGION_KEY, THEMES_KEY, FACILITIES_KEY, GUIDANCE_KEY } from './current-trip-storage.js';
...
export function replaceTripWithBackup(storage, values) {
  ...
  const rawProfiles = JSON.parse(values[FACILITIES_KEY] || '[]');
  const rawGuidance = JSON.parse(values[GUIDANCE_KEY] || '{}');
  const safeGuidance = sanitizeGuidancePreferences(rawGuidance);
  ...
  if (!Array.isArray(rawProfiles) || JSON.stringify(resolveFacilityKeys({ facilityKeys: rawProfiles })) !== JSON.stringify(rawProfiles)
    || !rawGuidance || typeof rawGuidance !== 'object' || Array.isArray(rawGuidance) || Object.keys(safeGuidance).length !== Object.keys(rawGuidance).length) throw new Error('편의와 안내 방식을 확인해 주세요.');
  ...
  if (ids.length && (identity?.id !== incomingIdentity?.id || JSON.stringify(ids) !== JSON.stringify(incoming) || currentSchedule !== values['wave-trip-schedule-v1'])) {
    ...
    const previous = createTravelBookSnapshot({ ...JSON.parse(currentSchedule), tripId: identity?.id, identity, id: identity?.binding?.kind === 'local' ? identity.binding.id : undefined, region, themes, theme, profiles: resolveFacilityKeys({ facilityKeys: JSON.parse(readTripValue(storage, FACILITIES_KEY) || '[]') }), guidancePreferences: sanitizeGuidancePreferences(JSON.parse(readTripValue(storage, GUIDANCE_KEY) || '{}')), title: savedBook?.title || `${region} · 이어서 만들던 여행`, places: orderedIds.map(id => catalog.find(place => place.id === id) || { id, name: `이름 확인이 필요한 장소 (${id})`, city: region }) });
