...
export const SERVER_BUDGET_MS = {
  ...
  // Two dependent lookups share one deadline; keep the client within20s.
  visitInfo: 17_500,
  returnTransport: 17_500,
  // 장소 재조회 → 주변 음식점 목록 → 편의 확인까지 세 단계가 한 마감을 나눠 쓴다.
  // 모자라면 확인하지 못한 편의를 unknown으로 남긴다. 없음으로 바꾸지 않는다.
  diningAccessibility: 17_500,
  parkingAlternatives: 17_500,
  restroomAlternatives: 17_500,
  sanitarySupply: 17_500,
  smokingArea: 17_500,
  trashBin: 17_500,
  route: UPSTREAM_TIMEOUT_MS.transport,
  location: UPSTREAM_TIMEOUT_MS.location,
  officialLocation: 12_000,
...
/**
 * 한 제공처의 지연 때문에 이미 성공한 다른 제공처 결과를 버리지 않는다.
 * @template T
 * @param {Promise<T>[]} work
 * @param {number} remainingMs
 * @param {() => T} onExpired
 * @returns {Promise<T[]>}
 */
export function eachWithinBudget(work, remainingMs, onExpired) {
  return Promise.all(work.map((item) => withinBudget(item, remainingMs, onExpired)));
}
