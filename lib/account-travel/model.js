...
export const TRAVEL_REGIONS = ["경남 전체", "창원", "진주", "통영", "사천", "김해", "밀양", "거제", "양산", "의령", "함안", "창녕", "고성", "남해", "하동", "산청", "함양", "거창", "합천"];
...
export function accountTripPayload(value) {
  ...
  return { version: 1, title: title || `${region} 여행`, region, travelStart, travelEnd,
    dayStartTime: typeof value.dayStartTime === "string" && /^([01]\d|2[0-3]):[0-5]\d$/.test(value.dayStartTime || "") ? value.dayStartTime : "10:00",
    travelMode: sanitizeTravelMode(value.travelMode),
    themes: selectedThemes(value.themes), profiles: resolveFacilityKeys({ profiles: value.profiles }),
    guidancePreferences: sanitizeGuidancePreferences(value.guidancePreferences), comfort: sanitizeComfort(value.comfort), placeIds: ids, scheduleAssignments,
