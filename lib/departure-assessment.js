...
function evidenceReadiness(places, current, en) {
  ...
  const verified = sourced.filter((place) => Number.isFinite(Date.parse(place.checkedAt)) && Array.isArray(place.accessibility) && place.accessibility.some(item => typeof item?.key === "string" && item.key.trim()));
  ...
  if (!current || !verified.length) return {
    ...
    summary: !current
      ...
      : en ? `Item-level official evidence and retrieval times need checking for all ${list.length} places.` : `${list.length}곳 모두 항목별 공식 편의근거와 조회 시각을 다시 확인해야 합니다.`,
  ...
  for (const place of verified) {
    ...
    for (const state of fields.values()) counts[state]++;
  }
  ...
  return {
    ...
    summary: en
      ? `${list.length} places · Reported available ${counts.confirmed} · Unknown ${counts.unknown} · Reported unavailable ${counts.negative}. ${missing} places lack item-level evidence. Official records do not guarantee access on site.`
...
export function assessDepartureReadiness({
  ...
  crowd = null, crowdPlaceId, generatedAt = "", routeCoverage, places = [], placeCriteriaCurrent = false, scheduleAssignments = {}, locale = "ko",
} = {}) {
  ...
  const items = [
    ...
    evidenceReadiness(places, placeCriteriaCurrent, en),
  ];
