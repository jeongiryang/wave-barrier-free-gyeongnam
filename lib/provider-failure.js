...
export function classifyProviderResponse(context, { status = 200, retryAfter = null, body, raw = "", now = Date.now() } = {}) {
  ...
  if (context.family === "public-data") {
    // An explicit wrapper is authoritative even when malformed. Its adapter
    // must reject it rather than borrowing an unrelated root success/error.
    const envelope = record(body) && Object.hasOwn(body,"response") ? body.response : body;
    const header = record(envelope?.header) ? envelope.header : envelope;
    // JSON strings can contain XML-looking text; only an unparsed XML reply
    // may supply the XML fallback code.
    const xmlCode = body === undefined ? raw.match(/<(?:returnReasonCode|resultCode)>\s*([^<]+)\s*</i)?.[1] : undefined;
    const code = String(header?.resultCode ?? xmlCode ?? "").trim();
