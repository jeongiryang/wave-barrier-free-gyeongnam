...
const allowedQueries = new Set(['action', 'contentId', 'radiusKm']);
...
export async function handleRestroomAlternatives(url: URL, env: Env) {
  ...
  if (queryKeys.length < 2 || queryKeys.length > 3 || queryKeys.some(key => !allowedQueries.has(key)) || url.searchParams.getAll('radiusKm').length > 1 || ![1, 3, 5, 10, 20].includes(radiusKm) || url.searchParams.getAll('action').length !== 1 || url.searchParams.getAll('contentId').length !== 1 || !/^[1-9]\d{0,11}$/.test(contentId)) {
  ...
  const resultSnapshot = await snapshots.get<RestroomAlternative[]>(`restroom:${contentId}:${radiusKm}`, 24 * 60 * 60000, remaining, async () => rankRestroomAlternatives(restroomArtifact, { latitude: point.lat, longitude: point.lng }, 3, radiusKm));
  ...
  return json({ status: items.length ? 'available' : 'empty', contentId, radiusKm, checkedAt: restroomManifest.generatedAt, source: '전국공중화장실표준데이터', items });
}
